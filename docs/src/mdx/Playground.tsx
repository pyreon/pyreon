...
// Live code playground: textarea-editor (left/top) + sandboxed iframe
// (right/bottom) running the user's code against `@pyreon/reactivity`
// loaded from esm.sh. The iframe's `<script type="module">` carries a
// minimal `h()` / `mount()` shim so snippets execute against real DOM
// with effect-driven text + attr bindings.
//
// CSS visual identity (tokens, fonts) lives in `tokens.css`.
