...
Common mistakes and anti-patterns across Pyreon, grouped by area, each with its fix. Distilled from the framework's own anti-pattern catalog (368 entries) — the same source MCP `get_anti_patterns` serves to AI agents. Many are caught automatically by [`@pyreon/lint`](/docs/lint), `pyreon doctor`, or MCP `validate`; the detector code is noted on each entry that has one.
...
- **[Lifecycle & Cleanup Mistakes](/docs/troubleshooting/lifecycle)** — 37 entries
- **[Build Pipeline Mistakes](/docs/troubleshooting/build)** — 57 entries
