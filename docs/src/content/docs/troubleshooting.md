...
Common mistakes and anti-patterns across Pyreon, grouped by area, each with its fix. Distilled from the framework's own anti-pattern catalog (375 entries) — the same source MCP `get_anti_patterns` serves to AI agents. Many are caught automatically by [`@pyreon/lint`](/docs/lint), `pyreon doctor`, or MCP `validate`; the detector code is noted on each entry that has one.
...
- **[Testing Mistakes](/docs/troubleshooting/testing)** — 25 entries
...
- **[Documentation Mistakes](/docs/troubleshooting/documentation)** — 17 entries
