...
| `bollinger(y, window, k?)` | The ±k·σ envelope as a filled `band` plus its middle line — an ARRAY of two marks to spread. Its bounds are computed from the series (a rolling window), which is what `band` supports through `transform`/`transform2`. Web-only: it returns an array to spread, which is a different shape from a mark call, so it warns by name on native rather than lowering to something it is not. |
