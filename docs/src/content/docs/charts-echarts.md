...
description: "@pyreon/charts/echarts — a reactive wrapper around the Apache ECharts library, with lazy loading, auto-detection, and typed options."
---

`@pyreon/charts/echarts` is a reactive wrapper around the [Apache ECharts](https://echarts.apache.org/) for Pyreon. You write ECharts options as a **function** — signal reads inside are tracked, so the chart re-renders whenever any tracked signal changes. ECharts itself is **lazy-loaded**: chart types and components are auto-detected from your options and dynamically imported on first render, so there are zero ECharts bytes in your initial bundle until a chart actually mounts.

<PackageBadge name="@pyreon/charts" href="/docs/charts-echarts" />

:::tip[Start with `<EChart>`]
This page is about the ECharts library wrapper. For new charts, [`<Chart>` from `@pyreon/charts`](/docs/charts) is Pyreon's own engine: no ECharts in your bundle, SVG on the server, and the same chart on iOS and Android. `<EChart>` is for when you need the ECharts library itself.
:::
...
`@pyreon/charts/echarts/vite` ships `chartsViteAlias()`, which resolves `tslib` to the flat-ESM `tslib.es6.js` and sidesteps the broken indirection. Spread it into `resolve.alias`:
...
If the alias is missing, `@pyreon/charts/echarts` detects the canonical tslib symptom at load time and **re-throws with an actionable message** telling you to add `chartsViteAlias()` — instead of leaving you with a raw `__extends` destructure error. The original error is preserved in the message.
:::
...
The `<EChart />` component is the simplest way to render a chart. Pass an `options` **function** that returns a standard ECharts configuration. Signal reads inside the function are tracked, so the chart updates reactively.
...
`@pyreon/charts/echarts` tracks the signals you read **inside** the options function. If you pass a static object (`options={ { ... } }`), the signal reads happen once at render and are never tracked — the chart will render once and never update when your data changes. Always wrap in `() => ({ ... })`.
:::
...
`<EChart />` binds handlers to the underlying ECharts instance. The `onEvents`
map is the **general form** — it binds *any* ECharts event by name; `onClick`,
`onMouseover`, and `onMouseout` are convenience shorthands (they map to
`click`/`mouseover`/`mouseout` and win on a key collision).
...
Each handler receives `(params, instance)` — `params` is a `ChartEventParams`
object (a duck-typed shape carrying the common ECharts event fields `name`,
`value`, `seriesIndex`, `dataIndex`, `seriesName`, `componentType`, `color`,
`event`, … plus an index signature for anything else ECharts attaches), and
`instance` is the live ECharts instance (call `instance.dispatchAction(...)`
without capturing a ref).

Binding is **leak-safe**: if a handler prop changes, the old listener is removed
before the new one is bound (no pile-up), and every listener is removed on
unmount.

:::note[`onEvents` covers every ECharts event]
Previously only `onClick`/`onMouseover`/`onMouseout` were wired. `onEvents`
replaces the "drop to `useChart` and call `inst.on(...)`" workaround for events
like `legendselectchanged`, `datazoom`, `brushselected`, `finished`, etc. The
`useChart` imperative path is still available for advanced cases (binding on a
timer, one-shot listeners), but is no longer required for arbitrary events.
:::

## Loading overlay — `showLoading`

`showLoading` reactively toggles ECharts' built-in loading spinner. It is
distinct from `useChart`'s `loading` signal, which tracks lazy **module**
loading *before* the instance exists — `showLoading` controls the overlay on an
already-mounted chart while your **data** is in flight.
...
ECharts cannot hot-swap a theme — it must dispose and recreate the instance. So
`theme` is read **once** at init. To switch themes at runtime (e.g. dark mode),
...
`useChart<TOption>(optionsFn, config?)` is the low-level hook with full lifecycle control. `<EChart />` is built on top of it. Use the hook when you need the raw ECharts instance, a manual `resize()`, or access to the `loading` / `error` signals.

```tsx
import { useChart } from '@pyreon/charts/echarts'
...
By default `useChart` (and `<EChart />`) accept the broad `EChartsOption` type. To get **exact autocomplete** for only the series types you use, pass a `TOption` built with ECharts' `ComposeOption<>` helper:
...
The same generic works on the component via `<EChart<DashboardOption> options={…} />`.
...
| `replaceMerge`| `string \| string[]`                  | —          | Component types to REPLACE (not merge) per update, e.g. `'series'`.                  |
...
| `showLoading` | `boolean`                             | `false`    | Reactive — toggles ECharts' built-in loading overlay.                                |
| `loadingOption`| `Record<string, unknown>`            | —          | Options for the loading overlay (text, color, …).                                    |
| `ariaLabel`   | `string`                              | —          | Text alternative → the container becomes `role="img"` with this label.               |
| `onEvents`    | `Record<string, (params, instance) => void>` | —   | **Any** ECharts event, keyed by name (`legendselectchanged`, `datazoom`, …).         |
...
:::warning[Set a height on the `<EChart />` style]
Same rule as `useChart` — ECharts requires explicit container dimensions. `<EChart options={…} />` with no `style` height renders an invisible chart.
:::

## Configuration

`useChart`'s second argument (`UseChartConfig`) — and the corresponding `<EChart />` props — accept these options. They are read once when the chart is created (except the option-update flags, which apply to every reactive update).
...
`@pyreon/charts/echarts` ships **zero ECharts bytes** in your initial bundle. When a chart first renders, the bridge inspects the option object and dynamically imports only the modules it needs:
...
First render of a given chart type has a brief async phase while modules import. `<EChart />` handles it internally (the div is empty until the chart is ready). With `useChart`, gate your UI on `chart.loading()`.
:::

## Manual registration (`@pyreon/charts/echarts/manual`)

For deterministic bundle sizes — or when building a library that shouldn't pull ECharts modules in automatically — use the `@pyreon/charts/echarts/manual` entry. It exposes the same `useChart` / `EChart` / types, plus a `use()` function. You import and register the ECharts modules yourself; the bundler tree-shakes everything you don't register.
...
// Then use EChart / useChart exactly as with @pyreon/charts/echarts
function MyChart() {
  const chart = useChart(() => ({
...
The manual entry's registration function is `use`, called with the modules as **arguments** (`use(BarChart, GridComponent, …)`), not an array. It is re-exported from `@pyreon/charts/echarts/manual` (under the hood it's ECharts' own `echarts/core` `use`). If you call `use(...)` before `echarts/core` has loaded, the registration is queued and applied once core resolves.
:::

:::tip[When to reach for manual]
Use `@pyreon/charts/echarts` (auto-detection) for app code — it gives you zero-config lazy loading. Reach for `@pyreon/charts/echarts/manual` when you need a guaranteed, deterministic bundle (e.g. shipping a component library, or pinning exactly which ECharts modules ship).
:::

## Auto-resize and cleanup

`useChart` (and therefore `<EChart />`) observes the container with a `ResizeObserver` and calls `chart.resize()` whenever the container's size changes — no manual wiring needed. You can also trigger a resize yourself via the `resize()` return value (useful after a layout change a `ResizeObserver` won't catch, like a programmatic style swap).
...
| `EChart`                        | component | `<TOption extends EChartsOption = EChartsOption>(props: EChartProps<TOption>) => VNodeChild`. Declarative chart component built on `useChart` with event binding and inline error display. |
...
| `EChartProps`                    | type      | Props for `<EChart />` (`options`, `style`, `class`, `theme`, `renderer`, `locale`, `notMerge`, `replaceMerge`, `lazyUpdate`, `onInit`, `showLoading`, `loadingOption`, `ariaLabel`, `onEvents`, `onClick`, `onMouseover`, `onMouseout`). |
| `ChartEventParams`              | type      | Duck-typed shape passed to event handlers (`name`, `value`, `seriesIndex`, `dataIndex`, …).   |
| `ChartEventHandler`             | type      | `(params: ChartEventParams, instance: ECharts) => void` — the `onEvents` handler shape.       |
...
### Exports — `@pyreon/charts/echarts/manual`

| Export                                | Kind      | Description                                                                       |
| ------------------------------------- | --------- | --------------------------------------------------------------------------------- |
| `useChart`                            | hook      | Same hook as `@pyreon/charts/echarts`, but with **no** auto-detection — you register modules via `use()`. |
| `EChart`                              | component | Same component as `@pyreon/charts/echarts`, with no auto-detection.                      |
| `use`                                 | function  | `(...modules: unknown[]) => void`. Variadic ECharts module registration (`echarts/core`'s `use`). Call once at startup. |
| `EChartsOption` / `UseChartConfig` / `UseChartResult` / `EChartProps` | type | Re-exported for convenience. |

### Exports — `@pyreon/charts/echarts/vite`
