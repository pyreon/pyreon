...
<Example file="./examples/i18n/i18n-switch-locale-translate" title="i18n — switch locale, translate" />
...
`I18nProvider` accepts `&#123; value: I18nInstance, children &#125;` props. `useI18n()` throws if called outside an `I18nProvider`. The `I18nContext` is also exported for advanced use cases.

## Trans Component

The `Trans` component enables rich JSX interpolation within translation strings. Use it when your translations contain inline markup like bold text, links, or other components. `t` is read from the nearest `<I18nProvider>` automatically — you don't pass it.
...
Pass an explicit `t` prop only to override the context instance, or to render `<Trans>` without an `<I18nProvider>` in scope (otherwise it throws). Use `i18nKey` — **not** `key`, which JSX reserves for reconciliation.
...
| `t`          | `(key, values?) => string` (optional) | Override the context `t`; defaults to `useI18n().t`            |
...
When `count` is `0`, an explicit `_zero` form is tried **before** the CLDR category — so English (whose CLDR category for 0 is `other`) can still have a dedicated empty-state message:

```ts
const i18n = createI18n({
  locale: 'en',
  messages: {
    en: { cart_zero: 'Your cart is empty', cart_one: '1 item', cart_other: '{{count}} items' },
  },
})

i18n.t('cart', { count: 0 }) // "Your cart is empty"
i18n.t('cart', { count: 1 }) // "1 item"
i18n.t('cart', { count: 5 }) // "5 items"
```

If no `_zero` key exists, resolution falls through to the CLDR category as usual (`count: 0` → `_other` in English).

## Context (gender / variant)

Pass `context` to select a variant key — the same mechanism i18next uses for gender. The lookup tries the context form first, then the base key:
...
i18n.t('friend', { context: 'male' }) // "His friend"
i18n.t('friend', { context: 'female' }) // "Her friend"
i18n.t('friend', { context: 'other' }) // "A friend" (no friend_other → base)
i18n.t('friend') // "A friend"
```

`context` combines with pluralization, most-specific first — `key_<context>_<plural>` → `key_<context>` → `key_<plural>` → `key`:

```ts
// item_male_one, item_male_other, item_one, item_other
i18n.t('item', { context: 'male', count: 1 }) // "item_male_one"
i18n.t('item', { context: 'male', count: 3 }) // "item_male_other"
```

## Default values

Pass `defaultValue` to return a fallback string (interpolated) when the key is missing — checked before the key-as-fallback:

```ts
i18n.t('missing.key', { defaultValue: 'Coming soon' }) // "Coming soon"
i18n.t('welcome', { defaultValue: 'Hi {{name}}', name: 'Bo' }) // "Hi Bo"
```

An existing translation always takes priority over `defaultValue`.

## Key nesting (`$t`)

Reference another key inside a translation with `$t(key)`. The nested call inherits the parent's interpolation values; inline JSON options merge on top:
...
i18n.t('welcome') // "Welcome to Pyreon"
i18n.t('summary') // "2 items"
```

Nesting is depth-capped (4 levels) so cyclic references (`a: "$t(b)"`, `b: "$t(a)"`) resolve to a literal `$t(...)` instead of hanging.
...
The i18n instance ships locale-reactive `Intl` formatters — `n()` (numbers/currency/percent), `d()` (dates/times) and `rt()` (relative time). They read `i18n.locale()`, so they re-run automatically on locale change, and they memoize the underlying `Intl.*Format` instance per `(locale, options)` — repeated calls in a rendered list are cheap.

```ts
i18n.n(1234.5) // "1,234.5" (en) / "1.234,5" (de)
...
// `d()` accepts a Date, an epoch-ms number, or a parseable string.
...
Because they read the locale signal, formatters are reactive inside render functions and effects — no wrapper needed:

```tsx
const FormattedPrice = (props: { amount: number }) => {
  const { n } = useI18n()
  return () => <span>{n(props.amount, { style: 'currency', currency: 'USD' })}</span>
}
```

### Named formats

Define reusable formats per locale once, then reference them by name (cleaner than repeating option objects):
...
const i18n = createI18n({
  ...
  fallbackLocale: 'en',
  numberFormats: {
    en: { currency: { style: 'currency', currency: 'USD' }, compact: { notation: 'compact' } },
  },
  dateFormats: {
    en: { short: { dateStyle: 'short' }, full: { dateStyle: 'full', timeStyle: 'short' } },
  },
  relativeTimeFormats: {
    en: { auto: { numeric: 'auto' } },
  },
})

i18n.n(9.99, 'currency') // "$9.99"
i18n.d(post.publishedAt, 'short') // "1/15/24"
i18n.rt(-1, 'day', 'auto') // "yesterday"
```

A named format missing in the active locale falls back to the `fallbackLocale`'s table.

### Inline format specifiers

Reference a format directly inside a translation string with `{{value, spec}}` — the value is formatted via the same formatters:
...
const i18n = createI18n({
  ...
  messages: {
    en: {
      ...
      points: 'You have {{n, number}} points',
      posted: 'Posted on {{when, date}}',
      ago: 'Updated {{n, relativetime, day}}',
    },
  },
})

i18n.t('total', { amount: 9.99 }) // "Total: $9.99"
i18n.t('points', { n: 1234.5 }) // "You have 1,234.5 points"
i18n.t('posted', { when: new Date() }) // "Posted on Jan 15, 2024"
i18n.t('ago', { n: -2 }) // "Updated 2 days ago"
```

Spec names: `number`, `date` / `time` / `datetime`, `relativetime` (the unit is the next comma arg — `{{n, relativetime, day}}`), any configured named format (`{{amount, currency}}`), or a custom named formatter:

```ts
const i18n = createI18n({
  locale: 'en',
  formats: { upper: (v) => String(v).toUpperCase() },
  messages: { en: { shout: '{{word, upper}}!' } },
})
i18n.t('shout', { word: 'hello' }) // "HELLO!"
```
...
| `numberFormats`    | `NumberFormats`                               | `undefined` | Per-locale named `Intl.NumberFormat` options        |
| `dateFormats`      | `DateFormats`                                 | `undefined` | Per-locale named `Intl.DateTimeFormat` options      |
| `relativeTimeFormats` | `RelativeTimeFormats`                      | `undefined` | Per-locale named `Intl.RelativeTimeFormat` options  |
| `formats`          | `NamedFormatters`                             | `undefined` | Custom named formatters (`(value, locale) => string`) usable inline |
...
| `n(value, options?)`                 | `(value, options?: Intl.NumberFormatOptions \| string) => string` | Format a number/currency/percent (reactive, memoized) |
| `d(value, options?)`                 | `(value, options?: Intl.DateTimeFormatOptions \| string) => string` | Format a date (Date/epoch/string; reactive, memoized) |
| `rt(value, unit, options?)`          | `(value, unit, options?: Intl.RelativeTimeFormatOptions \| string) => string` | Format a relative time (reactive, memoized) |
...
```tsx
function I18nProvider(props: { value: I18nInstance; children: VNode }): VNode
```
...
Replace `&#123;&#123;key&#125;&#125;` placeholders in a string with values from the given record. An optional `format` resolver handles inline format specs (`&#123;&#123;val, spec&#125;&#125;`); when omitted (bare `interpolate()` use), specs are ignored and the value is coerced plainly. The i18n instance supplies a locale-bound `format` resolver automatically inside `t()`.

```ts
function interpolate(
  template: string,
  values?: InterpolationValues,
  options?: { format?: (value: unknown, spec: string) => string },
): string
```
