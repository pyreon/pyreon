...
| `@pyreon/flow` | the core `createFlow` state, `<Flow instance=&#123;flow&#125;>` host, and nested `<Background>` lower natively, while `<Controls>`/`<MiniMap>`/`<Handle>` chrome and custom renderer maps still depend on SVG/DOM behavior and require explicit native follow-ups or the WebView bridge |
