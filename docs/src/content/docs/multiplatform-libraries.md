...
| `@pyreon/flow` | the core state, host, literal custom node maps with static interactive handles, layouts, and common overlays lower natively, while advanced node chrome and custom edge renderers still require explicit native follow-ups or the WebView bridge |
