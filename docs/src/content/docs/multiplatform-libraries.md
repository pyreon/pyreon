...
| `@pyreon/flow` | the core state, host, literal custom node maps with static interactive handles and node resizers, layouts, and common overlays lower natively, while toolbars and custom edge renderers still require explicit native follow-ups or the WebView bridge |
