...
| `@pyreon/flow` | the core `createFlow` state and `<Flow instance=&#123;flow&#125;>` host lower natively, while the optional `<Background>`/`<Controls>`/`<MiniMap>`/`<Handle>` chrome and custom renderer maps still depend on SVG/DOM behavior and require explicit native follow-ups or the WebView bridge |
