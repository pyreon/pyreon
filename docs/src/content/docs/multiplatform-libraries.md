...
**Constraints**:
- Stick to the supported TypeScript surface. See `docs/src/content/docs/pmtc-supported-typescript.md` for the exact subset (it's most of TS but excludes things like dynamic `import()` and full Proxy semantics).
...
1. Read [`docs/src/content/docs/multiplatform.md`](./multiplatform.md) for the 4-layer model + canonical primitives.
2. Read [`docs/src/content/docs/pmtc-supported-typescript.md`](./pmtc-supported-typescript.md) for what TS syntax PMTC handles.
3. Read [`docs/src/content/docs/pmtc-per-target-setup.md`](./pmtc-per-target-setup.md) for the per-target build flow.
...
## The declared tier of every package

Every package manifest declares its multiplatform story as data
(`multiplatform: { tier, rationale }` — see `@pyreon/manifest`), gated by
`check-multiplatform-tier` in `validate-fast`, so a new package cannot
...
| `@pyreon/charts` | the DEFAULT export wraps ECharts (a browser canvas engine) and stays web — keep it in a `<Web>` branch or embed it through the `<WebView>` bridge subpath. `@pyreon/charts/plot` is the multiplatform engine: every family and every host except `<OptionChart>` / `<MapChart>` lowers to a native PyreonChartCanvas over the GENERATED engine (a bare host follows the device colour scheme, as it follows `prefers-color-scheme` in a browser). Marks lower too, `sma` / `ema` / `trend` included (numeric-literal window); `bollinger` is the one that does not — it returns an ARRAY of marks to spread rather than a single mark call, so it warns by name instead of lowering to something it is not (see nativeFrontend and the Charts row of the capability matrix) |
