...
<Example file="./examples/a11y/save-button-live-region" title="announce() + <LiveRegion> + <VisuallyHidden> — live" />

The demo above combines all three primitives on one real "Save" button — click it (or navigate here with a screen reader on) to hear both the visible status line and the separate `announce()` confirmation.
