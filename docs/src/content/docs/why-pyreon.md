...
- **And the expanded coverage found a loss.** On mounting a deep component tree (2,047 instances) Pyreon is **1.88× slower than Solid** — the first op in the suite where it is beaten outright. It is published here rather than left out, because a benchmark that only contains your best shapes is marketing.
