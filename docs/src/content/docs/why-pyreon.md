...
- **Octane is right behind — and mostly tied, not trailing.** [Octane](https://octanejs.dev) (compiled React, Inferno's successor) joined the suite in 2026-08. Once its own handicap is removed, it plausibly ties Pyreon on most ops (pending fresh CI on the table-layout-fixed board) and **wins `clear rows` outright** — a real, architectural gap: Octane registers no per-row subscriptions at all, where Pyreon's fine-grained bindings pay a real teardown cost on every clear. A separate fix (still open) narrows that gap from ~1.47× to ~1.15× without closing it. The lead we can honestly claim is narrower than previously published: two outright wins (bulk-create, append), a tie-cluster, one loss.
