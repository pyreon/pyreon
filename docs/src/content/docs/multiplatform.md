...
**Weighted totals (2026-07-08 baseline; M2.3 + M3.1 + M3.2 + M3.2b + M3.3 + M2.2 + a11y-label + M2.2b + M2.4-i18n applied):** device-proven (R4+) coverage
**≈ 47%** (50.35 / 107 — the Platform-APIs row's +1.0 each for haptics/share/link/notifs, +1.0 for `useSizeClass()` (M2.2), +0.45 for the `accessibilityLabel` device assertion, +1.0 for the adaptive Stack↔Inline layout (M2.2b), plus **+0.9 for the `createI18n` translation device assertion (M2.4)**: the i18n row moves 0.0 → 0.3 as single-arg `t(key)` + configured-locale selection are now behaviorally device-proven (iOS local Simulator + Android CI-gated), see that row); compile-proven (R2+) upper bound **≈ 74%** —
