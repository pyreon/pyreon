...
| Styling & design system | 6 | 0.0 | R1/R2 ONLY, and this row exists because its ABSENCE was silently inflating every percentage on this page. `styled()`, `@pyreon/elements` `Element`, `@pyreon/coolgrid`, `@pyreon/attrs`, rocketstyle static + reactive dimensions, `defineTheme` tokens and text typography all LOWER (see the styling table above — 14 supported features, 4 architectural non-goals). But **no native example instantiates a single one of them**, and no device test asserts a rendered style, so the whole pipeline — theme resolution → style emit → rendered appearance — has never run on a device. Compile-proven is not the same claim as device-proven, and for a design system it is the weaker one: a theme token that resolves to the wrong colour, or a dimension flip that emits a modifier the platform ignores, compiles perfectly. Weight 6 = less than Core UI (10, the primitives themselves), equal to Forms and Animations |
