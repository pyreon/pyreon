...
| Release & distribution (archive/signing/store) | 5 | 0.0 | **This row was MISSING entirely** — the matrix measured capabilities inside the app and omitted SHIPPING it, which inflated every headline on this page the same way the once-missing Styling row did; this table is supposed to BE the denominator. What exists: the Android scaffold ships R8/ProGuard config verified once against a real `./gradlew assembleRelease` (a minified release build of a useFetch app compiles clean — kotlinx-serialization's keep rules ride the artifact), and the asset pipeline always emits `AppIcon.appiconset` / `ic_launcher`, so a release build has an icon. What does NOT exist: no `xcodebuild archive` / `exportArchive` / `exportOptionsPlist` path anywhere in the repo (every iOS invocation is Simulator-SDK `build`/`test`), no signing story beyond debug/ad-hoc, no release-keystore vocabulary, no versioning (versionCode / marketing-version) surface, no store-submission path — and NO release build of either platform has ever been installed and RUN on a device, so the R4+ fraction is honestly 0.0, not a rounded-up token for the one `assembleRelease` compile. Store submission additionally needs Apple Developer + Play Console accounts (user-side credentials); everything before that step is credential-free engineering |
| Crash reporting & observability | 3 | 0.0 | **ABSENT — no vocabulary at all.** No crash-handler seam, no `useCrashReporter`, no breadcrumb/log capture, and no symbolication story: the Android release config produces R8 `mapping.txt` and Xcode produces dSYMs, but nothing retains or uploads either, so even a hand-wired crash reporter would show unreadable stacks. A production app without crash visibility cannot be OPERATED — you learn about crashes from store reviews. Weight 3 = the Offline and Background rows' tier: not the app's surface, but non-optional for shipping |
