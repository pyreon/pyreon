...
| Animations & transitions | 6 | 0.2 | **`<Transition show>` device-proven (M2.7)** — the counter's Toggle Box flips a signal and the animated child ("Animated Box") shows/hides through the platform animation path (iOS `.transition(.opacity)` on an `if show` gate + `.animation(.default, value:)` on a stable ZStack; Android `AnimatedVisibility(visible = show)`); the device gate asserts it disappears then reappears (`waitForNonExistence` / `assertDoesNotExist`), bisect-verified. BEHAVIORAL on the show/hide — the fade TIMING itself is not asserted (an opacity curve isn't queryable). v1 vocabulary is the conditional-visibility fade only; configurable duration/easing, enter≠leave, spring/keyframe, gesture-driven, shared-element + layout/reorder animations are all still absent |
...
**Weighted totals (2026-07-08 baseline; M2.3 + M3.1 + M3.2 + M3.2b + M3.3 + M2.2 + a11y-label + M2.2b + M2.4-i18n + M2.5-colorScheme + M2.6-machine + M2.7-animations applied):** device-proven (R4+) coverage
**≈ 48%** (51.55 / 107 — the Platform-APIs row's +1.0 each for haptics/share/link/notifs, +1.0 for `useSizeClass()` (M2.2), +0.45 for the `accessibilityLabel` device assertion, +1.0 for the adaptive Stack↔Inline layout (M2.2b), +0.9 for the `createI18n` translation device assertion (M2.4), plus **+1.2 for the `<Transition show>` animation device assertion (M2.7)**: the Animations & transitions row moves 0.0 → 0.2 as the conditional-visibility show/hide fade is now behaviorally device-proven (iOS local Simulator + Android CI-gated), see that row. `useColorScheme` (M2.5) + `createMachine` (M2.6) also landed device-proofs but deepened already-recorded rows — dark mode flips the per-hook audit `❌`→device-proven, and the state-machine transition strengthens the maxed State row — so neither moved the fraction); compile-proven (R2+) upper bound **≈ 74%** —
