...
> - **(1) Per-PR gate — SYNTAX + STUB TYPE-CHECK.** `swiftc -parse` (syntax, accepts unresolved types) runs on every emit; `kotlinc` type-resolves against Compose **stubs**; and `swiftc -typecheck` type-resolves against SwiftUI+PyreonRuntime **stubs** (`swift-stubs.ts`, the Swift sibling of `kotlin-stubs.ts`) — the stubs' generic constraints mirror the real SDK EXACTLY (`animation<V: Equatable>`, so a `[Todo]` value is correctly REJECTED) so this catches the type-corruption class that pure `-parse` waves through. **Scope caveat:** the stub `-typecheck` covers the 2 example apps + 30 of 37 compiler fixtures (canonical primitives + common modifiers + i18n/machine/permissions/link/webview + the rx fixtures); the gate has already earned its keep by SURFACING real emitter bugs that were then FIXED so the emitted native code actually compiles (rx `.first`/`.last`/`.min`/`.average` now infer Optional/Double bindings; a null-returning component now emits `EmptyView()` not `body { nil }`). Emit that references the service-runtime + `@Observable` surface (`PyreonRouter`/`RouterProvider`/`useNavigate`/`PyreonStoreProtocol`/`PyreonForm`/`PyreonAuth`/`PyreonFetch` + the `@Observable` macro) still reaches only `-parse` per-PR until the stub is expanded, and a fixture-declared `interface` isn't yet synthesized into a struct — see the silent-failure cliff below.
