...
**Weighted totals (2026-07-08 baseline; M2.3 + M3.1 + M3.2 + M3.2b + M3.3 + M2.2 + a11y-label + M2.2b applied):** device-proven (R4+) coverage
**≈ 46%** (49.45 / 107 — the Platform-APIs row's +1.0 each for haptics/share/link/notifs, +1.0 for `useSizeClass()` (M2.2), +0.45 for the `accessibilityLabel` device assertion, plus **+1.0 for the adaptive Stack↔Inline layout (M2.2b)**: the Adaptive/tablet row moves 0.2 → 0.4 as the size-class-driven view-branch ternary now lowers to a valid `if/else` (swiftc-proven FAIL→PASS) and is device-compiled/rendered, see that row); compile-proven (R2+) upper bound **≈ 74%** —
