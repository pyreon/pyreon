...
> - **(1) Per-PR gate — SYNTAX + STUB TYPE-CHECK.** `swiftc -parse` (syntax, accepts unresolved types) runs on every emit; `kotlinc` type-resolves against Compose **stubs**; and `swiftc -typecheck` type-resolves against SwiftUI+PyreonRuntime **stubs** (`swift-stubs.ts`, the Swift sibling of `kotlin-stubs.ts`) — the stubs' generic constraints mirror the real SDK EXACTLY (`animation<V: Equatable>`, so a `[Todo]` value is correctly REJECTED) so this catches the type-corruption class that pure `-parse` waves through. **Scope caveat:** the stub `-typecheck` covers the 2 example apps + 27 of 37 compiler fixtures (canonical primitives + common modifiers + i18n/machine/permissions/link/webview); emit that references the service-runtime + `@Observable` surface (`PyreonRouter`/`RouterProvider`/`useNavigate`/`PyreonStoreProtocol`/`PyreonForm`/`PyreonAuth`/`PyreonFetch` + the `@Observable` macro) still reaches only `-parse` per-PR until the stub is expanded, and the gate has already SURFACED real emit bugs excluded pending an emitter fix (`.first`/`.last`/`.min`-emit-an-optional-into-a-non-optional-binding, a null-returning component emitting `body { nil }`) — see the silent-failure cliff below.
