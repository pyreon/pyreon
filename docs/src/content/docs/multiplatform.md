...
**❌ Web-only by design** (a hard DOM/canvas/vendor dependency the compiler has no path for — these will NOT be *native-rendered* as SwiftUI/Compose; PMTC can't compile CodeMirror/ProseMirror/etc. But several CAN be **hosted in a `<WebView>`** — see the bridge escape hatch right after this table):
...
**🌉 Escape hatch — host a web-only component in a `<WebView>` (the bridge).** The "❌" packages can't be *native-rendered*, but a `<WebView>` embeds a real browser engine (WKWebView on iOS, Android WebView), so the web component runs *inside* it — flow's SVG host, CodeMirror, a document preview. The bridge is **bidirectional**:
...
| Charts (plot engine) | 5 | 0.7 | **`@pyreon/charts` renders natively.** Every engine module (bar/line/area/points and their stacks, pie, funnel, treemap, sunburst, tree, river, polar, sankey, graph, chord, heat, candlestick, boxplot, calendar, gantt, parallel, single-axis, geo, the chrome, the a11y description, decimation, zoom, brush, the visualMap strip) is generated into `PyreonChartEngine.swift/.kt`, byte-locked against a fresh generation and compiled by real `swiftc` and `kotlinc`. Every host lowers to `PyreonChartCanvas` — the data-prop, accessor and frame hosts, `<PlotChart marks>`, the `<Chart>` grammar, and `<MapChart>` from a precomputed `GeoShape[]`. Device-asserted on both platforms in the required `native-device` job: 20 of the 22 hosts render, with behavioural reads for funnel and radar taps, boxplot picks, gauge and dataZoom state, map roam (pixel diffs), toolbox zoom, `saveAsImage` and the universal-transition row toggle. **Why not 1.0:** the web-only-by-mechanism props (`keyboard`, `accessibleTable`, hover events, `crosshair`, `link`, `facet`) warn by name; `<ChordChart>` and `<SingleAxisChart>` have no device test; the heatmap `visualMap` drag and candlestick zoom have no device assertion since the option gallery was removed |
...
## WebView host — embedding web-only-rich viz (flow / editors)

Some libraries are **structurally web-only** — `@pyreon/code` (CodeMirror), `@pyreon/document`
...
Two packages wrap a web engine that has no native equivalent and cannot
be reimplemented as a native view: `@pyreon/code` (CodeMirror 6, a DOM editor) and `@pyreon/rich-text`
...
compiles to web + iOS + Android hosting a flow diagram, a real CodeMirror
editor and a real TipTap WYSIWYG, with **0 warnings on both targets**.

**This is one codebase on every target, and it ships. It is NOT a
natively-rendered view**, and the difference is worth stating plainly
...
package (`src/webview.browser.test.tsx`: a real `view.dispatch` in CodeMirror and a real `execCommand` edit in
