...
| Accessibility | 3 | 0.3 | **`accessibilityLabel` DEVICE-PROVEN (iOS)** — the counter's XCUITest asserts a labelled element is queryable in the REAL accessibility tree by its LABEL ("A11y status ready"), NOT by its visible glyph "●", so `.accessibilityLabel` genuinely overrode the accessible name in the live tree (bisect-verified: strip the prop → the query fails). The rest stays R2/emit-locked: `accessibilityHidden` (XCUITest string queries don't reliably reflect it — tooling limitation, not an emit gap), roles/focus-order/live-announcements. **The ANDROID half is now device-asserted too** — the same `accessibilityLabel` lowers to `Modifier.semantics { contentDescription = … }`, and the Compose test finds the node BY THE LABEL and asserts its text is the glyph "●", which pins the semantics block to the annotated element rather than a wrapper or sibling (an assertion that some node merely carries the description would pass on a misplaced block). Fraction 0.15 → 0.3: label lowering is now proven on BOTH platforms, while `accessibilityHidden`, roles, focus order and live announcements remain unproven on either |
...
re-scores the matrix). `useStorage` (todomvc terminate+relaunch) and
`useAuth` (the finance sign-in flow) are now done; remaining:
`useDatabase` behaviour is now done too (iOS — the counter's
terminate+relaunch record assertion). Remaining: 1. `useDatabase` on
ANDROID, 2. `useWebSocket`, 3. `useClipboard`. The three
manual-`.start()` hooks (geolocation/push/payments) are gated on M3.9
