...
| `@pyreon/attrs` `attrs(Base).attrs({…})` | ✅ | default-prop HOC — rewrites `<X …use-site>` to `<Base …use-site …defaults>` (use-site wins); literal + theme-token default attrs. BOTH call forms lower: the documented `attrs(Base)` and the config-object `attrs({ component: Base })`. Until 2026-07 only the config form was recognised, so the DOCUMENTED one emitted `attrs(Base).attrs(…)` verbatim and failed the native build with "cannot find 'attrs' in scope" |
