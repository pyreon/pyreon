---
title: "Zero — Full-Stack Meta-Framework — API Reference"
description: "Full-stack meta-framework: fs-routing, SSR/SSG/ISR/SPA, API routes, server actions, adapters, i18n"
---

# @pyreon/zero — API Reference
...
| Symbol | Kind | Summary |
| --- | --- | --- |
| [`zero`](#zero) | function | Top-level Vite plugin chain for @pyreon/zero. |
| [`I18nRoutingConfig`](#i18nroutingconfig) | type | Configuration shape for `zero({ i18n })`. |
| [`expandRoutesForLocales`](#expandroutesforlocales) | function | Fans a flat route list into per-locale variants based on `I18nRoutingConfig`. |
...
| [`vercelAdapter`](#verceladapter) | function | Vercel deployment adapter. |
| [`cloudflareAdapter`](#cloudflareadapter) | function | Cloudflare Pages adapter. |
...
| [`registerServiceWorker`](#registerserviceworker) | function | Registers the worker generated by `zero({ pwa })` (`<base>sw.js`, `updateViaCache: 'none'`). |
...
| [`useRequestLocals`](#userequestlocals) | hook | Bridge middleware-attached request locals into the component tree. |
| [`defineAction`](#defineaction) | function | Define a server action (import from `@pyreon/zero/actions`). |
| [`Form`](#form) | component | A `<form method="post">` bound to a server action (`@pyreon/zero/actions`). |
| [`useSubmission`](#usesubmission) | hook | Reactive state of an action's submissions (`@pyreon/zero/actions`): `pending()`, `input()` (the FormData in flight — ren |
| [`Link`](#link) | component | Default navigation link built on an `<a>` tag. |
| [`useLink`](#uselink) | hook | Composable that returns all link behavior — `&#123; ref, handleClick, handleMouseEnter, handleTouchStart, isActive, isExactAc |
| [`createLink`](#createlink) | function | HOC that wraps any component with link behavior. |
| [`prefetchRoute`](#prefetchroute) | function | Imperatively prefetch a route's JS chunk by injecting `<link rel="prefetch">` + `<link rel="modulepreload">` into `docum |
| [`Icon`](#icon) | component | Renders a FULL loaded SVG — it does NOT synthesize its own `<svg>` around hand-authored `<path>` children. |
| [`createIcon`](#createicon) | function | Builds a reusable icon component from a LOADED svg — a raw `<svg>…</svg>` markup string (`?raw`) OR an imported SVG comp |
| [`iconsPlugin`](#iconsplugin) | function | Vite plugin (from `@pyreon/zero/server`): point it at a folder of `*.svg` files and it writes a strictly-typed generated |
| [`createNamedIcon`](#createnamedicon) | function | Runtime half of `iconsPlugin` — builds a strictly-typed `<Icon name="…" />` from a name→source registry. |
| [`Image`](#image) | component | Default optimized image. |
| [`OptimizedImage`](#optimizedimage) | component | One-prop form of `<Image>` for `?optimize` imports. |
| [`useImage`](#useimage) | hook | Composable that returns resolved image attributes + signals — `&#123; containerRef, inView, loaded, src, srcSet, sizes, aspec |
| [`createImage`](#createimage) | function | HOC that wraps any component with image optimization. |
| [`Script`](#script) | component | Default optimized third-party script loader. |
| [`useScript`](#usescript) | hook | Composable returning script load-state signals + sentinel ref — `&#123; sentinelRef, loaded, errored, pending, needsSentinel, |
| [`createScript`](#createscript) | function | HOC that wraps any component with script load behavior. |
| [`createImageRegistry`](#createimageregistry) | function | Collapses N hand-written image imports into one typed accessor. |
| [`NoOptimize`](#nooptimize) | component | Subtree boundary that disables `<Image>` optimization for every descendant — drops them all to bare `<img>` elements (no |
| [`imagePlugin`](#imageplugin) | function | Vite plugin that transforms image imports with `?optimize` / `?component` / `?raw` query params into optimized responsiv |
| [`usePreloadFont`](#usepreloadfont) | function | Runtime hook to emit `<link rel="preload" as="font">` tags for fonts not declared in the global config (per-route hero f |
| [`inferFontMimeType`](#inferfontmimetype) | function | Pure function that maps file extensions to IANA-registry MIME types for use in font preload `<link type=...>` tags. |
| [`PreloadFontOptions`](#preloadfontoptions) | type | Options for `usePreloadFont`. |
| [`fontPlugin`](#fontplugin) | function | Vite plugin that auto-optimizes Google Fonts and local fonts declared in `zero({ font: { google, local } })`. |
| [`fontImportPlugin`](#fontimportplugin) | function | Vite plugin that transforms `import x from './path.woff2?font'` into typed `FontDescriptor` modules with auto-generated  |
| [`FontDescriptor`](#fontdescriptor) | type | Descriptor object returned by `import x from './path.woff2?font'`. |
| [`usePreconnect`](#usepreconnect) | hook | Emit a `<link rel="preconnect" href="..." crossorigin>` into the head. |
| [`useDnsPrefetch`](#usednsprefetch) | hook | Emit a `<link rel="dns-prefetch" href="...">` into the head. |
...
**See also:** `I18nRoutingConfig` · `GetStaticPaths` · `Adapter` · `createISRHandler`

---

### I18nRoutingConfig `type`
...
```tsx
// Prefix-except-default (canonical SEO shape — default unprefixed)
...
### expandRoutesForLocales `function`
...
**See also:** `I18nRoutingConfig` · `zero` · `parseFileRoutes`
...
**See also:** `zero` · `I18nRoutingConfig`
...
```tsx
import { vercelAdapter, cloudflareAdapter, netlifyAdapter, staticAdapter } from '@pyreon/zero/server'
...
**See also:** `zero` · `createISRHandler` · `vercelAdapter`

---

### createISRHandler `function`
...
```tsx
import { createISRHandler, createServer } from '@pyreon/zero/server'
...
**See also:** `createISRHandler` · `createMemoryStore` · `createFsStore`
...
```ts
function createMemoryStore<E = ISRCacheEntry>(opts?: { maxEntries?: number }): ISRStore<E>
```
...
```tsx
createISRHandler(handler, { revalidate: 60, store: createMemoryStore({ maxEntries: 5000 }) })
```

**See also:** `createISRHandler` · `ISRStore` · `createFsStore`

---

### createFsStore `function`

```ts
function createFsStore<E = ISRCacheEntry>(dir: string): ISRStore<E>
```
...
**See also:** `createISRHandler` · `ISRStore` · `createMemoryStore`

---

### vercelAdapter `function`

```ts
function vercelAdapter(): Adapter
```
...
**See also:** `Adapter` · `zero`

---

### cloudflareAdapter `function`

```ts
function cloudflareAdapter(): Adapter
```
...
### netlifyAdapter `function`

```ts
function netlifyAdapter(): Adapter
```
...
```ts
function aiPlugin(config?: AiPluginConfig): Plugin // server-only
```
...
```tsx
plugins: [pyreon(), zero(), seoPlugin({ ... }), aiPlugin()]
```
...
```ts
function registerServiceWorker(options?: { url?: string; scope?: string; onUpdate?: (activate: () => void) => void }): Promise<ServiceWorkerRegistration | null>
```

Registers the worker generated by `zero({ pwa })` (`<base>sw.js`, `updateViaCache: 'none'`). The worker precaches exactly the emitted hashed assets (+ prerendered pages under `mode: 'ssg'`), serves navigations network-first and hashed assets cache-first. New versions WAIT by default; `onUpdate(activate)` fires when one is installed so the app can ask, and `activate()` switches + reloads. Resolves `null` during SSR, outside production builds, and without service-worker support.
...
- Expecting it to register in `vite dev` — it no-ops outside production builds on purpose (a caching worker fights HMR)
...
```ts
function https(options?: HttpsOptions): Plugin // server-only; dev + preview
```

Serves the dev server over TLS. NOT for localhost — `http://localhost` is already a secure context, so the gated browser APIs work there without it. It exists for REAL DEVICES: a phone reaches the dev server at `http://192.168.1.24:3000`, which is NOT a secure context, so the EIGHT secure-context-gated hooks (`useGeolocation`, `useDeviceMotion`, `useAudioRecorder`, `useBluetooth`, `useClipboard`, `useNotifications`, `useShare`, `useWakeLock`) and service workers are all unavailable — silently, because the browser just doesn't define the API. `lan: true` certifies this machine's network address AND binds the server to it. Certificates come from three tiers in order: `{ cert, key }` if given; a local `mkcert` CA if one is installed (warning-free); otherwise a self-signed certificate generated with zero dependencies, which works immediately behind a one-time browser interstitial. Pyreon never installs a certificate authority itself — a local CA key can mint a certificate for ANY domain, so trusting one is the user's decision, not a plugin's.
...
- Reaching for it to fix localhost — `http://localhost` is ALREADY a secure context. If an API is missing there, the browser genuinely does not implement it and TLS will not help
...
**See also:** `zero` · `HttpsOptions` · `lanAddresses`
...
**See also:** `useRequestLocals`

---

### useRequestLocals `hook`

```ts
function useRequestLocals(): Record<string, unknown>
```
...
```ts
function defineAction<T>(handler: (ctx: ActionContext) => T | Promise<T>): Action<T>
```
...
```tsx
<Form action={action} revalidate={false} class="new-post">
...
**See also:** `defineAction` · `useSubmission`
...
```ts
function useSubmission<T>(action: Action<T>): Submission<T>
```
...
**See also:** `useLink` · `createLink` · `prefetchRoute`
...
```ts
function useLink(props: LinkProps): UseLinkReturn
```
...
```tsx
import { useLink } from '@pyreon/zero/link'
...
**See also:** `Link` · `createLink` · `UseLinkReturn`
...
```tsx
import { createLink } from '@pyreon/zero/link'
...
**See also:** `Link` · `useLink` · `LinkRenderProps`
...
```ts
<Icon as={ImportedSvgComponent} | svg={rawSvgMarkupString} {...hostProps} />
```

Renders a FULL loaded SVG — it does NOT synthesize its own `<svg>` around hand-authored `<path>` children. You load an svg (it already contains the `<svg>` root) and Icon makes it container-sizable + theme-aware. Two source props: `as` — an imported SVG *component* (`import X from './x.svg?component'`), rendered DIRECTLY with no host wrapper (recommended; it's a real `<svg>` so container-fill is reliable); `svg` — the raw `<svg>…</svg>` *markup string* (`import x from './x.svg?raw'`), inlined via a single `<span>` host (a markup string needs a parent to mount — this one host is unavoidable for the string form). Defaults (`fill="currentColor"`, `display:block;width:100%;height:100%`) are overridable — consumer props spread through and win. No fixed size → fills its container; `fill="currentColor"` themes via CSS `color`. Intentionally no `useIcon` hook (an icon has no composable behaviour); two layers: `createIcon` (one component per loaded glyph) + `Icon` (one-off). **`svg` renders RAW and is never sanitized** — the sanitized `innerHTML` prop needs a `DOMParser` and so cannot run during SSR, which an icon must. The contract is that `svg` takes markup YOU control (a build-time `?raw` import); a string from an API, CMS or URL is an XSS hole. Prefer `as` where possible — an imported SVG component is compiled markup and carries none of this.

**Example**

```tsx
import { Icon } from '@pyreon/zero'
...
**See also:** `createIcon` · `IconProps` · `Image`
...
```tsx
import { createIcon } from '@pyreon/zero'
...
**See also:** `Icon` · `IconProps` · `createNamedIcon` · `iconsPlugin`
...
```ts
iconsPlugin({ dir | sets, out?, mode?: 'inline' | 'image' }): Plugin
```
...
**See also:** `createNamedIcon` · `Icon` · `IconProps`

---

### createNamedIcon `function`
...
```tsx
// icons.gen.tsx (auto-generated by iconsPlugin):
...
**See also:** `iconsPlugin` · `Icon` · `IconProps`
...
**See also:** `useImage` · `createImage` · `OptimizedImage` · `NoOptimize` · `ImageProps` · `ImageRenderProps`
...
```ts
<OptimizedImage source={img} alt={alt} priority={false} />
```
...
**See also:** `Image` · `ProcessedImage` · `useImage`
...
```ts
function useImage(props: ImageProps): UseImageReturn
```
...
```tsx
import { useImage } from '@pyreon/zero/image'
...
**See also:** `Image` · `createImage` · `UseImageReturn`
...
```tsx
import { createImage } from '@pyreon/zero/image'
...
```tsx
import { Script } from '@pyreon/zero/script'
...
**See also:** `useScript` · `createScript` · `ScriptProps` · `ScriptStrategy`
...
```ts
function useScript(props: ScriptProps): UseScriptReturn
```
...
```tsx
import { useScript } from '@pyreon/zero/script'
...
**See also:** `Script` · `createScript` · `UseScriptReturn`
...
```tsx
import { createScript } from '@pyreon/zero/script'
...
```ts
(config?: ImagePluginConfig): Plugin
```

Vite plugin that transforms image imports with `?optimize` / `?component` / `?raw` query params into optimized responsive images at build time. Generates multiple widths, modern formats (WebP, AVIF), tiny blur placeholders (base64 inline), and outputs optimized images to the build directory. Automatically wired by `zero({ image: {} })` — you typically don't need to add it manually to vite.config. In dev, uses `/@fs/` URLs; in build, emits assets and bakes the descriptor (src, srcset, width, height, placeholder, formats) into the JS module.

**Example**

```tsx
// vite.config.ts — explicit wiring (optional if using zero plugin)
...
**See also:** `Image` · `ProcessedImage` · `ImagePluginConfig`

---

### usePreloadFont `function`

```ts
function usePreloadFont(href: string, opts?: PreloadFontOptions): void
```
...
**See also:** `inferFontMimeType` · `PreloadFontOptions` · `fontPlugin`

---

### inferFontMimeType `function`

```ts
function inferFontMimeType(href: string): string
```

Pure function that maps file extensions to IANA-registry MIME types for use in font preload `<link type=...>` tags. Handles `.woff2` → `font/woff2`, `.woff` → `font/woff`, `.ttf` → `font/ttf`, `.otf` → `font/otf`, `.eot` → `application/vnd.ms-fontobject`, and defaults unknown extensions to `font/woff2` (wrong type is less harmful than missing type, which the preload-scanner silently ignores).
...
**See also:** `usePreloadFont` · `fontImportPlugin`

---

### PreloadFontOptions `type`

```ts
interface PreloadFontOptions { type?: string crossorigin?: 'anonymous' | 'use-credentials' }
```

Options for `usePreloadFont`. Both fields are optional — `type` is auto-inferred from the file extension, and `crossorigin` defaults to `'anonymous'` (required by the CSS Fonts CORS spec). Override `type` for unknown extensions; use `'use-credentials'` for rare credential-bearing same-origin fonts only.

**Example**

```tsx
usePreloadFont('/fonts/inter.woff2')
...
```tsx
// In vite.config.ts with the zero plugin:
...
**See also:** `usePreloadFont` · `FontDescriptor` · `fontPlugin`
...
**See also:** `fontImportPlugin` · `usePreloadFont`
...
**See also:** `useDnsPrefetch` · `usePreload` · `PreloadOptions`
...
Emit a `<link rel="dns-prefetch" href="...">` into the head. A cheaper but weaker hint than `preconnect` — only resolves the DNS, doesn't open the TCP/TLS connection. Use for origins that are LIKELY but not certain to be hit (analytics endpoints that may not fire, third-party widgets that may not render). Does NOT take `crossorigin` (DNS resolution is scheme-agnostic). Pair with `preconnect` for browser fallback — preconnect-capable browsers ignore the dns-prefetch, while older browsers without preconnect support still get the DNS hint.
...
**See also:** `usePreconnect` · `usePreload` · `PreloadOptions`
...
Emit a `<link rel="preload" as="..." href="..." crossorigin>` for a specific resource that the page will hit in the critical path. Unlike generic preload markup, this hook enforces the `as` parameter (required — the preload scanner ignores `<link rel="preload">` without it). Use for LCP images (when not using `<Image priority>`), CSS/fonts loaded at runtime, JSON responses the critical path needs, and web worker scripts. Deduplicates via `@pyreon/head`'s href-keying — two `usePreload(h)` calls with the same href emit ONE preload tag.
...
**See also:** `usePreconnect` · `useDnsPrefetch` · `PreloadOptions`
...
**See also:** `usePreload` · `usePreconnect` · `useDnsPrefetch`
...
```ts
function useNoOptimize(): boolean
```

Reads the current `<NoOptimize>` boundary state. Returns `true` if the render scope is within a `<NoOptimize>` boundary (optimization disabled), `false` otherwise. Primarily used internally by `<Image>` to decide whether to bypass optimization; not intended for public application code.

**Example**

```tsx
// Inside Image component (internal usage pattern)
...
> **Note:** mode: 'ssg' returns Plugin[] (the SSG plugin auto-attaches a companion `ssgPlugin()`); Vite's plugins array flattens nested arrays so `plugins: [pyreon(), zero()]` works as-is.

> **i18n strategies:** `prefix-except-default` (default) keeps the default locale unprefixed (SEO-canonical for primary-locale apps). `prefix` prefixes every locale including default (best when no locale is primary). Switching strategies changes the dist filesystem layout — plan migration paths if you flip mid-product.

> **getStaticPaths × i18n cardinality:** 3 IDs × 3 locales × 2 strategies of accidents = bigger SSG output than you expected. Use `ssg.concurrency` to parallelize the render; use `ssg.onProgress` to surface heartbeat lines on long builds (CI silent-stretches look hung otherwise).

> **Adapter.build invocation:** Auto-invoked at the end of every mode-specific build companion. SSG mode (`ssgPlugin`) calls `adapter.build({ kind: 'ssg', outDir, config })` after every path renders. SSR/ISR modes (`ssrPlugin`) bundle the SSR handler to `dist/server/entry-server.js` and call `adapter.build({ kind: 'ssr', serverEntry, clientOutDir, outDir, config })`. The SSR plugin prefers a user-authored `src/entry-server.ts` when present; otherwise it synthesizes the canonical `createServer({ routes, routeMiddleware, apiRoutes })` entry. SPA mode ships only a client bundle (no adapter.build call).

> **Locale-aware RouterLink — not yet shipped:** RouterLinks under i18n duplication emit LITERAL hrefs from their `to` prop. Cross-locale navigation falls through to the default-locale route. A locale-aware-link feature is a future PR; for now, write per-locale hrefs explicitly or use the router's programmatic navigation in handlers.
