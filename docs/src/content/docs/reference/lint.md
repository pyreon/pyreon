...
description: "Pyreon-specific linter — 100 rules across 20 categories, config files, watch mode, AST cache, CLI + LSP"
---
...
//   pyreon-lint --list                                     # list all 100 rules
...
| Symbol | Kind | Summary |
| --- | --- | --- |
| [`lint`](#lint) | function | 100 rules across 20 categories. |
...
100 rules across 20 categories. Auto-loads `.pyreonlintrc.json`. Presets: `recommended`, `strict`, `app`, `lib`. Per-rule options via tuple form in config (`["error", { exemptPaths: [...] }]`) or `ruleOptionsOverrides`. Wrong-typed options surface on `result.configDiagnostics`. Uses `oxc-parser` with AST caching.
