...
description: "Pyreon-specific linter — 97 rules across 19 categories, config files, watch mode, AST cache, CLI + LSP"
---
...
| Symbol | Kind | Summary |
| --- | --- | --- |
| [`lint`](#lint) | function | 97 rules across 19 categories. |
...
97 rules across 19 categories. Auto-loads `.pyreonlintrc.json`. Presets: `recommended`, `strict`, `app`, `lib`. Per-rule options via tuple form in config (`["error", { exemptPaths: [...] }]`) or `ruleOptionsOverrides`. Wrong-typed options surface on `result.configDiagnostics`. Uses `oxc-parser` with AST caching.
