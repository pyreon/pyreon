...
description: "Pyreon-specific linter — 92 rules across 18 categories, config files, watch mode, AST cache, CLI + LSP"
---
...
//   pyreon-lint --list                                     # list all 92 rules
...
| Symbol | Kind | Summary |
| --- | --- | --- |
| [`lint`](#lint) | function | 92 rules across 18 categories. |
...
92 rules across 18 categories. Auto-loads `.pyreonlintrc.json`. Presets: `recommended`, `strict`, `app`, `lib`. Per-rule options via tuple form in config (`["error", { exemptPaths: [...] }]`) or `ruleOptionsOverrides`. Wrong-typed options surface on `result.configDiagnostics`. Uses `oxc-parser` with AST caching.
