...
**What crosses natively:** PyreonA11y — `announce(...)`
...
| [`clearAnnouncements`](#clearannouncements) | function | Remove both live regions (`polite` and `assertive`) that `announce()` lazily creates, so the NEXT `announce()` call re-c |
