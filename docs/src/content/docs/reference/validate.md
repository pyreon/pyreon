...
**What crosses natively:** a top-level `const X = s.object({ … })` declaration AND the standalone inline chain `s.object({ … }).safeParse(x).success`, emitted as a Swift Codable struct / Kotlin data class with `parse` + constraint checks (`.safeParse` returns a web-faithful `{ success, data }`); only a literal `s.object({ … })` shape lowers
...
- String format checks: email / url (http(s) by default; `url({ protocol })` accepts any RFC 3986 scheme matching the RegExp, as zod 4 does — the default stays http(s) so `javascript:` / `data:` are not newly admitted into rendered links) / uuid / ip / phone / creditCard + cuid2 / ulid / nanoid / emoji / base64 / jwt + ISO date/dateTime/time — every format routed through the client/server registry seam, so a server can upgrade any of them in place via `installFormatValidator`.
