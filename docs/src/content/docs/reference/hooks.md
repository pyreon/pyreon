...
description: "65 signal-based hooks: state (useToggle/useCounter/usePrevious/useLatest/useControllableState), DOM (useEventListener/useClickOutside/useFocus/useHover/useFocus"
---
...
**See also:** `useAudioRecorder` · `a11y`
...
**See also:** `useImagePicker` · `useFilePicker` · `usePermissions`
...
**See also:** `useAppState` · `useDocumentVisibility`
...
**See also:** `useNativeModule` · `usePermissions`
...
**See also:** `useMap` · `useWebSocket` · `useOnline`
...
```tsx
const ws = useWebSocket('wss://example.com/chat')
onMount(() => ws.connect())
...
**See also:** `useSecureStorage` · `useBiometrics`
...
**See also:** `useNotifications` · `useAuth`
...
**See also:** `useAuth` · `useSecureStorage`
...
**See also:** `useSecureStorage` · `useStorage (in @pyreon/storage)`
...
**See also:** `useOnline` · `useDocumentVisibility`
...
```tsx
setCrashTransport((report) => fetch('/api/crashes', { method: 'POST', body: report }))
```
