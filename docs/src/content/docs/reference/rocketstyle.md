---
title: "Multi-Dimensional Styling — API Reference"
description: "Multi-dimensional component styling — states, sizes, variants, custom dimensions, dark/light mode, all cached"
---

# @pyreon/rocketstyle — API Reference

> **Generated** from `rocketstyle`'s `src/manifest.ts` — the same source that powers `llms.txt` and MCP `get_api`. Do not edit this page by hand; edit the manifest. For the conceptual guide, see [rocketstyle](/docs/rocketstyle).
...
**See also:** `Provider` · `isRocketComponent` · `@pyreon/attrs` · `@pyreon/styler`
...
**See also:** `rocketstyle` · `.theme()` · `.states() / .sizes() / .variants()`
...
```tsx
// Parent provides its pseudo-state; child derives its own state from it
...
**See also:** `.theme()` · `.config()` · `@pyreon/attrs`
...
**See also:** `.states() / .sizes() / .variants()` · `.styles()` · `resolveModeVar`
...
**See also:** `.multiple() / .modifiers()` · `.theme()` · `rocketstyle`
...
**See also:** `.states() / .sizes() / .variants()` · `rocketstyle`
...
**See also:** `resolveTheme` · `.theme()` · `@pyreon/styler`
...
```tsx
const withTooltip = (Component) => (props) => Component(props)
...
**See also:** `.compose()` · `isRocketComponent`
...
**See also:** `rocketstyle` · `@pyreon/attrs`
...
```tsx
import { resolveTheme } from '@pyreon/rocketstyle'
...
**See also:** `.theme()` · `@pyreon/unistyle`
