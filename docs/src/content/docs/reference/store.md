...
description: "Global state management — Pinia-inspired composition stores returning StoreApi<T>"
---

# @pyreon/store — API Reference

> **Generated** from `store`'s `src/manifest.ts` — the same source that powers `llms.txt` and MCP `get_api`. Do not edit this page by hand; edit the manifest. For the conceptual guide, see [store](/docs/store).
...
**See also:** `StoreApi` · `addStorePlugin` · `resetStore` · `defineStore (schema mode)`
...
**See also:** `SchemaStoreApi` · `SchemaStoreConfig` · `SchemaStoreContext` · `StoreApi`
...
```tsx
const u = useUser()  // SchemaStoreApi<{ name: Signal<string>; prefs: Signal<{theme: string}> }>
...
**See also:** `defineStore (schema mode)` · `DeepPartial` · `StoreApi`
...
**See also:** `defineStore (schema mode)` · `SchemaStoreContext`
...
**See also:** `defineStore (schema mode)` · `SchemaStoreConfig`
...
**See also:** `defineStore` · `addStorePlugin`
...
```tsx
// Register BEFORE any store hook is first called.
addStorePlugin((api) => {
  api.subscribe((mutation) => {
...
```tsx
resetStore('counter') // next useCounter() call builds a fresh store
```
...
**See also:** `resetAllStores` · `defineStore` · `StoreApi`
...
```tsx
afterEach(() => resetAllStores()) // canonical test isolation
```
...
```tsx
// server, after render. Store state is user data, so the JSON must be
...
- Embedding the snapshot with a bare `JSON.stringify` — store state is user data, so a stored string containing `</script>` or `<!--<script` breaks out of the inline script. Escape the `<` class plus U+2028/U+2029 before interpolating, as the example does
...
**See also:** `StoreActions` · `SignalsOf` · `defineStore`
...
**See also:** `StoreState` · `defineStore`
...
**See also:** `defineStore` · `StoreState`
