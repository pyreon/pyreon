...
```tsx
// Top of any package's own src/index.ts:
...
```tsx
// A primitive that defers mounting (simplified island shape):
...
**See also:** `runWithContextOwner` · `setContextOwner` · `getCurrentScope`
...
**See also:** `getContextOwner` · `setContextOwner`
...
```tsx
const prev = setContextOwner(myOwner)
try {
  doWork()
} finally {
...
**See also:** `runWithContextOwner` · `getContextOwner`
