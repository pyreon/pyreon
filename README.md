...
| [`@pyreon/charts`](packages/fundamentals/charts/) | Two engines: a lazy-loaded ECharts bridge, and Pyreon's own tree-shakeable plot engine with canvas + server-side SVG backends |
...
| [`@pyreon/lint`](packages/tools/lint/) | 132 Pyreon-specific lint rules across 25 categories — reactivity, JSX, SSR, performance |
...
The monorepo uses Bun workspaces with 76 packages across 6 categories (`packages/core/`, `packages/fundamentals/`, `packages/ui-system/`, `packages/tools/`, `packages/zero/`, `packages/native/`). Each package resolves `src/` directly via the `"bun"` export condition — no build step needed during development.
