...
- **72 packages.** Forms, routing, state management, charts, drag & drop, i18n, multiplatform (SwiftUI + Compose), and more.
...
| [`@pyreon/hooks`](packages/fundamentals/hooks/) | 51 hooks — useHover, useFocus, useBreakpoint, useClipboard, useHaptics, useShare, useLinking, useNotifications, useBiometrics, useImagePicker, useFilePicker, useDialog, useTimeAgo, etc. |
...
The monorepo uses Bun workspaces with 72 packages across 6 categories (`packages/core/`, `packages/fundamentals/`, `packages/ui-system/`, `packages/tools/`, `packages/zero/`, `packages/native/`). Each package resolves `src/` directly via the `"bun"` export condition — no build step needed during development.
