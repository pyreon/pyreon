...
- **74 packages.** Forms, routing, state management, charts, drag & drop, i18n, multiplatform (SwiftUI + Compose), and more.
...
The monorepo uses Bun workspaces with 74 packages across 6 categories (`packages/core/`, `packages/fundamentals/`, `packages/ui-system/`, `packages/tools/`, `packages/zero/`, `packages/native/`). Each package resolves `src/` directly via the `"bun"` export condition — no build step needed during development.
