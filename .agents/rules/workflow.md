...
3. **Test end-to-end against the real shape.** Per `.agents/rules/test-environment-parity.md`: mock-vnode tests must have a parallel real-`h()` test; browser packages need real-Chromium smokes; framework-primitive changes need e2e against examples. The test environment must match the shape that ships to users.
...
- Check AGENTS.md for documented patterns and conventions
...
| **A release published 70 of 76 packages and every gate said OK for a month** | The 0.51.0 publish hit npm `E422 Error verifying sigstore provenance bundle` on four `@pyreon/native-*` packages (transient, npm-side), `native-cli` was correctly blocked behind them, and NOTHING retried or resumed: `publish.ts` had no retry, release.yml only publishes when a Version PR merges, `heal-release-chain` judges "npm has the version" by the `@pyreon/core` anchor, and the daily `published-state` gate compared FOUR sentinels — so it printed `OK 4/4` every day while every fresh multiplatform scaffold installed a 0.50.0 native compiler against a 0.51.0 core. Found 2026-09-08 by sweeping all 76 packages by hand | Three structural fixes: `check-published-state` now compares EVERY publishable package's npm latest to its repo version (`classifyLag`) and reds a PARTIAL RELEASE by name; `publish.ts` retries only npm errors with evidence of being transient (5xx/socket) via `scripts/publish-retry.ts`; release.yml gained `resume-detect` → `resume-publish`, which republishes from the release TAG ONCE per version. **The root cause was NOT transient, and the first live resume is what proved it**: npm's full message is `Failed to validate repository information: package.json: "repository.url" is ""` — those six manifests had no `repository` field at v0.51.0 (#2727 fixed them on main hours later), so provenance rejects that tag forever and the fix rides the NEXT release. Two lessons: **retry only what you have evidence is transient — an infrastructural-looking code is not evidence**; and **a replay-the-tag remediation cannot fix a release broken BY its tag, so bound it to one attempt and escalate** (otherwise it reds every main push)|
| **A `run:` step dies with `exit code 1` and NOTHING printed, though the same script passed locally** | GitHub runs `run:` under `bash -e`. A command substitution in an assignment (`x=$(probe); code=$?`) aborts the whole step the moment the probe exits non-zero — and a probe that exits 1 to MEAN something (`check-published-state --lag-json`: 1 = lag found) never gets to report it. The local dry-run used plain `bash`, so it passed; the first live run of release.yml's `resume-detect` (34201482228) died on that line | Wrap the probe in `set +e` … `set -e`, or `x=$(probe) \|\| code=$?`. **Dry-run a step's script with `bash -e`, never bare `bash`** — the shell flag is part of the environment under test |
| **`Install` fails at `bun install --frozen-lockfile` with `Unknown lockfile version`, and every job that installs goes red with it** | `bun.lock` was regenerated by a bun NEWER than the `.bun-version` pin. 1.3.14 reads and writes `lockfileVersion: 1`; bun 1.4 writes 3, and the pinned bun cannot parse it. The trap is that bun only REWRITES the lockfile when a resolution actually CHANGES — so a mismatched local bun goes unnoticed for weeks and then the first dependency edit silently upgrades the format. It surfaces as several unrelated red checks (observed on #3433: `Install`, `typecheck (all)`, `bootstrap-exit-codes`, iOS and Android all died at their setup/install step) with nothing downstream having run, so it reads as a broken branch rather than one bad file. | `bun run check-lockfile-version` (in `validate-fast`/pre-push) names the observed version, the pinned bun and the exact command. Fix: `bunx bun@<.bun-version> install` and commit the result. The gate asks a READABILITY question, not an equality one — `BUN_LOCKFILE_SUPPORT` maps each pinned bun to the `lockfileVersion`s it can READ (measured: 1.3.14 reads 1 and rejects 3; 1.4.0 reads both and writes 3), so a version-3 lockfile is correct under a 1.4.0 pin and a gate asserting one exact pair would red on state CI installs from happily. A bun version the map does not know fails by name, so a bump has to visit the file where "can CI's bun read this lockfile?" gets asked |
...
| **Check Doc Claims** | Adding/removing a docs page, or changing a count AGENTS.md / README quotes (hook count, lint rule count, doc page count, etc.) | Run `bun run check-doc-claims` after touching `docs/` or any LOCKED count source |
| **Check Doc Claims — red on a PR that touched no docs** | The count on `main` is already wrong, and every branch cut from it inherits the failure. Two doc PRs merging in parallel each bump the SAME claim by one from the SAME base (209 → 210, twice) while the tree gains two pages; non-strict protection means neither re-ran against the other. Observed 2026-09: #3188 and #3195, leaving main claiming 210 against 211 actual. **The Docs Freshness Guard cannot heal this one** — it re-runs `gen-docs` + `gen-all`, and a doc-claim count is hand-written prose neither generator writes, so it stays red until a human edits the number | Check main FIRST rather than hunting your own diff: `git show origin/main:AGENTS.md \| grep 'doc pages'` against the gate's own count (`countDocPages` in `packages/tools/cli/src/doctor/gates/doc-claims.ts` walks ALL of `docs/**` for `.md`, so a `docs/src/content/docs` grep undercounts). If main is red, fix the number in your branch — identical one-line edits auto-merge, so every in-flight PR can carry it and whichever lands first heals main |
...
| **Check Bundle Budgets — green locally, RED in CI on the same commit** | The budget's headroom is smaller than the gzip delta between macOS (where you measure) and ubuntu (where the gate runs). Measured 2026-08 on `@pyreon/cli`: local **16227 B**, CI **16394 B** in two independent runs — a stable **167 B** apart, against **29 B** of headroom. So `validate-fast` read 38/38 while CI was red, and re-running locally only reconfirms the wrong answer. A budget with less headroom than that delta is not tight, it is *un-satisfiable*: the same commit gets a different verdict per machine. Note tight is not itself bad — a small budget makes the gate MORE sensitive to real growth, which is the point; the failure mode is specifically headroom below the measurement's own noise | Read the CI log for the measured KB — the local number is not the one being gated, and **the gate's own "Suggested budget" used to be computed from the LOCAL number plus the bare minimum, so following it on macOS produced a second failure a few dozen bytes higher** (three times in one afternoon: `@pyreon/native-compiler`, `@pyreon/toast`, `@pyreon/table`; the suggestion is now derived from a worst-case remote measurement instead, and `budget-suggestion.test.ts` locks it against those three observations). Bump that ONE entry **by hand** to sit above the CI figure with margin (peers sit at 1.3–3%). **Do NOT reach for `--update`**: it rewrites *every* entry from the current machine, so running it on macOS ratchets ~70 unrelated budgets down to macOS-measured sizes, each then one platform delta from failing in CI — it turns one red gate into a field of them. Check the whole table's headroom first (`--json`, compared against `scripts/bundle-budgets.json`); in the 2026-08 instance exactly one package sat below the variance, so a sweep was never warranted. **And the gate's own "raise to N B to retire it" SUGGESTION is platform-derived too** — the required band is ~1.5% OF THE MEASUREMENT, so a suggestion computed on macOS is short for an ubuntu-gated budget by roughly the platform delta (2026-09: local suggested 374067, ubuntu then needed 374332 — one more CI round to learn 265 B). Take the number from the CI log, and give it real margin rather than the new minimum: a budget at exactly the noise floor is one drifting byte from red |
| **`check-cache-key-sync` — path-list mismatch** | A cache step under a `bootstrap-*`/`bun-*` prefix declares a different `path:` block than the other sites under that prefix (through `key:` OR `restore-keys:`). An entry is versioned by key AND path list, so the mismatched site can never hit — the documented prefix fallback in `setup-pyreon` was inert this way for months | Make every site under the prefix declare the identical `path:` block (the gate names the variants and their sites) |
| **Fast Gates — `validate-fast --ci-complement`** | A gate that lives ONLY in `scripts/validate-fast.ts` (no workflow step of its own) failed. It used to be enforced by the pre-push hook alone; CI now runs the hook-only remainder as one step, computed at runtime from `scripts/gate-wiring.ts` | Run `bun scripts/validate-fast.ts --ci-complement` locally — it is exactly what CI ran; `check-gates-wired` reds if the step is removed |
| **`Build` — `check-bin-liveness` "declares bin … with NO liveness policy"** | A published package added or renamed a `bin` target. The work list is DERIVED from every manifest's `bin` map (keyed on the TARGET path — the old package-keyed table never spawned a second bin, and `create-zero` ships two) | Add the target path to `BIN_POLICY` in `scripts/check-bin-liveness.ts` with its `--help`/`--version`/MCP policy; a stale policy row fails in the other direction |
...
| **`native chart engine — generated, drift-locked` fails after a `packages/fundamentals/charts/src/engine/*` edit** | Those modules are `ENGINE_FILES`: the Swift/Kotlin engine is GENERATED from them and COMMITTED, so a source edit without a regeneration leaves the two disagreeing — and the native half keeps the OLD behaviour. A `tickColor` added to `river.ts` themed the web and left native drawing the hardcoded slate, from one source file (#3444). **Do not diagnose it by reading the diff's `Expected`/`Received` labels** — two sessions inverted that same assertion three times in one day, each producing a confident wrong causal story ("main drifted", "a change landed after this branch"), and each pointing at a remedy that could not work. The direction is unreadable and the question is not directional: run the generator and ask whether the diff is EMPTY. `grep -c <thing> <committed file>` answers it without CI at all — just be clear WHICH ref you are grepping, since "is it on the PR" and "did the work happen" are different questions with different answers | `bun packages/native/compiler/scripts/gen-chart-engine.ts`, then commit the regenerated `PyreonChartEngine.swift` / `.kt` / `chart-engine-structs.ts` alongside the source edit. A change to an `ENGINE_FILES` module is a TWO-PART change |
...
| **Scaffold Smoke (…)** — the batch containing `cpa-smoke-monorepo-vercel` (cells are packed 6→3 by `ci-batch.ts`; read the batch's `members`) | Workspace version ahead of npm (release in flight) | Auto-skipped by `shouldSkipIsolatedCell`; if it still fails, the npm-version check failed or your branch is named `changeset-release/*` |
...
| **`Release` fails at CHECKOUT with `could not read Username for 'https://github.com'`, and the `chore: version packages` PR goes stale/conflicting** | `secrets.RELEASE_PAT \|\| secrets.GITHUB_TOKEN` falls back when the PAT is ABSENT (an unset secret is `''`, falsy) but NOT when it is EXPIRED or REVOKED — that value is non-empty, so `\|\|` takes it, checkout hands it to git, and the first `git fetch` dies naming neither the secret nor the reason. The job then dies at step 2, before `bun install`, so even `heal-release-chain` (`if: always()`) cannot run — there is no checkout to run it from. The Version PR stops being regenerated and drifts into conflict, which reads as a changesets problem rather than a credentials one. Observed 2026-08: EVERY Release run in the visible history had failed this way, with no successful run at all | The PAT is now PROBED against the API before it is trusted (`Select a usable checkout token`); an expired one degrades to `GITHUB_TOKEN` with a `::warning::` instead of taking the pipeline down. Rotating the secret is still the real fix — the PAT exists so pushed tags TRIGGER downstream workflows, which `GITHUB_TOKEN` deliberately does not do; `heal-release-chain` dispatches `release-native.yml` explicitly to cover that gap. **General rule: a secret documented as OPTIONAL must not be consumed in a position where an INVALID value is fatal — `\|\|` tests presence, not validity** |
| **A PR shows ONE check (a default-setup CodeQL "Code Quality" run) and nothing else — no CI, no gates, no "Expected" placeholders — through open, reopen, AND new pushes** | The PR is CONFLICTING. `pull_request` workflows run against `refs/pull/N/merge`; when GitHub cannot build that merge commit it dispatches NOTHING — the Actions check suite sits `queued` with zero runs, and only workflows keyed to `/head` (the default-setup CodeQL) appear. There is no red X and no error anywhere, which sends you chasing Actions outages, billing, skip-tokens, and event-payload traps (all were checked, in that order, before the real cause). Hit on #3110 the day four sibling PRs merged: the hot generated files (AGENTS.md package table, llms-full, api-reference) conflict any branch cut before them | FIRST diagnostic for a checks-never-started PR: `gh pr view N --json mergeable,mergeStateStatus` — `CONFLICTING DIRTY` is the answer. Fix: merge origin/main, union-merge the prose hunks, resolve GENERATED-file hunks by REGENERATING (never by hand), re-run check-doc-claims (main may have moved counts), push. Dispatches materialize within a minute of the PR turning MERGEABLE |
...
| **`Native Compiler Validation` / `iOS — xcodebuild` / `Android — gradlew` run on a PR that touches nothing native (a compiler-only or a dependency-bump PR)** | Both native `decide` jobs classified the diff with an inline regex whose `packages/<cat>/<pkg>/native/` arm ALSO matched the JSX compiler's napi crate (`packages/core/compiler/native/`), and whose `package.json` arm matched every manifest in the repo. Each false match cost ~50 min of real swiftc on the 5-slot macOS pool, ~50 min of iOS simulator and ~30 min of Android emulator — 6 of the last 40 PRs (2026-09-07). Invisible because the lanes are GREEN when they run; only the queue depth shows the cost | ONE classifier, `scripts/native-surface-touched.ts` (fail-closed, unit-tested), feeds both decides; a `package.json` counts only when it declares `pyreon.native`. **General rule: a path regex that says "this directory NAME means X" must be checked against every sibling that uses the same name for something else — `ls -d packages/*/*/native` before trusting it** |
| **`Validate emitted Swift (real-SDK typecheck, macOS)` takes 36–58 min on EVERY native PR** | The lane had no verdict cache at all while its Linux twin restored one — the "a fix applied to ONE lane is folklore" class, for a cache. The content-addressed store (`validate-cache.ts`) keys on the compiler's own version string, so it is safe across Xcode releases by construction | Restored/saved under `native-verdicts-ci-macOS-typecheck-`; the nightly `schedule` run stays deliberately uncached |
...
| **`Coverage (Full)` — red, and not caused by your PR** | It runs on `push:main` + `merge_group` ONLY (PRs see the fast `Coverage` floor check), so it can rot unnoticed. In 2026-07 it had been failing on **every** completed main run, across 15 packages: a safety-net gate that is red-on-arrival cannot distinguish a regression from the baseline — it is a dead gate | Fixing the gate IS the work; never re-run past it. Triage PER PACKAGE in three honest categories: **(a)** accounting artifact — a file at ~0% in the node run because only a real-Chromium `*.browser.test.tsx` covers it → add to `coverageExclude` with a rationale comment (never exclude a file with no coverage anywhere); **(b)** cheap real gap → write the genuine test; **(c)** honest re-baseline → lower the threshold to the MEASURED actual with a `check-coverage.ts` exemption entry (the drift check requires table ⇄ config to match exactly), and ratchet back up like `lint-baseline.json`. Never lower thresholds in bulk. **It rotted AGAIN by 2026-08 — same shape, seven packages** (six drifted under the multiplatform wave: charts/code/rich-text via their new `/webview` subpaths, hooks via the native hooks, feature, loom; plus `@pyreon/config` reporting a bogus `0%`). Recurring in under a month says the once-per-main cadence is the root cause, not the individual drift: nothing measures coverage at PR time, so every PR is free to drop it and only main goes red — where a red gate blocks nobody. **Before triaging, re-read the gate's OUTPUT critically**: its progress lines were interleaved by 4-way concurrency (a newline-less `write` then a later `log`), so one package's percentage printed beside another package's name — `@pyreon/atlas`'s 79.72% appeared next to `@pyreon/zero`, which reads as a real finding about the wrong package. The TABLE is authoritative, the progress lines were not (fixed to one atomic line per package). **It rotted a THIRD time by 2026-08-12 (hooks + lint), and the PR-time step added to stop that was itself the reason.** The step measured the AFFECTED set — the transitive dependent closure — and skipped when that exceeded 15 packages. But coverage is a property of a package's OWN sources + OWN tests: editing `@pyreon/core` cannot move an example's number. So the closure was both useless here AND harmful: a `@pyreon/vite-plugin` / `@pyreon/zero` change expands to ~49 workspaces, trips the cap, and the step skips — meaning **the packages whose coverage matters most were exactly the ones never measured before merge**. Fixed by `affected.ts --changed-only` (`directOnly`), which emits only directly-changed workspaces: the same PR measures 3 instead of 49, so the cap stops firing for the ordinary case. **The general rule: when a gate takes a cheap subset of a full run, the subset must be chosen by what the gate MEASURES, not by the blast radius the correctness jobs need — and any cap that silently skips is a hole, so verify what the step actually did (its <1s runtime was the tell) rather than trusting its green.** |

| **`Benchmark` red on main (CI job REMOVED 2026-09-23 — no consumer read its artifact; still applies to a local `bench:all` / `bench:bundle-size`): `[bundle-size] Failed to bundle @pyreon/reactivity … No matching export in "package.json" for import "name"`, four core packages UNMEASURED** | Every package entry reads `import { name, version } from '../package.json' with { type: 'json' }`. Per the import-attributes spec a JSON module has ONLY a default export, and esbuild enforces that once the attribute is present — so the bench's esbuild bundle of `src/` failed on every entry, while rolldown's `lib/` build (which inlines the two fields) was fine. Same run: `[bench] Failed to run scripts/bench/core/router.ts: spawnSync /bin/sh ETIMEDOUT` — the router bench's full protocol is documented at 6–9 minutes and run-all gave every script 120s, so the router row had been silently absent from every main artefact | `scripts/bench/bundle-size.ts` loads `package.json` through a plugin that emits `name`/`version` as named exports (what consumers' bundlers ship); `scripts/bench/run-all.ts` takes per-script `args`/`timeoutMs`, runs router `--quick` under a 420s budget, and exits non-zero naming any bench that did not run (the artefact still carries every row that did). **General rule: a bench that cannot run is an UNMEASURED row and must fail the run, never an omitted row in a green artefact** |
| **`Scaffold Smoke` red: `error: GET https://registry.npmjs.org/@types/node/-/node-26.6.0.tgz - 404` then `bun install exited with code 1`** | A scaffolded project resolves live ranges against the registry, and a JUST-published version can be listed before its tarball has replicated — the same URL answered 200 an hour later (2026-09-15). Nothing in the template was wrong; the cell reported a registry-side transport failure as a scaffold failure | `runBunInstall` classifies the output (`isTransientRegistryFailure`: tarball 404/5xx, `ECONNRESET`, `ETIMEDOUT`, `socket hang up`) and retries once after 20s; a resolution error (`No version matching`) or a build error is never retried. Pure classifier, unit-tested |
| **`Coverage (Full)` `cancelled` at exactly 15 minutes on every main push, `java` orphans in the cleanup, no coverage table** | #3441 brought `packages/native` into the coverage scan; the PR-time step drops `@pyreon/native-compiler` (its own comment measured 1381s) and says "measured on main instead" — but the main job it deferred to had the SAME 15-minute cap, restored NO verdict cache, and ran the suite's hundreds of cold kotlinc spawns inside a 4-way pool with file parallelism ON (the serial flag is set only by the native TEST cells). One package pinned the job at its cap on every push since, and an unrelated package (`@pyreon/lathe`) reported a load-induced failure beside it. Reads as "the Kotlin subprocess tests time out under load" — the tests were fine; the JOB had no cache and no isolation | `Coverage (Full)` now runs `--skip=@pyreon/native-compiler` (the script FAILS if that name stops matching, so the skip cannot rot into a re-inclusion), and `Coverage (Native)` measures that package alone with the shard verdict cache restored, files serial, and a 45-minute cap. **General rule: when a step defers a package to "another job", that job must actually be able to run it — same cap, same missing cache, same pool means the deferral only moved the failure** |
...
**CI wall-clock here is QUEUE-dominated, so job COUNT is the lever — not job speed.** Measured on run 31023199747 (a normal PR): 1,732 min of summed queue wait against 87 min of summed work, a **19.8x tax**, with the median cell waiting 30.6 min for a slot it used for under a minute. A matrix cell is not free — before running anything it pays a queue wait plus checkout, `setup-pyreon` and (for e2e) a Playwright install: measured 35-52s, against suites that RUN for 8-30s. `e2e (cssvars)` spent 35s of setup on 8s of testing. Splitting a job into N only buys wall-clock while runners are FREE; under contention the cells serialise through the queue anyway, so the split costs N setups and N queue entries for the same work — and because the runner pool is shared, a wide fan-out slows every OTHER branch too. Hence `scripts/ci-batch.ts`: pack cells into a bounded number of batches (LPT, so the slow tail gets its own runner), which took the fan-out from 44 cells to 12. When adding a matrix cell, ask whether it needs its own RUNNER or just its own step — the default answer is a step. Cells that must stay 1:1 (the `native-*` test cells key a verdict cache on their own category) go in the `--isolate` list. **Re-measured 2026-09-01: the org is on the FREE plan (20 concurrent jobs org-wide), and an ordinary code PR spent 641 min queued for 24 min of work (27×) — so the 2026-09 pass consolidated 29 jobs + 6 standalone workflows into 13 jobs (`Install` = install+decide+lib/, `Fast Gates` = every lib-free gate, `Build` = every lib-needing gate, ONE `Test` aggregator) and dropped 17 names from both protection authorities. The gate names in the table above (`Check Bundle Budgets`, `Check Import Budgets`, `Check Distribution`, `Coverage`, `Verify Modes`, `Lint`, `Release Readiness`, `Doc Examples`, …) are now STEP names inside `Build` / `Fast Gates`; the fix commands are unchanged. Diagnostic recipe for the next pass: `gh api repos/pyreon/pyreon/actions/runs/<id>/jobs`, EXCLUDE `conclusion == "skipped"`, then per job `started_at − created_at` is queue and `completed_at − started_at` is work; a wall clock many times the summed work with a queue sum in the hundreds of minutes is slot starvation, and the only levers are job count and DAG depth.**
...
`.agents/rules/testing.md` "Test the shipped ENTRY, not the export").
...
- **It overrides a MID-SESSION instruction too, and that is not hypothetical.**
  ...
  and AGENTS.md's — were on the losing side of that by default; the policy held
  ...
  If the policy genuinely changes, change it HERE, in AGENTS.md, and in the
...
1. **`bun run validate-fast`** — runs lint + 9 cheap CI gates that have
   ...
   - `check-doc-claims` — AGENTS.md / README numeric claims match source
...
never run them (`.agents/rules/testing.md`: ~397s uncached vs 6s warm). The
...
9. If you added/removed a hook in `@pyreon/hooks` or a `.md` file in `docs/`: `bun run check-doc-claims` — asserts the hook count and doc-page count claims in README/manifest/AGENTS.md/docs index stay in sync with the actual source. Catches the drift where one claim site gets bumped and others don't.
...
15. If API surface changed: update AGENTS.md, docs/, README, llms.txt, llms-full.txt, MCP api-reference (via the manifest, not the generated file)
...
4. **All documentation surfaces updated** (every PR, not just API changes):
   ...
   - `.agents/rules/anti-patterns.md` if a new anti-pattern was discovered
   - `.agents/rules/` — any other rule file relevant to the change
...
- **API surface changed?** Update `AGENTS.md`, `docs/`, `README`, `llms.txt`, `llms-full.txt`, MCP `api-reference.ts` as part of the same PR.
