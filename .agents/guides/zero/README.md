...
- **Server functions (page form posts)** (`form-actions-server.ts`, `form.tsx`). A POST to a PAGE URL is an action submission when it names one (`?_action=<id>`, what `<Form>` renders — a query-only URL, so base + locale prefix come from the document for free and SSR/client render the identical string) or when the route exports `action` (the generated route-middleware module adds a marker entry, AFTER the page's own middleware, that points `ctx.locals['zero:routeAction']` at it; it never runs it). `createFormActionMiddleware` runs last among endpoints, reuses `checkActionOrigin` + `readActionPayload` (the SAME origin check and `bodyLimit`, 1 MiB default, enforced on Content-Length AND while streaming) with `/_zero/actions/*`, and warms lazy route modules once on a registry miss. `redirect()` → 303 (base-prefixed; a 307 would re-POST); otherwise the page re-renders as a synthetic GET through the BASE handler (never the ISR cache), the snapshot handed over via a `WeakMap<Request>` → `ctx.locals` → a registered reader (`_setActionSnapshotReader`, because `@pyreon/server`'s barrel is not client-safe), `Cache-Control: no-store`. `X-Zero-Action: 1` (the enhanced `<Form>`) gets JSON instead; a redirect travels as data since `fetch` would follow a 3xx. Client state is one cell per action id (bounded by call sites) with a version counter against stale resolution; server state is per request. `useSubmission` success → `router.revalidate()`, which only re-rendered the leaf once `RouterView`'s leaf `equals` learned to re-emit on REPLACED loader data (first arrival stays with `pendingMinMs`). Not in `vite dev` yet — the dev SSR middleware does not run the endpoint chain at all.
