...
- **Server functions (page form posts)** (`form-actions-server.ts`, `form.tsx`). A POST to a PAGE URL is an action submission when it names one (`?<raw page query>&_action=<id>`, what `<Form>` renders from the router's `_currentPath` — a query-only URL, so base + locale prefix come from the document for free and SSR/client render the identical string) or when the route exports `action` (the generated route-middleware module adds a marker entry, AFTER the page's own middleware, that points `ctx.locals['zero:routeAction']` at it; it never runs it). `createFormActionMiddleware` runs last among endpoints, reuses `checkActionOrigin` + `readActionPayload` (the SAME origin check and `bodyLimit`, 1 MiB default, enforced on Content-Length AND while streaming) with `/_zero/actions/*`, and warms lazy route modules once on a registry miss. `redirect()` → 303 (base-prefixed; a 307 would re-POST); otherwise the page re-renders as a synthetic GET through a dedicated handler (never the ISR cache) whose ONLY middleware restores the POST's locals + response headers — app/route middleware run once per request — the snapshot handed over via a `WeakMap<Request>` → `ctx.locals` → a registered reader (`_setActionSnapshotReader`, because `@pyreon/server`'s barrel is not client-safe), `Cache-Control: no-store`. `X-Zero-Action: 1` (the enhanced `<Form>`) gets JSON instead; a redirect travels as data since `fetch` would follow a 3xx. Client state is one cell per action id (bounded by call sites) with a version counter against stale resolution; server state is per request. `useSubmission` success → `router.revalidate()`, which only re-rendered the leaf once `RouterView`'s leaf `equals` learned to re-emit on REPLACED loader data (first arrival stays with `pendingMinMs`). Build-time action manifest (`action-manifest.ts`): the plugin scans `src/` with `collectActionIds` (the SAME `locateActions` derivation the transform bakes) and appends `_registerActionModules({ id: () => import(file) })` to the generated `virtual:zero/route-middleware` — the module every server entry and the dev pipeline already import, so no entry change — and `_resolveAction` loads the defining module on a registry miss (both `/_zero/actions/*` and page posts; warming all route modules stays as the fallback). Before it, a fresh server 404'd every action whose module no render had loaded. In `vite dev` the watcher invalidates the route-middleware module whenever a `src/` source file joins, leaves or changes inside the manifest (`add`/`change`/`unlink`; a file outside it is read only for the `mayDefineActions` marker), so an action in a new non-route module is reachable mid-session. Lives in `createRequestPipeline` (`pipeline.ts`), so `vite dev` runs the same code: the caller supplies `renderActionPage` (production: a createHandler whose only middleware is `createActionRerenderMiddleware`; dev: `renderDevActionPage` → `renderSsr`), both restoring state via `readCarriedActionState`. Without a renderer the form middleware degrades to a 303 back to the page. Dev's `renderSsr` now routes `pathname + search` (it dropped the query — dev SSR rendered every page as if its query were empty).
...
- Resource hints: `usePreconnect`, `useDnsPrefetch`, `usePreload`. Icons: `<Icon>`/`createIcon`, `iconsPlugin`/`createNamedIcon`. Favicons: `faviconPlugin` (sharp required; theme-switched assets need the `data-favicon-theme` attribute). `ogImagePlugin`. Per-route OG from JSX: a route file's `export const og` (SVG JSX from `{ path, params, data }`, typed `OgImage` from `@pyreon/zero/server`) → SSG: hashed PNG in `assets/og/` + injected `og:image`; SSR/ISR: `/_zero/og/<path>.png` endpoint (CDN `s-maxage`/SWR) + injected absolute `og:image`. sharp required; SVG root only; `routeOg: { width, height, siteUrl }`. PWA: `zero({ pwa: { manifest, skipWaiting? } })` → `manifest.webmanifest` + linked, and a `sw.js` written post-output/pre-adapter precaching exactly the emitted hashed assets (+ SSG pages); navigations network-first, `<base><assetsDir>/` cache-first; new worker WAITS unless `skipWaiting`. Client: `registerServiceWorker({ onUpdate })` from `@pyreon/zero` (no-op in dev/SSR). node/bun adapters serve `sw.js`/`*.webmanifest` must-revalidate.
