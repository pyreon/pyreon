...
- `assertRouteFileShapes` runs in the routes virtual-module load: no default export, default-only `_layout`, literal `loader` → a `[Pyreon] Invalid route file(s)` error naming the file. The routes module rethrows `[Pyreon]` errors instead of degrading to `routes = []`.
