...
| `@pyreon/charts` | Pyreon's own charting engine: `<Chart>` with mark children, twenty family components, `/svg` (SSR), `/engine` (everything, unstable); theme tokens via `ChartThemeProvider`, `chartThemes`, `palettes`; draws natively on iOS/Android. Bench: `bench:engine`. |
