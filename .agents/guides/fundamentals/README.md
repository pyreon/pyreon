...
| `@pyreon/machine` | `references/machine.md` | constrained-signal state machines |
...
| `@pyreon/i18n` | `createI18n({ locale, messages, loader?, fallbackLocale?, … })`; `t(key, values?)` with `{{name}}` interpolation, format specifiers (`{{amount, currency}}`), plurals (`_one`/`_other`/`_zero`), `context`, `defaultValue`, `$t(key)` nesting (depth 4). Formatters `n`/`d`/`rt`. Entries `@pyreon/i18n` (with `Trans`, `I18nProvider value={i18n}`, `useI18n`) and `@pyreon/i18n/core`. Opt-in typed keys via `createI18n<typeof en>()` (`MessageKeys<M>`, `TranslationParams`). |
