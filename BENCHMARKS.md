...
- `kinetic` vs Motion: the timing window closed before kinetic's enter state was applied, "enter" revealed one element instead of N, the CSS baseline never animated, and the page ran on the 100 µs clamped clock. **The Kinetic-vs-Motion claim is withdrawn** (see §13).
...
- **`@pyreon/virtual` breaks on @tanstack/virtual-core 3.17.11** (its declared `^3.17.7` range already allows it): the `item(i).start()` accessor yields no position in a real browser (`virtual.browser.test.tsx` "repositions a STAYING row…"; passes on 3.17.4). Root cause: `<For>` builds its first rows detached, so a row's `ref` measured 0 px, and 3.17.11 compensates scroll for zero-size rows. Fixed in #3594 (merged); this run used 3.17.4.
- **SSR `NODE_ENV` reads** — fixed for Vite production builds in #3586; unbundled Node remains.
- **`@pyreon/kinetic` vs Motion** — the published claim stays withdrawn; the re-run numbers are PROVISIONAL. The full run first stalled for 17+ minutes at 2,000 elements. That was the bench's own end-state gate, not kinetic: it called `el.getAnimations()` once per element, and Chromium walks every animation in the document on each call, so N elements × N CSS transitions was O(N²) — the hand-written CSS baseline blew up identically (6.8 s at 250 → 48.8 s at 500). With a single `document.getAnimations()` pass the run completes in 45 s. Medians (25 samples, isolated clock): enter 500 — kinetic 2.20 ms, Motion 5.67, CSS baseline 5.63; enter 2,000 — 8.79 / 18.01 / 25.84; stagger 300 — 3.11 / 11.07 / 4.02; stagger 1,000 — 14.81 / 51.16 / 23.74 (kinetic 2.0–3.6× faster than Motion in every cell). Not published as a verdict because kinetic measures BELOW the hand-written CSS baseline in every cell, which should be impossible for the same work and suggests the timed window does not capture CSS-transition work equally across arms; load also rose from 5.3 to 9.9 during the run.
