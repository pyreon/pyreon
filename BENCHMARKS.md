...
- **`@pyreon/virtual` breaks on @tanstack/virtual-core 3.17.11** (its declared `^3.17.7` range already allows it): the `item(i).start()` accessor yields no position in a real browser (`virtual.browser.test.tsx` "repositions a STAYING row…"; passes on 3.17.4). The monorepo override stays at 3.17.4 in this run; the adapter fix is tracked separately.
