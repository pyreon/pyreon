...
> **This is a dated snapshot (2026-07-01), and its verdicts have since been superseded — most recently and most substantially on 2026-08-18.** The row-list section predates [Octane](https://octanejs.dev) joining as the 8th framework, so its "6 of 9 outright" tally is against a 7-framework field. It is now doubly stale: an 8-framework run (2026-08-14) found Pyreon trailing only on `clear rows`, and that run has ITSELF been partly retracted — our own benchmark harness had handicapped Octane (a `String(row.id)` wrapper that disabled a compiler fast path) and separately let a V8 engine cache inflate the retained-heap numbers for five implementations. The corrected field (pending merge as open PRs #2893/#2894/#2895/#2896/#2897/#2899) is mostly a **statistical tie** with Octane — two outright Pyreon wins (`create 10,000`, and `append` after a re-adjudication that resolved an initial bimodal-timing false start — see the "Corrected tally" note in the flagship section of `docs/benchmarks.md`), six ties, and one loss (`clear rows`). A 2026-08-18 profiling pass additionally found `create`/`replace` are browser-layout-bound (this instrument cannot resolve a framework difference there; the small real signal is a JS-only ~+28% Pyreon cost invisible in wall clock) and that the suite's `create N rows` op measures a REPLACE for 19 of its 20 sampled runs (no reset, monotonic row ids). Retained memory is a 3-way tie with Preact for 3rd of 8, not "2nd among frameworks." The `@pyreon/head` figures (4.9–6.4×) came from a harness later found to flatter Pyreon; the fair rebuild measures 1.3–2.1×. And "Preact's signals-core leads every row" predates the 2026-07 batch-queue work. Numbers not called out here were not re-measured for this note — the current per-op record lives in `.claude/skills/pyreon-benchmarks/SKILL.md`, which is the authoritative copy.
