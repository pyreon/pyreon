...
> **This is a dated snapshot (2026-07-01), and its verdicts have since been superseded — most recently and most substantially on 2026-08-18, TWICE in one day.** The row-list section predates [Octane](https://octanejs.dev) joining as the 8th framework, so its "6 of 9 outright" tally is against a 7-framework field. It is now stale in three compounding ways: an 8-framework run (2026-08-14) found Pyreon trailing only on `clear rows`; that run was partly retracted for handicapping Octane (a `String(row.id)` wrapper that disabled a compiler fast path) and for a V8 engine cache that inflated retained-heap numbers for five implementations; and the RE-CORRECTED field was itself found to have used `table-layout: auto` in the bench fixture, which forced Chromium to re-measure the whole table's column widths on any op that widened a cell — this caused `append`'s bimodal timing AND inflated the published Pyreon-vs-Solid `partial update` lead from a real ~2.1× to a false ~4.9×, a fifth artifact this campaign found in its own harness (retracted as explicitly as the rest). The FINAL corrected field (pending merge as open PRs #2893/#2894/#2895/#2897/#2899/#2901/#2903 — #2896 excluded, unrelated) is mostly a **statistical tie** with Octane — two outright Pyreon wins (`create 10,000`, `append` — both WIDENED by the table-layout fix, not narrowed), a plausible tie-cluster, and one loss (`clear rows`, ~1.45× on `main` today, unaffected by the layout fix). A separate, sixth correction (2026-08, PR #2912, open) found the EXPLANATION this campaign had previously given for that loss — "the price of fine-grained per-row subscriptions, disposal chain already at its floor" — was wrong by more than an order of magnitude: the real cost was a duplicate per-key lookup table, not the per-row signal teardown; a fix narrows the loss to ~1.15× without closing it. A profiling pass additionally found `create`/`replace` are browser-layout-bound (this instrument cannot resolve a framework difference there; the small real signal is a JS-only ~+28% Pyreon cost invisible in wall clock) and that the suite's `create N rows` op measures a REPLACE for 19 of its 20 sampled runs (no reset, monotonic row ids). Retained memory is a 3-way tie with Preact for 3rd of 8, not "2nd among frameworks." The `@pyreon/head` figures (4.9–6.4×) came from a harness later found to flatter Pyreon; the fair rebuild measures 1.3–2.1×. And "Preact's signals-core leads every row" predates the 2026-07 batch-queue work. Numbers not called out here were not re-measured for this note — the current per-op record lives in `.claude/skills/pyreon-benchmarks/SKILL.md`, which is the authoritative copy.
