...
// Tasks showcase. Mirror of `native-router-demo-android/app/build.gradle.kts`
// (#1455 — same source-set wiring for @pyreon/native-router-kotlin).
...
android {
    ...
    defaultConfig {
        applicationId = "com.pyreon.PyreonTasks"
        minSdk = 26
        targetSdk = 35
        versionCode = 1
        versionName = "0.0.1"
    ...
    // Source-set wiring for @pyreon/native-router-kotlin — same as
    // native-router-demo-android (#1455). The router-kotlin package
    // ships source-only (no Gradle module / no AAR) so consumers
    // either copy the .kt files in or add the package's
    // `src/main/kotlin/` as an additional source root.
    sourceSets {
        getByName("main") {
            kotlin {
