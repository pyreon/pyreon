  ...
  return (
    <div>
      ...
      <Paragraph style="margin-bottom: 12px; font-size: 13px;">
        Resize the box (drag the corner):
      </Paragraph>
      <div
      ...
      <p style="font-size: 13px; color: #6b7280; margin-bottom: 24px;">
        Size:{' '}
        <strong>
          {() => `${Math.round(elemSize().width)} × ${Math.round(elemSize().height)}`}
        </strong>
      </p>
      ...
      <div style="position: relative; max-width: 280px; margin-bottom: 24px;">
        <Button state="primary" onClick={() => open.set(!open())}>
          Open Dropdown
        </Button>
        {() =>
          open() ? (
      ...
      <Paragraph style="margin-bottom: 12px; font-size: 13px;">
        Scroll to see the box become visible:
      </Paragraph>
      <div style="height: 200px; overflow-y: auto; border: 1px solid #d1d5db; border-radius: 8px; padding: 16px;">
        ...
        >
          Status:{' '}
          {() =>
            (intersection() as IntersectionObserverEntry | null)?.isIntersecting
              ? 'visible'
              : 'not visible'
          }
        </div>
