...
function CheckboxIndicator(props: { checked: boolean }) {
  return (
    <span
      style={() =>
        `width: 18px; height: 18px; border: 2px solid ${props.checked ? '#3b82f6' : '#d1d5db'}; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; background: ${props.checked ? '#3b82f6' : 'white'}; color: white; font-size: 12px; transition: all 0.15s; flex-shrink: 0;`
      }
    >
      {() => (props.checked ? '✓' : '')}
    </span>
