...
const cardStyle =
  'padding: 16px; background: #f3f4f6; border-radius: 8px; max-width: 360px; margin-bottom: 16px;'
  ...
  return (
    <div>
      <Title size="h2" style="margin-bottom: 12px">
        .config(), .statics(), isAttrsComponent()
      </Title>
      <Paragraph style="margin-bottom: 24px">
        `.config()` immutably branches a component (rename, swap inner). `.statics()` attaches
        metadata. `isAttrsComponent()` is a runtime type guard.
      </Paragraph>

      <Title size="h3" style="margin-bottom: 12px">
        .config() — immutable branching
      </Title>
      ...
      <MetaBox style={cardStyle}>MetaBox.meta = {JSON.stringify(meta)}</MetaBox>
