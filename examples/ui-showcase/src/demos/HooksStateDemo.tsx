  ...
  return (
    <div>
      ...
      <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 24px;">
        <Button state="primary" onClick={() => visible.toggle()}>
          Toggle
        </Button>
        <Button state="secondary" onClick={() => visible.setTrue()}>
          Set true
        </Button>
        <Button state="secondary" onClick={() => visible.setFalse()}>
          Set false
        </Button>
        <span>
          Value: <strong>{() => String(visible.value())}</strong>
        </span>
      </div>
      ...
      <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 24px;">
        <Button state="primary" onClick={() => count.set(count() + 1)}>
          Increment
        </Button>
        <span>
        ...
        <span>
          Previous: <strong>{() => String(prev() ?? 'undefined')}</strong>
        </span>
      </div>

      <Title size="h3" style="margin-bottom: 12px">
        useDebouncedValue(value, 500ms)
      </Title>
      <input
        type="text"
      ...
      <p style="font-size: 13px; color: #6b7280; margin-bottom: 24px;">
        Live: <strong>{() => search() || '(empty)'}</strong> | Debounced:{' '}
        <strong>{() => debounced() || '(empty)'}</strong>
      </p>

      <Title size="h3" style="margin-bottom: 12px">
        useInterval(fn, () =&gt; running() ? 1000 : null)
      </Title>
      ...
      <div style="display: flex; gap: 8px; align-items: center;">
        ...
        <Button state="secondary" onClick={() => ticks.set(0)}>
          Reset
        </Button>
        <span>
