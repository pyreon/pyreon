#!/usr/bin/env bash
# build.sh — drive the Pyreon → Swift compile loop for the iOS
# router-demo sample. Designed to be invoked either:
...
#
# Mirror of `examples/native-counter-ios/scripts/build.sh` — only
# difference is the example dir name. SRC_DIR is THIS directory's
# `src` (the SHARED canonical source; the web sibling at
# `native-router-demo-web/` imports from the same path).
...
# version (not whatever might be globally installed).
cd "${REPO_ROOT}"
