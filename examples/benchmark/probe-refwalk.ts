...
try {
  ...
  console.log(`\nREF-WALK CEILING @ 10,000 rows (isolated; sink=${out.sink})`)
  console.log('='.repeat(64))
  console.log(`${'shape'.padEnd(24)}${'re-walk'.padStart(12)}${'chained'.padStart(12)}${'saved'.padStart(12)}`)
  for (const [k, v] of Object.entries(out.results)) {
    const f = (ms: number) => (ms < 1 ? `${(ms * 1000).toFixed(0)}µs` : `${ms.toFixed(2)}ms`)
    console.log(`${k.padEnd(24)}${f(v.walk).padStart(12)}${f(v.chain).padStart(12)}${f(v.saved).padStart(12)}`)
  }
} finally {
  await browser.close()
  preview.kill()
