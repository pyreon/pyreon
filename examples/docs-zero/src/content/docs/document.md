...
<Playground title="One document tree → many formats (markdown preview)" height={380} code={`// @pyreon/document renders ONE primitive tree to 14+ formats:
...
const field = (label, sig, opts = {}) =>
  h('label', { class: 'col', style: { gap: '4px', flex: opts.flex ?? 1, minWidth: '0' } },
    ...
    h('input', {
      ...
      style: { width: '100%' },
      value: () => String(sig()),
      onInput: (e) => sig.set(opts.type === 'number' ? Number(e.target.value) : e.target.value),
    }),
  )
