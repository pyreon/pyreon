...
<Playground title="Hotkeys — keypress recorder" height={260} code={`// useHotkey('mod+s', cb) handles scopes, modifier normalization,
...
const log = signal([])
const add = (label) => log.update(l => [label, ...l].slice(0, 6))
