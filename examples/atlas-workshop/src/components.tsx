...
// ── actions addon (event log) ──────────────────────────────────────────────
export const ActionsHead = el.attrs({ tag: 'div' }).theme(() => cx('display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;'))
...
export const ActionRow = el.attrs({ tag: 'div' }).theme((t: T) => cx(`display:flex;align-items:center;gap:10px;padding:9px 11px;border-radius:9px;margin-bottom:6px;animation:atlas-in .18s;background:${t.surface2};`))
...
export const LabWrap = el.attrs({ tag: 'div' }).theme((t: T) => cx(`flex:1;overflow-y:auto;padding:28px 32px;background:${t.bg};`))
export const LabGrid = el.attrs({ tag: 'div' }).theme(() => cx('display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px;max-width:1100px;margin:0 auto;'))
...
export const LabTileHead = el.attrs({ tag: 'div' }).theme((t: T) => cx(`display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:${t.chrome};border-bottom:1px solid ${t.border};`))
