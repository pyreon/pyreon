...
const Wrap = element.attrs({ tag: 'ul', alignX: 'center' }).theme((t) => ({
  display: 'flex',
  flexWrap: 'wrap',
...
const Badge = element.attrs({ tag: 'li' }).theme((t) => ({
...
const BadgeGrid = () => (
  <Section id="badge-grid">
    <SectionHeader title="Skills & Tags">
      A responsive badge row. Hover any badge — the color swap tests the hover theme pipeline (base
      + hover via makeItResponsive, inside @layer rocketstyle).
    </SectionHeader>
    <Wrap>
      {badges.map((label) => (
        <Badge>{label}</Badge>
