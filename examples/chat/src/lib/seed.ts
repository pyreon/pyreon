/**
 * Seed data for the chat audit example.
 *
 * Adapted from `examples/app-showcase/src/sections/chat/data/seed.ts`
 * but consumed by the DEV SSE BACKEND (`vite.config.ts`), not by the
 * client. The client only receives messages via the SSE stream + the
...
export const AUTHORS: Array<{ name: string; color: string }> = [
...
export const LIVE_BODIES: Record<string, string[]> = {
  general: [
...
// Mulberry32 for deterministic history.
...
function buildHistory(channelId: string): Message[] {
  const bodies = LIVE_BODIES[channelId] ?? ['👍']
  const messages: Message[] = []
  let cursor = Date.now() - 60_000 // start 1min ago
  const total = 60
...
export const initialHistory: Record<string, Message[]> = {}
for (const channel of channels) {
  initialHistory[channel.id] = buildHistory(channel.id)
}
