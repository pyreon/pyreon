...
- **Default-pipeline migration** (PR `feat/cpse-element-migration`) — opt-in `init({ styleExtraction: true })`: `PyreonUI` wires styler's new `setStyleExtraction` (injecting unistyle's `cpseRewrite`; styler can't import unistyle — dep direction), so the **non-reactive** `styled` / `Element` resolve (`doResolve`) emits a value-agnostic rule + inline custom properties. A `cpseVarsCache` keyed by `$element` makes the value survive `elClassCache` hits. **Flag-off = byte-identical** (every branch gated; full styler/ui-core/elements/rocketstyle/coolgrid/ui-components suites pass unchanged, node + Chromium). Proven flag-on in real Chromium (plain `styled` + a real `<Element>` fire CPSE; `doResolve` branch bisect-verified). **Honest limits:** O(1) **rules**, NOT O(1) resolve (styler caches by value-bearing `$element`); the **reactive** (rocketstyle) path stays classic; and it currently extracts every flat declaration incl. constants (`position`/`display`), inflating inline bytes — a future refinement can skip non-value-varying declarations. Measure per app before enabling.

**Remaining refinements (genuinely open):**

- _(Responsive values + the default-pipeline migration were previously staged — both SHIPPED; see the shipped list above.)_ Open: skip constant (non-value-varying) declarations in the `doResolve` path to cut inline-style bytes; extend CPSE to the reactive (rocketstyle accessor) path; upstream a real-app win-matrix measurement to gate broad `init({ styleExtraction })` adoption.
