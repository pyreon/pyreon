...
- **Responsive arrays in `cpseStyled`.** The engine supports per-breakpoint var naming (tested). Correct media-query rules must be assembled through `makeItResponsive` (the styler knows the nested-`@media` conventions a bespoke string-assembly would get wrong), so this requires threading `extractVars` through `makeItResponsive`'s DI contract — a focused, low-blast-radius (cpseStyled-only) effort, the natural next increment. Responsive components keep the classic path today.
