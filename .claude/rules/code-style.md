...
- **Error-gated correctness tier (live).** Promoted to `error` (findings driven to 0): `oxc/no-accumulating-spread` (real O(n²) — fixed in rocketstyle by mutate-accumulator), `no-unsafe-finally`, `no-unsafe-optional-chaining` (`(x?.y as T)?.z`), `no-control-regex`, `no-unused-expressions`, `no-unused-vars`, `no-useless-escape`, `import/no-duplicates`, `unicorn/no-thenable`, `unicorn/prefer-string-starts-ends-with`, `jsx-a11y/no-redundant-roles`, `vitest/valid-expect` (set `maxArgs: 2` — vitest's `expect(actual, message)` is valid; do NOT delete the authors' failure-diagnostic messages), `vitest/no-standalone-expect`. Companion scope-`off`s: `no-unused-expressions` off for `**/*.bench.ts` + `scripts/bench/**` (the bare expression IS the measured work), `unicorn/no-thenable` off for `packages/native/compiler/src/**` (the IR uses `then` as the if/ternary branch field — not a promise), `no-useless-escape` off for `docs/.vitepress/**` (Vue SFCs legitimately embed `<\/script>` HTML-safety escapes in sandboxed inline code).
- **Rules turned `off` after triage proved them false-positive for this codebase** (verify-first, NOT laziness): `oxc/no-map-spread` — flags `arr.map(x => ({ ...x, foo: bar }))`, but that IS the canonical immutable-update idiom (required for signal reactivity — mutating breaks it, `Object.assign` is the same thing verbose). It was net-negative (flagged the *correct* pattern across every store/state file). Plus the per-rule floods (`vitest/expect-expect`, `jsx-a11y/label-has-associated-control`, …). **The lesson (3× now): before gating a rule, READ its findings — `no-thenable`/`valid-expect`/`no-control-regex`/`no-map-spread` all looked fixable but blanket-fixing would have shipped bugs or churned correct code.**
- **Remaining `warn` backlog** (burn-down candidates, fix-and-promote next): `unicorn/no-new-array` (24), `promise/no-callback-in-promise` (9), `unicorn/no-useless-fallback-in-spread` (3), the jsx-a11y interactivity set. **Two big policy rules** are a separate decision (not bugs): `typescript/no-non-null-assertion` (~296 — `!` is a deliberate TS escape-hatch in this codebase; accept-and-`off`-in-lib OR commit to the `if (!x) return` style) and `no-console` (~115 after overrides — split `off` in scripts/CLI/examples, `error` in shipped `packages/*/src`).
