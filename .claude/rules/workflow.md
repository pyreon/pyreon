...
| **Check Doc Claims — red on a PR that touched no docs** | The count on `main` is already wrong, and every branch cut from it inherits the failure. Two doc PRs merging in parallel each bump the SAME claim by one from the SAME base (209 → 210, twice) while the tree gains two pages; non-strict protection means neither re-ran against the other. Observed 2026-09: #3188 and #3195, leaving main claiming 210 against 211 actual. **The Docs Freshness Guard cannot heal this one** — it re-runs `gen-docs` + `gen-all`, and a doc-claim count is hand-written prose neither generator writes, so it stays red until a human edits the number | Check main FIRST rather than hunting your own diff: `git show origin/main:CLAUDE.md \| grep 'doc pages'` against the gate's own count (`countDocPages` in `packages/tools/cli/src/doctor/gates/doc-claims.ts` walks ALL of `docs/**` for `.md`, so a `docs/src/content/docs` grep undercounts). If main is red, fix the number in your branch — identical one-line edits auto-merge, so every in-flight PR can carry it and whichever lands first heals main |
