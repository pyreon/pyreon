...
- If a suite has a "skip when the artifact is missing" guard, use **`hasBuiltLib(path, what)`** from `@pyreon/test-utils/built-lib` — a skipped suite must never masquerade as coverage. It prints what it skipped and why, and THROWS under `PYREON_REQUIRE_BUILT_LIB=1`, which the CI test cells set because they restore the bootstrap and therefore guarantee `lib/`.

  Two details are the whole reason it is a helper rather than a line of `existsSync`. **(a) The right answer differs by environment.** A fresh worktree legitimately has no `lib/`, so a bare `expect(existsSync(LIB)).toBe(true)` — the shape this rule used to prescribe, and which two suites adopted — fails locally and tells a contributor to fix something that is not broken; the env flag is what lets the same guard skip there and fail in CI. **(b) A pattern applied to ONE file is folklore.** `@pyreon/lint`'s bin-liveness spec had the loud assertion and its own sibling `parallel.test.ts` covered only one of its two artifacts, while `charts`' plot tree-shaking and `store`'s dev-gate stripping had none at all — seven guarded specs, four of which could vanish with no signal, including the one that exists because a published bin was once a total no-op. The helper makes the behaviour structural instead of remembered.
