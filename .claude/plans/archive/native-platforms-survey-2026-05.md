# Native platforms — design exploration (ARCHIVED 2026-05-21)

**Status**: ARCHIVED. Superseded by [`.claude/plans/native-platforms.md`](../native-platforms.md), which commits to PMTC (Pyreon Multi-Target Compiler) as the chosen direction.

This survey laid out four options (RN bridge / compile-to-source / signal-aware bridge / Skia) and recommended Option C (signal-aware bridge) under the framing "3-4 month MVP, reuse what's possible." Under the user's reframed scope — "truly native, take all the time, build from scratch if needed, one day" — the right answer shifted to Option B (compile-to-native-source), elevated and renamed to PMTC in the live plan.

Preserved here for context: the option survey, the bridge-vs-compile trade-offs, and the original four-option analysis. The recommendation in this archived doc is **superseded**; the live plan's recommendation is authoritative.

---

# Native platforms — design exploration (original content)
...
The renderer pattern (DOM + Server in parallel today) is the load-bearing architectural choice that makes native viable. **`@pyreon/runtime-native` slots into the existing pattern; it does not require redesigning anything Pyreon-side.** The hard work is on the _native_ side of the bridge.
...
> _Pyreon-native: write once, render to web, server (SSR), AND iOS/Android native widgets. Signal-based reactivity means the bridge is fundamentally cheaper than React Native's — every UI update is a single property message, never a serialized diff. Hot reload preserves both component state and native UI state. 10-100× smaller bridge payloads, same widget fidelity._
