...
- **#1 — collapse tail / partial collapse: REAL, roadmap-scale (the genuine open P0 work).** The honest open items are exactly the "Remaining" list above (cross-package real-app gate; partial collapse for sites with *some* non-literal props — collapse the static dimension slice, keep the dynamic one runtime). First measurable step when picked up: instrument `scanCollapsibleSites` bail reasons on the real `examples/ui-showcase` + `@pyreon/ui-components` corpus and bucket by bail cause — that quantifies the partial-collapse addressable surface before any code is written (mirrors the E2 "95.3% statically resolvable" measurement that justified the slice).
