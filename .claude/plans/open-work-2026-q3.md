...
**Ad hoc — E-autosplit (compiler-driven automatic code-splitting): KILL (measured), do not re-propose.** User-driven experiment (PR #698, closed throwaway; analyzer + RESULTS.md in branch `chore/experiment-e-autosplit-graph` history if reproduction needed). A static reachability+weight analyzer (reusing `@pyreon/lint`'s `no-eager-import` deferral taxonomy + real bundle-budget gz weights) over all 95 files of `examples/app-showcase` (~110 KB gz of heavy deps) found **0/15 auto-split candidates, 0 projected removable bytes, recall 0%** vs frozen ground truth. Two structural reasons, both measured: (1) **wrong granularity** — heavy deps are the *rendered content* of their section (render-reachable within their own module); the real split boundary is the route/section, which Zero's fs-router **already auto-splits**, so the headline "no hand-written `lazy()` for routes" DX already exists and an import-graph pass adds nothing; (2) **fragile where it could apply** — the lone genuine candidate (`ExportButtons` deferred `download()`) is missed because idiomatic handlers are extracted named functions, not inline JSX arrows → needs intraprocedural call-graph/escape analysis, not import-site inspection. Conclusion: a smarter compiler *over the import dependency graph* is empirically ~0-surface and brittle for auto-chunking; the compiler cannot get meaningfully smarter at the import level. Residual leverage, if ever pursued, is narrower and **must be sized before built**: **F1** (cheap, ~1 day, high decision-leverage) — measure app-showcase initial-chunk gz with Zero route-split on vs forced-off; if route-level already removes ~all the heavy bytes (likely), the whole direction is closeable. **F2** (only if F1 shows real residual) — escape-analysis auto-split of *non-route* deferred subtrees + extracted-handler heavy calls; correctness-risky (auto-inserted Suspense/`import()` boundary). Neither built. Same shape as the resumability E1 kill: a bounded measurement that retired an attractive multi-week build.
