...
~7.8% at create-10k — the per-row signal allocation and keyed-`<For>` map, as
documented; `append`'s cost is unpublished (void, see above).
...
Scope caveat SUPERSEDED TWICE (2026-08-04, then 2026-08-14, then retracted 2026-08-18): the full 8-framework field has had three same-day `--repeat 5`-class runs and one full retraction. The 2026-08-14 run's verdicts ("Pyreon outright on partial-update, remove, create-10k; ties on create-1k, replace, swap, append; select at floor; clear outright Octane") are themselves now retracted for the same reason as the rest of this section — Octane's `String(row.id)` handicap was still active. **The corrected verdicts are in the header paragraph above**: 1 outright (create-10k), 6 ties (create-1k, replace, partial-update, swap, remove, select), 1 loss (clear rows), 1 void (append) — pending merge of #2893/#2894/#2895/#2896/#2897/#2899. The 7-framework table further above predates Octane entirely and is kept only as a historical per-op medians record.
