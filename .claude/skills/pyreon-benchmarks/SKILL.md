...
### Charts vs ECharts 6 — 2026-09-23, `--repeat 3` (60 pooled samples/op), load 6.83 → 13.14 stamped (after the OptionChart plan memo)
...
| --- | --- | --- | --- | --- |
| mount 1k | 13.12 | 12.91 | **4.88** | 1.54 |
| mount 100k | **30.54** | 35.19 | 49.02 | 20.27 |
| update 100k (every value) | 20.70 | **14.79** | 34.03 | 15.93 |
| update 1k (one value) | 1.97 | **1.88** | 2.36 | 0.72 |

**Verdicts.**

- **Pyreon wins three of four ops outright.** The 100k update is 1.6–2.3× faster than ECharts, the 100k mount 1.61× (PlotChart) and 1.39× (OptionChart), and the 1k update 1.20–1.26×. An earlier run at load ~6 (before the plan memo) gave the same ordering and ratios within ±0.1×, with lower absolutes.
- **Pyreon LOSES the 1k mount: 2.67× slower.** The whole loss is the default offscreen accessible data table. The chart itself (the diagnostic arm) mounts in 1.54ms, 3.2× faster than ECharts. ECharts ships no table: its `aria` is off by default. About 11ms of the 13.1ms is creating and LAYING OUT a 1,000-row table. Fixed table layout plus containment saved only ~1ms, and the rest is inherent to 3,000 real table cells.
