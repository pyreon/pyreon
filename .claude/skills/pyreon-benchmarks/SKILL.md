...
**What is actually true, and the two statements are not the same.** The OP is layout-dominated: ~75ms of layout that every entry pays identically, so create/replace/remove are hard to WIN and the suite ranks frameworks there poorly. But the **GAP** to Vanilla is overwhelmingly framework JS. Three independent instruments agree at **~3.5-3.8ms** (PR #2902's split: JS 10.69 vs 7.17ms, layout 75.11 vs 74.07; PR #2954's ablation ladder: 3.82ms [CI 3.62-4.00], layout delta spans zero). Conflating "the op is layout-bound" with "the gap is layout-bound" is what produced the retired advice.
