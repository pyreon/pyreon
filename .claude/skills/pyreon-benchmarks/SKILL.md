...
### Flow diagram — `@pyreon/flow` vs React Flow 12 (PRELIMINARY, 2026-09-08)

`bun run --filter=@pyreon/example-benchmark bench:flow` (`bench-scenarios.ts --scenario flow`, `src/impl/scenario-flow.ts`): 500 nodes / 499 edges, six ops, both arms driving their library's PUBLIC imperative API (`createFlow` + `updateNode`/`selectNode`/`addNode`/`viewport.set` vs `ReactFlowInstance.updateNode`/`addNodes`/`setViewport` under `flushSync`), per-iteration DOM verification, React Flow's deferred edge pass awaited OUTSIDE the timed region (it hides nodes until measured). **The only runs so far were at load ~7 (smoke) and a Pyreon-only pass at load ~10 — NOT publishable; a load<3 `--repeat 3` run is owed before any of these numbers leaves this paragraph.** Smoke medians: mount Pyreon 14.5 ms vs React Flow 15.2 ms (1.05×); drag one node ×60 frames 4.2 vs 34.8 ms (8.2×); select 160 µs vs 1.15 ms (7.2×); add 50 nodes + 50 edges 2.3 vs 4.4 ms (1.9×); pan+zoom ×60 ticks 197 µs vs 22.2 ms (~110×); unmount 1.4 vs 8.5 ms (6×). Caveats that travel with the eventual number: no synthetic pointer events on either side (a real drag would add each library's hit testing + d3-drag, un-drivable offscreen); React Flow re-renders its node wrappers on every store write, which is what the drag/pan ratios measure — that is its architecture, not a handicap; author-judged. The Pyreon arm's selectors exclude the `.pyreon-flow-edge-interaction` hit path so the count stays 1 per edge once #3361 lands.
