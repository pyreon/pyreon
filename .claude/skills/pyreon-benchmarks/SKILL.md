...
- **Pyreon LOSES the 1k mount: 2.28× slower** (was 2.67× before the table was chunked). The whole loss is the default offscreen accessible data table. The chart itself (the diagnostic arm) mounts in 1.45ms, 3.1× faster than ECharts. ECharts ships no table: its `aria` is off by default. About 9ms of the 10.4ms is creating and laying out a 1,000-row table.
  - **Chunking the table (50-row `<tbody>` blocks with `content-visibility: auto`) took ~2.1ms off** (12.54 → 10.41ms back-to-back, ECharts 4.58 / 4.71). The rows stay in the accessibility tree — `a11y-table-chunks.browser.test.tsx` reads Chromium's AX tree over CDP; a `display:none` control drops them, while `content-visibility:hidden` does NOT (Chromium keeps its rows too, so it is useless as a control). Fixed table layout plus containment had saved only ~1ms before that; what remains is creating 3,000 real cells.
