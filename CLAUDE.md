...
73 published packages across 6 categories under `packages/`, plus 7 private support packages:
...
- `packages/tools/` (12 published): cli, lint, mcp, vite-plugin, typescript, storybook, atlas, react-compat, preact-compat, vue-compat, solid-compat, svelte-compat; + `devtools` (private)
...
Plus `docs/` (Pyreon-native docs site on @pyreon/zero + @pyreon/zero-content — 194 doc pages covering all packages, `<Example>` live-mount primitive, dogfoods the framework) and `examples/`.
