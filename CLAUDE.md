...
CI runs the playground + ssr-showcase + fundamentals projects on every PR as a required check (`E2E` job), plus `bun run test:e2e:ui-regression` and `bun run test:e2e:compat` as separate steps (own configs, own webServer boots). **77 tests total** (43 playground + 12 ssr-showcase + 10 ui-showcase regression + 12 compat layer) plus 5 fixme'd compiler-bug tests, ~4 minutes wall-clock. Playwright report uploaded as artifact on failure.
