...
- **api-reference.ts** uses OPT-IN region markers per package: `// <gen-docs:api-reference:start @pyreon/<name>>` / `... end @pyreon/<name>>`. Marker-less packages stay hand-written. 25 packages are on the MCP pipeline today (reactivity, core, router, runtime-dom, store, state-tree, validation, form, query, hooks, permissions, machine, storage, i18n, document, flow, charts, code, hotkeys, table, virtual, feature, rx, toast, url-state). The 7 remaining hand-written blocks — document-primitives, head, lint, mcp, server, ui-core, unistyle — don't have manifests yet and flip individually as their manifests are authored.
