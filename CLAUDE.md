...
| `@pyreon/lint`           | Pyreon-specific linter — 67 rules, 13 categories, config files, watch mode, AST cache, LSP server                                                       |
...
- `listRules()` — returns metadata for all 67 rules
...
- 67 rules across 13 categories: reactivity (13), jsx (11), lifecycle (5), performance (5), ssr (3), architecture (7), store (3), form (3), styling (4), hooks (3), accessibility (3), router (4), ssg (3)
...
VitePress documentation site at `docs/` — part of the monorepo workspace. 78 doc pages covering all packages. Interactive `<Playground>` component embeds live code editors with sandboxed preview on 10+ pages (reactivity, core, runtime-dom, store, rx, form, machine, hooks, permissions, i18n, toast, storage, url-state). Code runs via ESM CDN imports in a sandboxed iframe with dark mode support.
...
- `@pyreon/zero` `mode: "ssg"` now actually generates per-route HTML at build time. Before this fix, `mode: "ssg"` and `ssg.paths` were typed in `types.ts` but had no runtime — the plugin had zero Rollup build hooks, so apps configured for SSG silently shipped a SPA shell with no per-route HTML. The new `ssgPlugin()` (auto-wired into `zeroPlugin()` when `mode === 'ssg'`) hooks `closeBundle`, runs a programmatic Vite SSR sub-build of a synthetic entry that imports `virtual:zero/routes` + zero's `createServer`, loads the resulting handler, and renders each path to `dist/<path>/index.html`. Path resolution: `ssg.paths` accepts `string[]`, `() => string[]`, or `() => Promise<string[]>`; if neither is set, the plugin auto-detects static paths from the fs-router tree (anything without `:param` or `*` segments). The recursive sub-build is gated by an env-var flag so it doesn't infinitely re-trigger itself. **Breaking change for `zeroPlugin()` consumers**: it now returns `Plugin[]` (`[mainPlugin]` for non-SSG modes, `[mainPlugin, ssgPlugin]` for SSG). Vite's plugins array natively accepts nested arrays, so `plugins: [pyreon(), zero()]` keeps working — but downstream test code that assumed `zeroPlugin()` returned a single Plugin needs `.[0]` or array-aware access.
  ...
  **Closes the doc limitation from L4.** L4 documented "no layout chrome" prominently in `docs/docs/zero.md:_404.tsx convention`. L5's update inverts the note: layouts DO wrap the 404 now. Host routing (the second L4 limitation) is unchanged — static hosts still need explicit per-locale 404 routing for unmatched URLs under locale prefixes.
  ...
  **M1 correctness fixes (single PR closing four typed-but-incomplete behaviors).** Surfaces after the SSG roadmap completed; each one independently bisect-verified.
  ...
  - **M1.3 — build-time ISR docs.** PR I shipped the runtime adapter hook + manifest format but `docs/docs/zero.md` covered runtime ISR only. M1.3 replaced the ~10-line ISR section with a ~135-line section covering BOTH runtime ISR (`mode: 'isr'`) AND build-time ISR (per-route `revalidate` export + `Adapter.revalidate(path)`), with the per-platform env-var table, cacheKey auth-gated warning + two examples, per-adapter caveats (Vercel POSTs to `/api/_pyreon-revalidate`, Cloudflare hits Cache API delete with rate-limit caveats, Netlify needs a Build Hook URL, static adapters are no-op), and composition guidance for mixing both layers. Drive-by markdown lint fix on the manifest table separator (`|---|` → `| --- |`).
  ...
  **M3.B docs + canonical example (single PR landing the SSG docs + showcase).** Follow-up to M3.A — extracts the SSG reference into a dedicated docs page AND extends the canonical blog example to exercise M3.A's three features end-to-end. Also closes a false-positive in `pyreon/missing-get-static-paths` that the example surfaced on first run.
  ...
  - **M3.2 — dedicated `docs/docs/ssg.md` page.** SSG content previously lived inside `zero.md` as a ~220-line subsection. The new dedicated page is ~580 lines covering: when-to-use SSG vs SSR/SPA/ISR, static + dynamic routes, `_404.tsx` with host-routing reference, loader-thrown redirects, per-route `revalidate` exports + the platform adapter table, `vercelRevalidateHandler` scaffold (M3.1) with security rationale, concurrency + progress reporting, subpath / base-path deploys, i18n route duplication + per-locale 404 + hreflang sitemap, sitemap auto-emit, render-error handling, path-collision detection (M1.4), the `pyreon doctor --check-ssg` audit (M3.4), the three SSG lint rules (M3.5), full manifests reference, per-platform deployment snippets (Netlify / Cloudflare Pages / GitHub Pages / Vercel / S3+CloudFront / nginx / Caddy), Next.js / Astro migration cheatsheet. Cross-referenced from `zero.md` (which is now slimmed to a quick-start overview) and wired into the VitePress sidebar under "Meta-Framework" right after Zero. Total doc-page count bumped to 77 (1 new).
  ...
  Reference: `docs/docs/ssg.md` (new), `examples/cpa-pw-blog/{src/routes/blog/[slug].tsx, src/routes/api/_pyreon-revalidate.ts, README.md}`, `packages/tools/lint/src/rules/ssg/missing-get-static-paths.ts` (API-route skip), `packages/core/compiler/src/ssg-audit.ts:detectDynamicRouteMissingGetStaticPaths` (parallel skip).
