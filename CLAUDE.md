...
| `@pyreon/atlas` | AI-native component workbench — derives a verified, machine-readable component catalog from source (`atlas scan`: controls from props, scenarios from variant axes incl. rocketstyle dimensions, five-check verify verdicts), serves a zero-config workbench (`atlas dev`: real Vite compiler, live controls, canvas addons, axe-core a11y panel, autodocs, Reactivity Lens), and runs the browser half of verification (`atlas verify-browser`: reactive coverage measured on the page's OWN devtools-bridge instance — new-node diff, workbench chrome excluded — plus pixelmatch snapshot baselines, merged back into `atlas-catalog.json`). Authoring opt-in via `atlas.config.ts` (theme/wrapper/presets/authored `play` scenarios). NO singleton sentinel by design — the scan loader loads scanned projects that import atlas |
