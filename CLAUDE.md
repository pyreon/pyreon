...
- `@pyreon/zero` `mode: "ssg"` now actually generates per-route HTML at build time. Before this fix, `mode: "ssg"` and `ssg.paths` were typed in `types.ts` but had no runtime — the plugin had zero Rollup build hooks, so apps configured for SSG silently shipped a SPA shell with no per-route HTML. The new `ssgPlugin()` (auto-wired into `zeroPlugin()` when `mode === 'ssg'`) hooks `closeBundle`, runs a programmatic Vite SSR sub-build of a synthetic entry that imports `virtual:zero/routes` + zero's `createServer`, loads the resulting handler, and renders each path to `dist/<path>/index.html`. Path resolution: `ssg.paths` accepts `string[]`, `() => string[]`, or `() => Promise<string[]>`; if neither is set, the plugin auto-detects static paths from the fs-router tree (anything without `:param` or `*` segments). The recursive sub-build is gated by an env-var flag so it doesn't infinitely re-trigger itself. **Breaking change for `zeroPlugin()` consumers**: it now returns `Plugin[]` (`[mainPlugin]` for non-SSG modes, `[mainPlugin, ssgPlugin]` for SSG). Vite's plugins array natively accepts nested arrays, so `plugins: [pyreon(), zero()]` keeps working — but downstream test code that assumed `zeroPlugin()` returned a single Plugin needs `.[0]` or array-aware access.
  ...
  **Two prominent limitations of PR C** (apply equally to PR K's per-locale fan-out): (1) **No layout chrome on the rendered 404.** Bypassing the router means no matched-chain walk → no `_layout.tsx` wrapping. Parent layouts that wrap every regular page (sticky headers, sidebars, footers, PyreonUI provider chrome) do NOT appear on the 404 output. Users who want branded chrome must inline it into `_404.tsx` itself. Tracked for refactor by PR L5 (router-driven 404 render via synthetic non-matching path). (2) **Host routing is required.** SSG writes `dist/404.html` but the static host (or your own reverse proxy) is responsible for SERVING it when an incoming URL doesn't match a prerendered file. Most managed hosts (Netlify / Cloudflare Pages / GitHub Pages / Vercel static) wire this by convention; S3+CloudFront / nginx / Caddy need explicit config. See `docs/docs/zero.md:_404.tsx convention` for per-platform deploy snippets. Without host routing, the framework emits the file but unmatched URLs return 404 with empty/default body — the carefully-rendered 404 HTML never reaches the user.
  ...
  **Limitations / known gaps**: (a) **404 renders WITHOUT layout chrome.** The not-found renderer pushes the component through `h(NotFound, null)` — bypassing the router's matched-chain walk. Parent `_layout.tsx` components (headers, footers, navigation) do NOT wrap the rendered 404 HTML, regardless of locale. Users wanting branded chrome on the 404 page must inline the relevant markup into `_404.tsx` itself. This is shared with PR C's default-locale 404 — PR K just inherited the same render shape and fanned it per-locale. PR L5 tracks the router-driven render refactor that would close this gap (renders 404 through a synthetic non-matching path so the matched chain walks to the layout boundary the `_404.tsx` sits inside). (b) **Host routing is required for unmatched URLs.** SSG writes `dist/{locale}/404.html` per locale BUT the static host must be configured to route `/de/unknown-page` → `dist/de/404.html`. Most managed hosts only serve the ROOT `dist/404.html` by convention; per-prefix 404 routing needs explicit declarations (Netlify `[[redirects]]`, Cloudflare Pages `_redirects`, nginx `try_files`, Caddy `handle_errors` per-locale matcher). GitHub Pages does NOT support per-directory 404 customization at all. Documented in `docs/docs/zero.md` `_404.tsx convention` section with per-platform snippets. (c) **One `_404.tsx` source = identical rendered HTML across locales.** ssr-showcase ships a single `_404.ts`; the per-locale variants all render byte-identical HTML. Real i18n apps with `useI18n()` in `_404` would see locale-specific text via the hydration-time t-call — server-side i18n in 404 rendering needs the runtime context wired in, which doesn't happen on the SSG path today (the 404 renderer doesn't navigate the router, so no locale context). (d) **`x-default` always uses the default locale's URL** — no `hreflangXDefault: false` opt-out yet; can be added when a real app needs it. (e) **No per-locale sitemap shards** (single sitemap with hreflang clusters chosen as the Google-canonical shape; sitemap-index + per-locale sharding can be added when a site outgrows the single-file size limit ~50k URLs).
