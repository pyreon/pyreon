...
- `@pyreon/zero` `mode: "ssg"` now actually generates per-route HTML at build time. Before this fix, `mode: "ssg"` and `ssg.paths` were typed in `types.ts` but had no runtime — the plugin had zero Rollup build hooks, so apps configured for SSG silently shipped a SPA shell with no per-route HTML. The new `ssgPlugin()` (auto-wired into `zeroPlugin()` when `mode === 'ssg'`) hooks `closeBundle`, runs a programmatic Vite SSR sub-build of a synthetic entry that imports `virtual:zero/routes` + zero's `createServer`, loads the resulting handler, and renders each path to `dist/<path>/index.html`. Path resolution: `ssg.paths` accepts `string[]`, `() => string[]`, or `() => Promise<string[]>`; if neither is set, the plugin auto-detects static paths from the fs-router tree (anything without `:param` or `*` segments). The recursive sub-build is gated by an env-var flag so it doesn't infinitely re-trigger itself. **Breaking change for `zeroPlugin()` consumers**: it now returns `Plugin[]` (`[mainPlugin]` for non-SSG modes, `[mainPlugin, ssgPlugin]` for SSG). Vite's plugins array natively accepts nested arrays, so `plugins: [pyreon(), zero()]` keeps working — but downstream test code that assumed `zeroPlugin()` returned a single Plugin needs `.[0]` or array-aware access.
  ...
  **`getStaticPaths` composition**: each duplicate route inherits the same `exports.getStaticPaths`. The SSG plugin's `expandUrlPattern` step then expands `/blog/[slug]` × locales × enumerator output into the cross-product (`/blog/a`, `/blog/b`, `/de/blog/a`, `/de/blog/b`, … under `prefix-except-default` with 2 slugs × 3 locales = 6 paths). Cardinality compounds, by design — `ssg.concurrency` (PR D) limits in-flight renders independent of route count.
  ...
  - **Real-app build** (`scripts/verify-modes.ts` — `ssr-showcase × ssg-i18n` cell with `locales: ['en','de','cs'], strategy: 'prefix-except-default'`, PLUS `ssr-showcase × ssg-i18n-prefix` cell with `strategy: 'prefix'`): the first cell asserts both shapes of the cross-product — static-route × locale (`dist/about/index.html`, `dist/de/about/index.html`, `dist/cs/about/index.html`) AND dynamic-route × locale (`dist/posts/{1,2,3}/index.html` from `getStaticPaths` + `dist/{de,cs}/posts/{1,2,3}/index.html` from the duplicated dynamic routes, 9 paths total). The prefix-strategy cell asserts EVERY locale is prefixed including default (`dist/en/about/index.html` exists, `dist/about/index.html` does NOT). Bisect-verified by reverting the `expandRoutesForLocales` calls in BOTH `ssg-plugin.ts:autoDetectStaticPaths` AND `vite-plugin.ts`'s virtual-routes load — cells fail with `expected file: dist/de/index.html` (de-locale variant never enumerated, SSG didn't render it). Restore → 7/7 ssr-showcase cells pass.
  - **Real-Chromium runtime** (`e2e/ssg-i18n.spec.ts` via `bun run test:e2e:ssg-i18n`): builds the same `ssr-showcase` app with `vite.config.i18n.ts` and serves via `vite preview` on port 5199 — config in `playwright.ssg-i18n.config.ts`. 5 specs exercise the RUNTIME contract that verify-modes can't: real browser hydration at each per-locale path, dynamic-route × locale composition under live router resolution (`/de/posts/1/` renders the right post with `getByTestId('post-title')` matching the inherited `getStaticPaths` enumerator output), and post-hydration signal interactivity at `/de/`. Bisect-verified by reverting the same `expandRoutesForLocales` calls — 4 of 5 specs fail (the default-locale spec correctly still passes, since `prefix-except-default` keeps the default unprefixed regardless). Restore → 5/5 pass. **Trailing-slash gotcha**: vite preview doesn't auto-rewrite nested directory paths like `/de/posts/1` to `/de/posts/1/index.html` — it falls back to the SPA index (the home page). Specs use trailing slashes throughout; static hosts (Netlify, Cloudflare Pages) auto-rewrite both forms, so this is a `vite preview` artifact, not a framework constraint. Same exclusion as ssg-subpath.spec.ts. **Out-of-scope**: locale-aware RouterLink emission (so `<RouterLink to="/posts/1">` rendered under `/de/posts` emits `href="/de/posts/1"`) is NOT covered by PR H — RouterLinks emit literal hrefs, which means cross-locale navigation drops to the default locale. That's a separate locale-aware navigation feature on the open-work backlog, not part of route duplication.

  Reference: `packages/zero/zero/src/i18n-routing.ts:expandRoutesForLocales` + the new `i18n?: I18nRoutingConfig` field on `ZeroConfig`. Real-app gates: `examples/ssr-showcase/vite.config.i18n.ts` (build), `playwright.ssg-i18n.config.ts` (preview + e2e), `e2e/ssg-i18n.spec.ts` (5 specs).
...
CI runs the playground + ssr-showcase + fundamentals projects on every PR as a required check (`E2E` job), plus `bun run test:e2e:ui-regression`, `bun run test:e2e:compat`, `bun run test:e2e:app-showcase`, `bun run test:e2e:ssg-subpath`, and `bun run test:e2e:ssg-i18n` as separate steps (own configs, own webServer boots). **96 tests total** (43 playground + 12 ssr-showcase + 10 ui-showcase regression + 12 compat layer + 4 app-showcase flow + 4 app-showcase dnd + 1 app-showcase charts + 4 ssg-subpath + 5 ssg-i18n + 1 fundamentals multi-route SWEEP) plus 5 fixme'd compiler-bug tests, ~5 minutes wall-clock. Playwright report uploaded as artifact on failure.
...
| `ssg-i18n`      | ✓ active | SSG i18n route duplication runtime gate (PR H). Builds `examples/ssr-showcase` with `vite.config.i18n.ts` (sets `zero({ i18n: { locales: ['en','de','cs'], defaultLocale: 'en', strategy: 'prefix-except-default' } })`) and serves the prerendered 24-page dist via `vite preview` on port 5199. 5 specs in [`e2e/ssg-i18n.spec.ts`](e2e/ssg-i18n.spec.ts): default-locale unprefixed render (`/about/`), `/de/about/` prefixed render (non-default locale duplicate), `/cs/about/` (third locale — multi-locale cardinality sanity), `/de/posts/1/` dynamic-route × locale composition (proves `getStaticPaths` inherits across the duplicate AND the runtime router resolves the locale-prefixed dynamic segment to the right component + params), post-hydration counter interactivity at `/de/` (proves hydration parity under locale prefix). Companion to verify-modes `ssr-showcase × ssg-i18n` (prefix-except-default) and `ssg-i18n-prefix` (every locale prefixed) cells — verify-modes gates the emitted filesystem tree, this gates the live-browser runtime. Bisect-verified: reverting `expandRoutesForLocales` in both `ssg-plugin.ts:autoDetectStaticPaths` AND `vite-plugin.ts`'s virtual-routes load → 4/5 specs fail (default-locale spec correctly passes since `prefix-except-default` keeps default unprefixed regardless). Lives in [`playwright.ssg-i18n.config.ts`](playwright.ssg-i18n.config.ts) — separate config (same boot-isolation reason as ssg-subpath / ui-regression / compat-layers). Specs use trailing slashes throughout because `vite preview` doesn't auto-rewrite nested directory paths like `/de/posts/1` → `/de/posts/1/index.html` (falls back to SPA index). Static hosts auto-rewrite both forms — same `vite preview` artifact gotcha as ssg-subpath. Locale-aware RouterLink emission is out of scope for PR H (RouterLinks emit literal hrefs; cross-locale navigation drops to the default locale unless a separate locale-aware-link feature lands). |
