...
| `@pyreon/loom` | Monorepo dependency observatory — `loom scan` reads the workspace like an install tool (npm/bun/yarn/pnpm globs incl. `packages/*/*`) and emits the fabric as DATA: internal graph (longest-path depths V-1-bounded, RUNTIME cycles — dev edges deliberately excluded, blast radius), external version-usage map, and 7 detectors with honest severities (version-drift with peer-contract + strict-superset-containment recognition / internal-range / cycle / phantom-dep / prod-import-of-dev-dep / peer-mismatch / unused-dep-as-info); red exit gates CI, `loom-report.json` is the machine surface. `loom dev` serves the five-view observatory (layered graph / adjacency matrix / cycle cards / impact ranking / manifest table + detail panel, ⌘K, ↑↓, dark/light) on the PUBLIC Pyreon UI stack; vite + vite-plugin OPTIONAL peers. Lexical import scan strips comments + template literals (recipe catalogs carry import lines as strings). Reads DECLARED truth only — no lockfile/registry (documented future layers). NO singleton sentinel (tool precedent) |
