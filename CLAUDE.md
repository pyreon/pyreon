...
- **Instrumented layers (47 counters today)**:
  ...
  - `query`: `useQuery`, `useMutation`, `observerNotify`, `setOptions`, `invalidate`, `isFetchingScan` (foundation PR — baselines for 4 stress journeys recorded in `perf-results/`: `queryMount-1000` (1k useQuery + 2k observerNotify from initial subscribe), `queryNotify-10k` (10 subscribers × 1000 cache updates ≈ 10k observerNotify, structural fan-out signal), `mutationInvalidate-1000` (1k mutations × 5 keys = 5000 invalidate, downstream observerNotify ≈ 405 from cache fan-out), `isFetchingScan-10k` (10k cache events × 2 internal events per setQueryData ≈ 20k scan calls — confirms `useIsFetching`'s O(cacheSize) scan is the dominant cost on update-heavy pages). The likely first follow-up is **observerNotify lazy signal allocation**: each `useQuery` allocates 9 signals at setup that the subscribe callback `batch()`-writes on every cache update, but most consumers only read 1-2 of them — the wasted writes show up as the 20070 `signalWrite` count in `queryNotify-10k`. A 5th `queryReactiveKey-1000` journey is deferred — the planned shape (N queries with reactive `queryKey` + K signal flips → N × K setOptions) hit a Pyreon reactive-effect edge case where tight-loop signal writes from `page.evaluate()` boundaries don't propagate to all subscribers; investigation continues in a follow-up PR.)
...
VitePress documentation site at `docs/` — part of the monorepo workspace. 74 doc pages covering all packages. Interactive `<Playground>` component embeds live code editors with sandboxed preview on 10+ pages (reactivity, core, runtime-dom, store, rx, form, machine, hooks, permissions, i18n, toast, storage, url-state). Code runs via ESM CDN imports in a sandboxed iframe with dark mode support.
