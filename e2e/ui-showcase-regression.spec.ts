...
test.describe('ui-showcase — runtime mode prop change', () => {
  // KNOWN GAP: signal-driven `mode` prop on `<PyreonUI>` does NOT
  // propagate to its internal `resolveMode` computed. The inversed
  // test above proves the mode CONTEXT works correctly (inner
  // inversed inherits and flips the parent's mode), but
  // `<PyreonUI mode={signal()}>` where `signal` updates at runtime
  // doesn't re-resolve. Probed empirically:
  //
  //   PROBE_outer: Mode: light          ← static "light", correct
  //   PROBE_inner: Mode: dark           ← inversed flip, correct
  //   PROBE_swappable: Mode: light      ← BEFORE click
  //   PROBE_swappable: Mode: light      ← AFTER click (signal flipped
  //                                       in the parent, but PyreonUI's
  //                                       resolveMode didn't pick it up)
  //
  // Likely root cause: `props.mode` access inside `resolveMode()` is
  // wrapped in `computed`, but the compiler's `_rp` wrapping on the
  // JSX prop may not be tracking the underlying signal correctly
  // through the destructuring-blind getter — OR PyreonUI's mode
  // computed is mounted before the signal is established. Needs
  // dedicated investigation in the PyreonUI / compiler interaction.
  // Tracked here so a future fix flips these tests green.
