...
console.log(`create1k: ${(performance.now()-t0).toFixed(1)}ms, rows=${el.querySelectorAll("tr").length}`)
...
rowsSig().forEach((row, i) => { if (i % 10 === 0) row.label.update(l => l + " !!!") })
...
const c = [...rowsSig()]; const tmp = c[1]!; c[1] = c[998]!; c[998] = tmp; rowsSig.set(c)
...
console.log(`create10k: ${(performance.now()-t6).toFixed(1)}ms, rows=${el.querySelectorAll("tr").length}`)
