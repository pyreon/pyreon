...
- `zero({...})` options are validated with did-you-mean suggestions. A missing `pyreon()` plugin, a page with no default export, a default-only layout and a literal `loader` each fail with one error that names the file. Dev SSR errors are logged to the terminal.
