...
- **`by={row => row.index}` keys the SLOT, not the data — read the row's data reactively.** A row stays mounted while its index stays in the window, so anything read ONCE from the data (`{items[row.index].name}` against a plain array captured at setup) is stale when the data under that index changes — a prepend, a filter, a refetch. Either read through the signal inside an accessor (`{() => items()[row.index]?.name}`), or pass `getItemKey: (i) => items()[i].id` to the virtualizer and key the `<For>` on it (`by={row => row.key}`), so a changed item is a different row.
...
- **A `styled()` scroll container needs `ref`, not `innerRef`** — a `styled()` component forwards plain `ref` to its DOM node; `innerRef` is a silent no-op there, so `getScrollElement()` returns `null` and the list renders **zero** rows (`totalSize` still looks correct). `useVirtualizer` now dev-warns when `getScrollElement()` is still `null` after mount. `@pyreon/elements`' `Element` is the only component that treats `innerRef` as first-class.
- **Options must be a function** `() => opts` for reactive tracking. Reading signals inside is the mechanism for live recalculation. It is read once per pass and REPLACES the previous options — a key you stop returning (a conditional `getItemKey`, `rangeExtractor`) falls back to TanStack's default rather than lingering.
