...
private fun runParityChecks() {
    ...
    run { // moveSelectedNodes shifts only the selection
        val f = PyreonFlowState<NodeData>(nodes = listOf<PyreonFlowNode<NodeData>>(PyreonFlowNode("1", position = PyreonXYPosition(0.0, 0.0), data = NodeData("1")), PyreonFlowNode("2", position = PyreonXYPosition(200.0, 120.0), data = NodeData("2")), PyreonFlowNode("3", position = PyreonXYPosition(400.0, 0.0), data = NodeData("3"))), edges = listOf<PyreonFlowEdge>())
    ...
    run { // viewport: zoom steps, clamps, pan and set
        val f = PyreonFlowState<NodeData>(nodes = listOf<PyreonFlowNode<NodeData>>(PyreonFlowNode("1", position = PyreonXYPosition(0.0, 0.0), data = NodeData("1")), PyreonFlowNode("2", position = PyreonXYPosition(200.0, 120.0), data = NodeData("2"))), edges = listOf<PyreonFlowEdge>())
        f.zoomIn()
        f.zoomIn()
        f.zoomOut()
    ...
    run { // history: undo and redo across CRUD and moves
        val f = PyreonFlowState<NodeData>(nodes = listOf<PyreonFlowNode<NodeData>>(PyreonFlowNode("1", position = PyreonXYPosition(0.0, 0.0), data = NodeData("1")), PyreonFlowNode("2", position = PyreonXYPosition(200.0, 120.0), data = NodeData("2"))), edges = listOf<PyreonFlowEdge>(PyreonFlowEdge("e1", source = "1", target = "2")))
