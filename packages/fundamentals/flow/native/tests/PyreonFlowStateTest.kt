...
import com.pyreon.runtime.PyreonFlowRect
...
private fun parityRect(r: PyreonFlowRect, x: Double, y: Double, w: Double, h: Double): Boolean = abs(r.x - x) < 1e-6 && abs(r.y - y) < 1e-6 && abs(r.width - w) < 1e-6 && abs(r.height - h) < 1e-6
...
private fun runParityChecks() {
    ...
    run { // intersection and bounds follow React Flow semantics
        val f = PyreonFlowState<NodeData>(nodes = listOf<PyreonFlowNode<NodeData>>(PyreonFlowNode("a", position = PyreonXYPosition(0.0, 0.0), data = NodeData("a")), PyreonFlowNode("b", position = PyreonXYPosition(100.0, 20.0), data = NodeData("b")), PyreonFlowNode("c", position = PyreonXYPosition(400.0, 0.0), data = NodeData("c")), PyreonFlowNode("h", position = PyreonXYPosition(50.0, 10.0), data = NodeData("h"), hidden = true), PyreonFlowNode("g", position = PyreonXYPosition(1000.0, 1000.0), data = NodeData("g")), PyreonFlowNode("k", position = PyreonXYPosition(20.0, 20.0), data = NodeData("k"), parentId = "g")), edges = listOf<PyreonFlowEdge>(), searchText = { it.label })
        f.updateNodeMeasurement("c", 50.0, 20.0)
        check(parityNodes(f, listOf<Triple<String, Double, Double>>(Triple("a", 0.0, 0.0), Triple("b", 100.0, 20.0), Triple("c", 400.0, 0.0), Triple("h", 50.0, 10.0), Triple("g", 1000.0, 1000.0), Triple("k", 20.0, 20.0))), "parity: intersection and bounds follow React Flow semantics — nodes")
        check(parityEdges(f, listOf<Triple<String, String, String>>()), "parity: intersection and bounds follow React Flow semantics — edges")
        check(f.selectedNodes().sorted() == listOf<String>(), "parity: intersection and bounds follow React Flow semantics — selected nodes")
        check(f.selectedEdges().sorted() == listOf<String>(), "parity: intersection and bounds follow React Flow semantics — selected edges")
        ...
        check(f.getIntersectingNodes("a").map { it.id } == listOf<String>("b"), "parity: intersection and bounds follow React Flow semantics — query 1 intersecting")
        check(f.getIntersectingNodes("k").map { it.id } == listOf<String>("g"), "parity: intersection and bounds follow React Flow semantics — query 2 intersecting")
        check(f.getIntersectingNodes("missing").map { it.id } == listOf<String>(), "parity: intersection and bounds follow React Flow semantics — query 3 intersecting")
        check(f.getIntersectingNodes(PyreonFlowRect(0.0, 0.0, 600.0, 100.0)).map { it.id } == listOf<String>("a", "b", "c"), "parity: intersection and bounds follow React Flow semantics — query 4 intersecting")
        check(f.getIntersectingNodes(PyreonFlowRect(0.0, 0.0, 200.0, 50.0), partially = false).map { it.id } == listOf<String>("a"), "parity: intersection and bounds follow React Flow semantics — query 5 intersecting")
        check(f.getIntersectingNodes(PyreonFlowRect(1000.0, 1000.0, 50.0, 50.0)).map { it.id } == listOf<String>("g", "k"), "parity: intersection and bounds follow React Flow semantics — query 6 intersecting")
        check(f.isNodeIntersecting("a", PyreonFlowRect(140.0, 30.0, 100.0, 100.0)) == true, "parity: intersection and bounds follow React Flow semantics — query 7 isIntersecting")
        check(f.isNodeIntersecting("a", PyreonFlowRect(140.0, 30.0, 100.0, 100.0), partially = false) == false, "parity: intersection and bounds follow React Flow semantics — query 8 isIntersecting")
        check(f.isNodeIntersecting(PyreonFlowRect(0.0, 0.0, 10.0, 10.0), PyreonFlowRect(0.0, 0.0, 100.0, 100.0), partially = false) == true, "parity: intersection and bounds follow React Flow semantics — query 9 isIntersecting")
        check(f.isNodeIntersecting("missing", PyreonFlowRect(0.0, 0.0, 100.0, 100.0)) == false, "parity: intersection and bounds follow React Flow semantics — query 10 isIntersecting")
        ...
        check(parityRect(f.getNodesBounds(listOf<String>("a", "c")), 0.0, 0.0, 450.0, 40.0), "parity: intersection and bounds follow React Flow semantics — query 12 nodesBounds")
        check(parityRect(f.getNodesBounds(listOf<String>("k")), 1020.0, 1020.0, 150.0, 40.0), "parity: intersection and bounds follow React Flow semantics — query 13 nodesBounds")
        check(parityRect(f.getNodesBounds(listOf<String>("missing")), 0.0, 0.0, 0.0, 0.0), "parity: intersection and bounds follow React Flow semantics — query 14 nodesBounds")
    }
...
fun main() {
    ...
    val parityConfigured = PyreonFlowState<NodeData>(connectionMode = "loose", elevateNodesOnSelect = false, elevateEdgesOnSelect = true, autoPanOnNodeDrag = false, autoPanOnConnect = false, autoPanSpeed = -4.0)
    check(parityConfigured.connectionMode == "loose" && PyreonFlowState<NodeData>(connectionMode = "sideways").connectionMode == "strict", "Android retains connectionMode, normalising unknown values to strict")
    check(!parityConfigured.elevateNodesOnSelect && parityConfigured.elevateEdgesOnSelect, "Android retains elevateNodesOnSelect / elevateEdgesOnSelect")
    ...
    // Context menu (native long-press) and pointer hover. Mirrors Swift.
    val menus = mutableListOf<String>()
    check(!changes.emitNodeContextMenu("1"), "with no listener a context menu is not consumed")
    val stopNodeMenu = changes.onNodeContextMenu { menus.add("node:${it.id}") }
    val stopEdgeMenu = changes.onEdgeContextMenu { menus.add("edge:${it.id}") }
    val stopPaneMenu = changes.onPaneContextMenu { menus.add("pane:${it.x}") }
    val stopNodeEnter = changes.onNodeMouseEnter { menus.add("enter:${it.id}") }
    val stopNodeLeave = changes.onNodeMouseLeave { menus.add("leave:${it.id}") }
    val stopEdgeEnter = changes.onEdgeMouseEnter { menus.add("enter:${it.id}") }
    val stopEdgeLeave = changes.onEdgeMouseLeave { menus.add("leave:${it.id}") }
    check(changes.emitNodeContextMenu("1") && changes.emitEdgeContextMenu("e3") && changes.emitPaneContextMenu(PyreonXYPosition(7.0, 8.0)), "a listened context menu is consumed")
    ...
    check(menus == listOf("node:1", "edge:e3", "pane:7.0", "enter:1", "leave:1", "enter:e3", "leave:e3"), "context-menu and hover events carry the node, edge and flow position")
