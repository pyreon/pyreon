...
private fun parityNodes(f: PyreonFlowState<NodeData>, want: List<Triple<String, Double, Double>>): Boolean {
    val got = f.nodes
    if (got.size != want.size) return false
    for (i in got.indices) { if (got[i].id != want[i].first || abs(got[i].position.x - want[i].second) >= 1e-6 || abs(got[i].position.y - want[i].third) >= 1e-6) return false }
    return true
}
