...
struct PyreonFlowStateTests {
    ...
    static func runStateChecks() {
        ...
        let parityConfigured = PyreonFlowState<NodeData>(connectionMode: "loose", elevateNodesOnSelect: false, elevateEdgesOnSelect: true, autoPanOnNodeDrag: false, autoPanOnConnect: false, autoPanSpeed: -4)
        check(parityConfigured.connectionMode == "loose" && PyreonFlowState<NodeData>(connectionMode: "sideways").connectionMode == "strict", "Apple retains connectionMode, normalising unknown values to strict")
        check(!parityConfigured.elevateNodesOnSelect && parityConfigured.elevateEdgesOnSelect, "Apple retains elevateNodesOnSelect / elevateEdgesOnSelect")
        ...
        // Context menu (native long-press) and pointer hover, through the same emitters the view calls.
        var menus: [String] = []
        check(!changes.emitNodeContextMenu("1"), "with no listener a context menu is not consumed")
        let stopNodeMenu = changes.onNodeContextMenu { menus.append("node:\($0.id)") }
        let stopEdgeMenu = changes.onEdgeContextMenu { menus.append("edge:\($0.id)") }
        let stopPaneMenu = changes.onPaneContextMenu { menus.append("pane:\($0.x)") }
        let stopNodeEnter = changes.onNodeMouseEnter { menus.append("enter:\($0.id)") }
        let stopNodeLeave = changes.onNodeMouseLeave { menus.append("leave:\($0.id)") }
        let stopEdgeEnter = changes.onEdgeMouseEnter { menus.append("enter:\($0.id)") }
        let stopEdgeLeave = changes.onEdgeMouseLeave { menus.append("leave:\($0.id)") }
        check(changes.emitNodeContextMenu("1") && changes.emitEdgeContextMenu("e3") && changes.emitPaneContextMenu(PyreonXYPosition(x: 7, y: 8)), "a listened context menu is consumed")
        check(!changes.emitNodeContextMenu("missing"), "a missing node's menu is not consumed")
        changes.emitNodeMouseEnter("1"); changes.emitNodeMouseLeave("1"); changes.emitEdgeMouseEnter("e3"); changes.emitEdgeMouseLeave("e3")
        check(menus == ["node:1", "edge:e3", "pane:7.0", "enter:1", "leave:1", "enter:e3", "leave:e3"], "context-menu and hover events carry the node, edge and flow position")
        stopNodeMenu(); stopEdgeMenu(); stopPaneMenu(); stopNodeEnter(); stopNodeLeave(); stopEdgeEnter(); stopEdgeLeave()
        check(!changes.emitNodeContextMenu("1"), "unsubscribing stops the context-menu listener")
    ...
    static func parityRect(_ r: PyreonFlowRect, _ x: Double, _ y: Double, _ w: Double, _ h: Double) -> Bool { abs(r.x - x) < 1e-6 && abs(r.y - y) < 1e-6 && abs(r.width - w) < 1e-6 && abs(r.height - h) < 1e-6 }
    ...
    static func runParityChecks() {
        ...
        do { // intersection and bounds follow React Flow semantics
            let f = PyreonFlowState<NodeData>(nodes: [PyreonFlowNode(id: "a", position: PyreonXYPosition(x: 0.0, y: 0.0), data: NodeData(label: "a")), PyreonFlowNode(id: "b", position: PyreonXYPosition(x: 100.0, y: 20.0), data: NodeData(label: "b")), PyreonFlowNode(id: "c", position: PyreonXYPosition(x: 400.0, y: 0.0), data: NodeData(label: "c")), PyreonFlowNode(id: "h", position: PyreonXYPosition(x: 50.0, y: 10.0), data: NodeData(label: "h"), hidden: true), PyreonFlowNode(id: "g", position: PyreonXYPosition(x: 1000.0, y: 1000.0), data: NodeData(label: "g")), PyreonFlowNode(id: "k", position: PyreonXYPosition(x: 20.0, y: 20.0), data: NodeData(label: "k"), parentId: "g")], edges: [], searchText: { $0.label })
            f.updateNodeMeasurement("c", width: 50.0, height: 20.0)
            check(parityNodes(f, [("a", 0.0, 0.0), ("b", 100.0, 20.0), ("c", 400.0, 0.0), ("h", 50.0, 10.0), ("g", 1000.0, 1000.0), ("k", 20.0, 20.0)]), "parity: intersection and bounds follow React Flow semantics — nodes")
            check(parityEdges(f, []), "parity: intersection and bounds follow React Flow semantics — edges")
            check(f.selectedNodes().sorted() == [], "parity: intersection and bounds follow React Flow semantics — selected nodes")
            check(f.selectedEdges().sorted() == [], "parity: intersection and bounds follow React Flow semantics — selected edges")
            check(abs(f.viewport.x - 0.0) < 1e-6 && abs(f.viewport.y - 0.0) < 1e-6 && abs(f.viewport.zoom - 1.0) < 1e-6, "parity: intersection and bounds follow React Flow semantics — viewport")
            check(f.getIntersectingNodes("a").map { $0.id } == ["b"], "parity: intersection and bounds follow React Flow semantics — query 1 intersecting")
            check(f.getIntersectingNodes("k").map { $0.id } == ["g"], "parity: intersection and bounds follow React Flow semantics — query 2 intersecting")
            check(f.getIntersectingNodes("missing").map { $0.id } == [], "parity: intersection and bounds follow React Flow semantics — query 3 intersecting")
            check(f.getIntersectingNodes(PyreonFlowRect(x: 0.0, y: 0.0, width: 600.0, height: 100.0)).map { $0.id } == ["a", "b", "c"], "parity: intersection and bounds follow React Flow semantics — query 4 intersecting")
            check(f.getIntersectingNodes(PyreonFlowRect(x: 0.0, y: 0.0, width: 200.0, height: 50.0), partially: false).map { $0.id } == ["a"], "parity: intersection and bounds follow React Flow semantics — query 5 intersecting")
            check(f.getIntersectingNodes(PyreonFlowRect(x: 1000.0, y: 1000.0, width: 50.0, height: 50.0)).map { $0.id } == ["g", "k"], "parity: intersection and bounds follow React Flow semantics — query 6 intersecting")
            check(f.isNodeIntersecting("a", PyreonFlowRect(x: 140.0, y: 30.0, width: 100.0, height: 100.0)) == true, "parity: intersection and bounds follow React Flow semantics — query 7 isIntersecting")
            check(f.isNodeIntersecting("a", PyreonFlowRect(x: 140.0, y: 30.0, width: 100.0, height: 100.0), partially: false) == false, "parity: intersection and bounds follow React Flow semantics — query 8 isIntersecting")
            check(f.isNodeIntersecting(PyreonFlowRect(x: 0.0, y: 0.0, width: 10.0, height: 10.0), PyreonFlowRect(x: 0.0, y: 0.0, width: 100.0, height: 100.0), partially: false) == true, "parity: intersection and bounds follow React Flow semantics — query 9 isIntersecting")
            check(f.isNodeIntersecting("missing", PyreonFlowRect(x: 0.0, y: 0.0, width: 100.0, height: 100.0)) == false, "parity: intersection and bounds follow React Flow semantics — query 10 isIntersecting")
            check(parityRect(f.getNodesBounds(), 0.0, 0.0, 1170.0, 1060.0), "parity: intersection and bounds follow React Flow semantics — query 11 nodesBounds")
            check(parityRect(f.getNodesBounds(["a", "c"]), 0.0, 0.0, 450.0, 40.0), "parity: intersection and bounds follow React Flow semantics — query 12 nodesBounds")
            check(parityRect(f.getNodesBounds(["k"]), 1020.0, 1020.0, 150.0, 40.0), "parity: intersection and bounds follow React Flow semantics — query 13 nodesBounds")
            check(parityRect(f.getNodesBounds(["missing"]), 0.0, 0.0, 0.0, 0.0), "parity: intersection and bounds follow React Flow semantics — query 14 nodesBounds")
        }
    ...
    /// connectionMode, decided by the same pure function both views call.
    static func runConnectionModeChecks() {
        let aOut = PyreonFlowInteractiveHandle(nodeId: "a", handleId: "out", type: "source", position: .right, x: 100, y: 20)
        let aIn = PyreonFlowInteractiveHandle(nodeId: "a", handleId: "in", type: "target", position: .left, x: 0, y: 20)
        let cIn = PyreonFlowInteractiveHandle(nodeId: "c", handleId: "in", type: "target", position: .left, x: 300, y: 20)
        let cOut = PyreonFlowInteractiveHandle(nodeId: "c", handleId: "out", type: "source", position: .right, x: 400, y: 20)
        let all = [aOut, aIn, cIn, cOut]
        func resolve(_ start: PyreonFlowInteractiveHandle, _ x: Double, _ mode: String) -> PyreonFlowConnection? {
            pyreonFlowResolveConnection(from: start, handles: all, point: PyreonXYPosition(x: x, y: 20), radius: 10, connectionMode: mode)
        }
        check(resolve(aOut, 300, "strict") == PyreonFlowConnection(source: "a", target: "c", sourceHandle: "out", targetHandle: "in"), "strict: source to target connects")
        check(resolve(aOut, 400, "strict") == nil, "strict: source to source is refused")
        check(resolve(cIn, 100, "strict") == PyreonFlowConnection(source: "a", target: "c", sourceHandle: "out", targetHandle: "in"), "strict: a drag from a target handle is still source -> target")
        check(resolve(aOut, 400, "loose") == PyreonFlowConnection(source: "a", target: "c", sourceHandle: "out", targetHandle: "out"), "loose: any pair, oriented from the start")
        check(resolve(aOut, 0, "loose") == nil, "the start node is never a candidate")
    }

    /// Stacking order and loose reconnect, through the same helpers the view calls.
    static func runZOrderChecks() {
        let stacked = PyreonFlowState<NodeData>(
            nodes: [
                PyreonFlowNode(id: "a", position: PyreonXYPosition(x: 0, y: 0), data: NodeData(label: "A")),
                PyreonFlowNode(id: "b", position: PyreonXYPosition(x: 200, y: 0), data: NodeData(label: "B")),
                PyreonFlowNode(id: "c", position: PyreonXYPosition(x: 400, y: 0), data: NodeData(label: "C")),
            ],
            edges: [
                PyreonFlowEdge(id: "ab", source: "a", target: "b", zIndex: 3),
                PyreonFlowEdge(id: "ac", source: "a", target: "c"),
                PyreonFlowEdge(id: "bc", source: "b", target: "c"),
            ],
            elevateEdgesOnSelect: true)
        check(pyreonFlowEdgeStrokes(state: stacked).map(\.id) == ["ac", "bc", "ab"], "edges draw in zIndex order, stably")
        stacked.selectEdge("ac")
        check(pyreonFlowEdgeStrokes(state: stacked).map(\.id) == ["bc", "ab", "ac"], "elevateEdgesOnSelect draws the selected edge last")
        stacked.elevateEdgesOnSelect = false
        check(pyreonFlowEdgeStrokes(state: stacked).map(\.id) == ["ac", "bc", "ab"], "without elevation a selection does not reorder")
        check(pyreonFlowNodeZ(zIndex: 5, selected: true, dragging: false, elevate: true) == 105, "a selected node is raised by 100")
        check(pyreonFlowNodeZ(zIndex: 5, selected: true, dragging: false, elevate: false) == 5, "elevateNodesOnSelect false keeps its own zIndex")
        check(pyreonFlowNodeZ(zIndex: nil, selected: false, dragging: true, elevate: false) == 1000, "a dragged node is raised by 1000")
        // Auto-pan velocity: the same arithmetic as the web's auto-pan.ts.
        check(pyreonFlowAutoPanVelocity(x: 400, y: 300, width: 800, height: 600) == (0, 0), "no auto-pan away from the edges")
        check(pyreonFlowAutoPanVelocity(x: 20, y: 300, width: 800, height: 600) == (7.5, 0), "half speed 20px into the left band")
        check(pyreonFlowAutoPanVelocity(x: 780, y: 300, width: 800, height: 600) == (-7.5, 0), "half speed, negative, near the right edge")
        check(pyreonFlowAutoPanVelocity(x: 900, y: 700, width: 800, height: 600) == (-15, -15), "full speed past the edge")
        check(pyreonFlowAutoPanVelocity(x: 0, y: 0, width: 60, height: 60) == (0, 0), "a canvas too small for two bands never pans")
        let sourceTypeHandle = PyreonFlowInteractiveHandle(nodeId: "c", handleId: "out", type: "source", position: .right, x: 0, y: 0)
        check(pyreonFlowReconnectConnection(edge: stacked.getEdge("ab")!, end: "target", handle: sourceTypeHandle) == nil, "strict reconnect refuses a source handle for the target end")
        check(pyreonFlowReconnectConnection(edge: stacked.getEdge("ab")!, end: "target", handle: sourceTypeHandle, loose: true) == PyreonFlowConnection(source: "a", target: "c", sourceHandle: nil, targetHandle: "out"), "loose reconnect accepts it")
    }
    static func main() {
        runParityChecks()
        runSvgPathChecks()
