...
import { buildFlowHostHtml } from '../webview'

describe('buildFlowHostHtml — the host page cannot be escaped through its options', () => {
  ...
  it('a `background` cannot smuggle a quote into the sheet', () => {
    const html = buildFlowHostHtml({ background: 'url("x")' })
  ...
  it('an ordinary colour still reaches the sheet unchanged', () => {
    ...
    expect(buildFlowHostHtml({ background: '#0b0d12' })).toContain('background:#0b0d12')
    expect(buildFlowHostHtml({ background: 'rgb(11 13 18 / 80%)' })).toContain('rgb(11 13 18 / 80%)')
  })
})
