...
export default defineManifest({
  ...
  multiplatform: {
    ...
    nativeFrontend:
      'PyreonFlowState — `createFlow({ nodes, edges })` (v1: literal node/edge config; CRUD, selection, pan/zoom/fitView, graph queries) lowers to the native @Observable/`remember` port, plus `PyreonFlowEdgeCanvas` (SwiftUI Canvas / Compose Canvas) for drawing the built-in edge path geometry (bezier/smoothstep/straight/step/waypoint — the same 4-command vocabulary `EdgeSegment` in `types.ts` encodes) from hand-written native code; the `<Flow>` JSX auto-lowering that would let ONE `.tsx` render the diagram natively with no hand-wiring is a tracked follow-up',
  },
