...
export default defineManifest({
  ...
  multiplatform: {
    ...
    nativeFrontend:
      'PyreonFlowState lowers `createFlow` and lifecycle-owned `useFlow` configuration, the public FlowInstance operation surface (`getNodes` / `getEdges` / `getViewport` and the measurement setters lower as native property reads and calls rather than methods), portable node/edge models, and all seven deterministic layout engines to @Observable/SwiftUI and Compose. Edge-path and marker helpers lower to matching native geometry. `<Flow instance={flow}>` lowers to the interactive `PyreonFlowView` host with statically resolvable custom node and edge maps, custom connection lines, live renderer accessors, handles and connection gestures, resizing, multiple toolbars, built-in edge geometry, node drag, canvas pan/zoom, visibility, measurement, keyboard accessibility, background, controls, minimap, positioned panels, and portable inline node/edge styling. Device evidence today covers the rendered host, counts, zoom and selection; gestures, keyboard equivalents and reduced motion are toolchain-verified but not yet device-asserted. `FlowLayersContext` and `flowStyles` are browser-only by member; dynamic renderer structure, browser CSS selectors, and arbitrary SVG path strings produce named diagnostics and use explicit platform branches when required.',
  },
