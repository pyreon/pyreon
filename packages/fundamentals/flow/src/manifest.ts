...
export default defineManifest({
  ...
  multiplatform: {
    tier: 'web-only',
    rationale:
      'the core state, host, literal custom node maps with static interactive handles and node resizers, layouts, and common overlays lower natively, while toolbars and custom edge renderers still require explicit native follow-ups or the WebView bridge',
    nativeFrontend:
      'PyreonFlowState lowers `createFlow` and lifecycle-owned `useFlow` configuration, models, and operations to @Observable/Compose state; standalone `computeLayout` dispatches to the same seven deterministic native layout engines. The five object-parameter edge-path builders plus positional `getEdgePath`, `getHandlePosition`, `getNodeIntersection`, `getEffectiveDimensions`, `getFloatingEndpoints`, `getSmartHandlePositions`, `resolveHandleAnchor`, and the public marker constants/resolution/ID/collection helpers lower to native geometry with matching path/label/anchor/marker results and explicit → measured → default dimension results. `<Flow instance={flow}>` lowers to the interactive `PyreonFlowView` SwiftUI/Compose host, including literal custom node renderer dispatch with live data/selection/dragging accessors, static `<Handle>` extraction (ids, sides, offsets, drawing, connection gestures), static `<NodeResizer>` extraction (corner/edge handles, zoom-aware minimum-constrained resize gestures), and one literal-config `<NodeToolbar>` per custom node rendered in a screen-space portal, plus built-in edge geometry, node drag, canvas pan/zoom, visibility, measurement, accessibility, `<Background>`, functional `<Controls>`, `<MiniMap>`, and positioned `<Panel>` overlays. Custom edge renderer maps stay a named diagnostic rather than a silent drop.',
  },
...
const json = flow.toJSON(); flow.fromJSON(json)       // round-trip serialization`,
      mistakes: [
        'Calling `pushHistory()` before every mutation out of habit — mutations checkpoint themselves (`autoHistory`, default on) and a manual push right before one is deduped; set `autoHistory: false` only if you want the manual model back.',
        ...
        'Assuming object-snapping rescans the graph per frame — `snapToObjects` (default `true`) precomputes candidate guide lines ONCE at drag start (an O(N) pass) and each drag frame is a flat primitive-array scan; co-dragged nodes of a multi-drag are excluded as candidates (they move rigidly with the pointer). Set `snapToObjects: false` to skip object snapping entirely when you don\'t need helper-line alignment',
...
function MyNode(props: NodeComponentProps<WorkflowData>) {
  return (
    ...
    >
      ...
      mistakes: [
        'Setting an inline `outline: none` on the canvas or nodes to hide the click ring — it hides KEYBOARD focus too; the UA and `flowStyles` already scope the ring to `:focus-visible`, so a pointer click draws none.',
        'Skipping `flowStyles` and wondering why the focus ring is the browser default and animated edges still dash under reduced motion — the themed ring and the `prefers-reduced-motion` rules live in that stylesheet.',
        'Asserting keyboard a11y with `hasAttribute` — a node with `focusable: false` still HAS `tabindex` (`-1`); assert the value.',
...
for (const { id, position } of positioned) flow.updateNode(id, { position })`,
  ...
  gotchas: [
    ...
    },
    {
      ...
      note: 'Every geometry consumer (edges, layout(), fitView, snap lines, minimap, culling) resolves node size as explicit width/height → MEASURED rendered box → 150×40 default. Nodes need no explicit sizes in the browser; explicit sizes override the measurement, and headless `computeLayout` (no DOM) sees explicit-or-default only.',
    },
    {
      label: 'Theming',
      note: 'All renderer colors go through `--pyreon-flow-*` CSS custom properties (25 vars) with light-mode fallbacks — set them on the container to re-skin; SVG colors are applied via `style` (never presentation attributes, where `var()` is invalid).',
    },
  ],
})
