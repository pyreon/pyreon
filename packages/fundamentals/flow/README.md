...
Renderer maps and structural renderer configuration must be statically discoverable by the native compiler. Inside a node, edge or connection-line renderer, SVG lowers natively: a `<path>` with any path data, and an inline `<svg>` whose shapes (`path`, `rect`, `circle`, `ellipse`, `line`, `polyline`, `polygon`, nested `<g>`) are drawn scaled by its `viewBox`, with `fill` / `stroke` / `stroke-width` inherited as in SVG. `<text>`, gradients and `transform` do not lower and are named in a warning. Plain `<div>` / `<p>` / `<span>` lower only where the layout provably matches the browser's: text-only content becomes a text run, and a `<div>` of block children becomes a flush-left stack. A `class`, a `style` or any inline-flow mix keeps the warning. Browser CSS classes have no native meaning; keep genuinely platform-specific presentation in `NativeIOS` / `NativeAndroid` branches. Supported inline node styles (`width`, `height`, `padding`, hex `background`/`background-color`, hex `border-color`, `border-width`, `border-radius`, `opacity`) and edge styles (`stroke`, `stroke-width`) lower directly.
