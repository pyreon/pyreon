...
describe('useImagePicker', () => {
  it('resolves an object URL for the picked file', async () => {
    const createObjectURL = vi
      ...
      .mockReturnValue('blob:pyreon/picked')
    ...
    await expect(useImagePicker().pick()).resolves.toBe('blob:pyreon/picked')
  ...
  it('resolves null when the picker is cancelled', async () => {
    ...
    await expect(useImagePicker().pick()).resolves.toBeNull()
    vi.restoreAllMocks()
  })
  ...
  it('settles once even if both change and cancel fire', async () => {
    // A browser firing both must not double-resolve or double-remove the node.
    withFileInput((input) => {
      Object.defineProperty(input, 'files', { value: [], configurable: true })
      input.dispatchEvent(new Event('change'))
  ...
  it('isAvailable() is true in a DOM environment', () => {
    expect(useImagePicker().isAvailable()).toBe(true)
  })
})
