...
describe('useSortable — per-item registration disposal (F3 leak)', () => {
  it('disposes the item cleanup on unmount and on re-register; no accumulation under churn', async () => {
    ...
    // Churn: 200 register/unmount cycles for the same key. Every
    // registration's cleanup must have been called exactly once — the
    // live set never accumulates dead registrations.
