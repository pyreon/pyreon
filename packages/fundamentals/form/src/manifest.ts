...
export default defineManifest({
  ...
  description:
    'Signal-based form management for Pyreon. Each field (`value`, `error`, `touched`, `dirty`) is its own `Signal<T>` so templates only re-run for the slice they read. First-class schema validation — pass a RAW Standard Schema (zod / valibot / arktype / `@pyreon/validate`) straight to `schema` with no adapter and no cast, or a `zodSchema` / `valibotSchema` / `arktypeSchema` typed adapter from `@pyreon/validation`. Dot-path field keys (`"address.city"`) declare first-class LEAF fields whose per-field-validator / schema errors route to the exact leaf — a REAL nested declarative schema (`z.object({ address: z.object({ city }) })`) is fed the nested value shape and its per-leaf-path errors auto-split to the leaf fields; a nested error with no leaf field routes to the nearest ancestor object field, and a key matching no field invalidates rather than being silently dropped. Per-field `validateOn: blur | change | submit`, async validators with optional `debounceMs` and version-based stale-result discarding, cross-field validation via `(value, allValues) => …`, type-aware input bindings (`register({ type: "checkbox" | "number" | "file" })`), runtime field registration (`registerField` / `unregisterField`) for data-driven forms, focus-first-error on failed submit (`focusOnError`, default on), reset ergonomics (`reset(values, { keep* })` + `resetField(field, { keepError })`), dynamic `useFieldArray` with stable keys for keyed rendering, and typed `useWatch` overloads for single / multi / all-field reactive watchers.',
  category: 'universal',
  multiplatform: {
    tier: 'service-backend',
    rationale:
      'useForm v2 + useFieldArray lower to PyreonForm/PyreonFieldArray; device-proven both platforms',
  },
