...
if (!(await form.validate())) form.focusFirstError()`,
  features: [
    ...
    'Dot-path leaf fields — `"address.city"` is a first-class field; errors route to the leaf (`nestValues`/`flattenValues` for nested payloads)',
    'Schema-error routing — leaf/ancestor field; orphan key invalidates (no silent drop)',
  ],
  api: [
    {
      name: 'useForm',
...
// h('input', { type: 'checkbox', ...form.register('remember', { type: 'checkbox' }) })`,
      mistakes: [
        ...
        'Expecting `values()` / `onSubmit` to be NESTED for dot-path fields — the value model is FLAT: a `"address.city"` field surfaces as `values()["address.city"]`, NOT `values().address.city`. Convert at the backend boundary with `nestValues(form.values())`. Pairing a `"address.city"` LEAF field with a NESTED-shaped schema (`z.object({ address: z.object({…}) })`) mis-routes — feed a flat-keyed schema (`z.object({ "address.city": … })`) or per-field validators for leaf routing',
      ],
...
const field = useField(form, 'email')`,
  ...
  gotchas: [
    ...
    {
      label: 'Raw Standard Schema — no adapter, no cast',
      note: '`schema` accepts a raw Standard Schema (zod ≥ 3.24 / valibot ≥ 1 / arktype ≥ 2 / `@pyreon/validate`\'s `s`) DIRECTLY — pass `z.object({...})` with no `zodSchema()` wrapper and no `as never` cast. It also still accepts a plain `SchemaValidateFn` and a `@pyreon/validation` typed adapter (reach for the adapter when you want the schema\'s field names checked against `TValues` at compile time). The `~standard` contract + bridge live in `@pyreon/validation`; form re-exports `ValidationError` / `ValidateFn` / `SchemaValidateFn` so the historical `import { ValidationError } from "@pyreon/form"` keeps working.',
    },
    {
      label: 'Dot-path leaf fields + nested schema-error routing',
      note: 'A field key containing a dot (`"address.city"`) declares a FIRST-CLASS leaf field, addressable exactly like a top-level one — `register("address.city")`, `useField("address.city")`, `setFieldValue`, `validators: { "address.city": … }`, and a schema/validator error keyed `"address.city"` route to it. The value model is FLAT: `values()` / `getValues()` / `onSubmit` return the flat keys (`{ "address.city": … }`) so field-name types stay honest; convert to a nested API payload with `nestValues(form.values())` and back with `flattenValues(serverData)`. Two error-routing modes: (1) per-field validators or a FLAT-keyed schema (`s.object({ "address.city": … })`) route to the exact LEAF field; (2) a NESTED schema whose value is a single OBJECT field (`{ address: { city } }` → field `address`) routes its dot-path error to that ANCESTOR object field. A schema-error key matching NO field (a shape mismatch, or the path-less `""` whole-form key) marks the form invalid, sets `submitError`, and dev-warns — never silently dropped. Declaring BOTH an object field `address` AND a leaf `address.city` dev-warns (the error would show on both). Field-level errors win over schema errors on the same field.',
    },
    {
      label: 'File inputs are value-less',
      note: '`register(field, { type: "file" })` returns a bag WITHOUT `value` or `checked` — a file input can\'t be value-controlled (`<input type="file" value=…>` is rejected). Its `onInput` writes the input\'s `FileList` (`target.files`) to the field, so `field.value()` is `FileList | null` — read `files?.[0]` for a single file, or iterate for a `multiple` input. The value flows into `values()` / `onSubmit` like any field.',
    },
    {
      label: 'Dynamic fields need explicit `registerField` — no auto-registration',
    ...
    {
      label: 'Focus-first-error is on by default',
      note: 'On a failed `handleSubmit`, focus moves to the first errored field (declaration order) that was bound via `register()` — accessible error recovery. Opt out with `focusOnError: false`. A field never passed through `register()` has no known id and is skipped. `form.focusFirstError()` is exposed for custom submit flows; both are SSR-safe no-ops on the server.',
    },
  ],
})
