...
- **`handleSubmit` does not reject when `onSubmit` throws** — the error lands in `form.submitError()` and the promise resolves, so a `<form onSubmit>` never produces an unhandled rejection. For a programmatic submit that should reject, call `await form.handleSubmit({ rethrow: true })`. Concurrent calls (double Enter) share one in-flight submit; `<Submit>` is disabled while validating as well as submitting.
