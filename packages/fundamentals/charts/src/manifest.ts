...
export default defineManifest({
  ...
  longExample: `import { Arc, Axis, Bar, Chart, Legend, Line, Rule, Tip, currency } from '@pyreon/charts'
...
rows.set([...rows(), { month: 'Apr', revenue: 190, target: 180 }])`,
  features: [
    ...
    '`@pyreon/charts/option` draws an ECharts option object on this engine; `@pyreon/charts/echarts` wraps the real ECharts library',
  ],
  api: [
    {
      name: 'Chart',
      ...
      example: `import { Axis, Bar, Chart, Legend, Line, Tip, currency } from '@pyreon/charts'
...
// -> '<svg xmlns="..." role="img" aria-labelledby=...>...</svg>'`,
    ...
    },
    {
      ...
      example: `import { PieChart, GaugeChart } from '@pyreon/charts'
import { signal } from '@pyreon/reactivity'
...
<GaugeChart value={() => cpu()} min={0} max={100} title="CPU" />`,
    ...
    },
    {
      ...
      example: `import { CandlestickChart } from '@pyreon/charts'
...
<CandlestickChart data={bars} open={(d: Bar) => d.o} high={(d: Bar) => d.h} low={(d: Bar) => d.l} close={(d: Bar) => d.c} x={(d: Bar) => d.day} />`,
    ...
    },
    {
      ...
      example: `import { HeatmapChart } from '@pyreon/charts'
...
/>`,
      mistakes: [
        'Missing style height on `<EChart>` — same as useChart, ECharts requires explicit container dimensions',
    ...
    },
    {
      ...
      summary:
        'From `@pyreon/charts/echarts`. Create a reactive ECharts instance. Options are passed as a function — signal reads inside are tracked and the chart updates automatically when any tracked signal changes. Lazy-loads the required ECharts modules on first render (zero bytes until mount). Returns `ref` (bind to a container div), `instance` (Signal<ECharts | null>), `loading` (Signal<boolean>), `error` (Signal<Error | null>), and `resize()`. Auto-resizes via ResizeObserver (`autoresize: false | { throttle }` to opt out/throttle) and disposes on unmount. `theme` accepts an accessor for reactive swaps; `initOptions` passes through to `core.init`; warm mounts (modules cached) are synchronous. `getCore()`/`connect()` are exported for `registerMap`/`registerTheme`/linked charts.',
...
// chart.instance() — raw ECharts instance for imperative API`,
  ...
  gotchas: [
    ...
    '`<EChart>` / `useChart` options must be a FUNCTION `() => EChartsOption`, not a plain object. Signal reads inside the function are tracked — changing any tracked signal reactively updates the chart.',
    {
      label: '<EChart>: lazy loading',
      note: 'ECharts modules are auto-detected from your options (series types, components) and dynamically imported. First render has an async loading phase — check `loading()`, or `<EChart>` handles it internally. Zero ECharts bytes in your initial bundle.',
    },
    {
      label: '<EChart>: manual entry',
      note: '`@pyreon/charts/echarts/manual` skips auto-detection — you register ECharts components yourself via `use()` for maximum tree-shaking control.',
    },
    {
      label: '<EChart>: events',
    ...
    {
      label: '<EChart>: theme is not reactive',
