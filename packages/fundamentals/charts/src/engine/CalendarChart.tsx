...
export function CalendarChart(props: CalendarChartProps): VNode {
  ...
  const layoutFor = (w: Double, hgt: Double): CalendarLayout =>
    layoutCalendar(props.start, props.end, { x: 4.0, y: 4.0, w: w - 8.0, h: hgt - 8.0 }, props.calendar)
  ...
  const handleClick = (ev: MouseEvent): void => {
    ...
    cb(hitCalendar(layoutFor(w, hgt), ev.clientX - r.left, ev.clientY - r.top))
  }
  ...
  const table = (): VNode => {
    ...
    return h(
      ...
      h('table', null,
        h('caption', null, props.title ?? 'Calendar data'),
