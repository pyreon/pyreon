...
describe('renderHeatChart', () => {
  it('draws one cell per datum, then a label per row and per column', () => {
    ...
    expect(rects(cmds).length).toBe(4)
    const rowLabels = texts(cmds).filter((c) => c.align === 'end').map((c) => c.text)
    const colLabels = texts(cmds).filter((c) => c.align === 'middle').map((c) => c.text)
  ...
  it('custom stops reach the cells', () => {
    ...
    const fills = rects(cmds).map((c) => c.fill)
