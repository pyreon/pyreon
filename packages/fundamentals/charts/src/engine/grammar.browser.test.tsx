...
import { Arc, Bar, Cell, Line, Chart, Tooltip, chartThemes } from '../engine'
...
describe('<Chart> grammar', () => {
  it('paints bar and line marks with the theme palette, and repaints in place on a data flip', async () => {
    const { container, unmount } = mountInBrowser(
      h(Chart<Row>, { data: () => rows(), x: 'q', width: 240, height: 160, animate: false, updateAnimation: false, showGrid: false }, h(Bar<Row>, { y: 'v' }), h(Line<Row>, { y: 'w', width: 3 }), h(Tooltip, {})),
    )
    await flush()
  ...
  it('<Chart onSelect> over <Cell> reports an INDEX, not the heatmap host\'s cell object', async () => {
    // A family host's own `onSelect` is shaped per family; `<Chart onSelect>`
    // promises the drawn item's index on every target, so the grammar routes
    // it to the host's `onSelectIndex`.
    interface Obs { day: string; hour: string; load: number }
    const obs: Obs[] = [{ day: 'Mon', hour: '9', load: 3 }, { day: 'Mon', hour: '10', load: 5 }]
    const picked: unknown[] = []
    const { container, unmount } = mountInBrowser(
      h(Chart<Obs>, { data: obs, width: 240, height: 160, animate: false, onSelect: (i: number) => picked.push(i) }, h(Cell<Obs>, { x: 'hour', y: 'day', value: 'load' })),
    )
    await flush()
    const canvas = container.querySelector('canvas')!
    const box = canvas.getBoundingClientRect()
    canvas.dispatchEvent(new MouseEvent('click', { clientX: box.left + box.width * 0.75, clientY: box.top + box.height * 0.5, bubbles: true }))
    expect(picked.length).toBe(1)
    expect(typeof picked[0]).toBe('number')
    unmount()
  })
})
