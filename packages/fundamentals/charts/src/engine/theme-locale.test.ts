...
describe('theme registry', () => {
  it('ships light and dark; registers, lists and resolves; unknown names warn and fall back', () => {
    ...
    expect(dark.palette![0]).toBe('#7b9bff')
    expect(dark.background).toBe('#141821')
    expect(dark.chartTheme.label).toBe('#9aa5b5')
  ...
  it('compileOption applies a theme: palette to series without a colour, text/axis/grid to the spec, background to the svg', () => {
    ...
    expect(c.spec.series[0]!.color).toBe('#7b9bff')
    expect(c.spec.series[1]!.color).toBe('#ff8f7e')
    expect(c.spec.theme.label).toBe('#9aa5b5')
    expect(c.background).toBe('#141821')
    ...
    expect(svg.indexOf('#141821')).toBeLessThan(svg.indexOf('#7b9bff'))
    expect(optionToSvg(opt, { width: 300, height: 200 })).not.toContain('#141821')
