...
describe('buildChartHostHtml — the host page cannot be escaped through its options', () => {
  ...
  it('a `background` cannot smuggle a quote into the sheet', () => {
    const html = buildChartHostHtml({ background: 'url("x")' })
  ...
  it('an ordinary colour still reaches the sheet unchanged', () => {
    ...
    expect(buildChartHostHtml({ background: '#0b0d12' })).toContain('background:#0b0d12')
    expect(buildChartHostHtml({ background: 'rgb(11 13 18 / 80%)' })).toContain('rgb(11 13 18 / 80%)')
  })

  it('an inlined engine script cannot close the <script> element', () => {
    const html = buildChartHostHtml({ echartsScript: 'var a = "</script><img src=x onerror=alert(1)>"' })
  ...
  it('an inlined engine script cannot open the double-escaped state', () => {
    ...
    const html = buildChartHostHtml({ echartsScript: 'var a = "<!--<script>"' })
    expect(html).not.toContain('"<!--<script')
    expect(html).toContain('<!\\--')
  })

  it('a `echartsSrc` cannot break out of its attribute', () => {
    const html = buildChartHostHtml({ echartsSrc: 'x.js" onload="alert(1)' })
