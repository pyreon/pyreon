...
import { buildRichTextHostHtml } from '../webview'

describe('buildRichTextHostHtml — the host page cannot be escaped through its options', () => {
  ...
  it('a `background` cannot smuggle a quote into the sheet', () => {
    const html = buildRichTextHostHtml({ background: 'url("x")' })
  ...
  it('an ordinary colour still reaches the sheet unchanged', () => {
    ...
    expect(buildRichTextHostHtml({ background: '#0b0d12' })).toContain('background:#0b0d12')
    expect(buildRichTextHostHtml({ background: 'rgb(11 13 18 / 80%)' })).toContain('rgb(11 13 18 / 80%)')
  })

  it('an inlined engine script cannot close the <script> element', () => {
    const html = buildRichTextHostHtml({ tiptapScript: 'var a = "</script><img src=x onerror=alert(1)>"' })
  ...
  it('an inlined engine script cannot open the double-escaped state', () => {
    ...
    const html = buildRichTextHostHtml({ tiptapScript: 'var a = "<!--<script>"' })
    expect(html).not.toContain('"<!--<script')
    expect(html).toContain('<!\\--')
  })

  it('a `tiptapSrc` cannot break out of its attribute', () => {
    const html = buildRichTextHostHtml({ tiptapSrc: 'x.js" onload="alert(1)' })
