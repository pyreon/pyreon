...
describe('Discord renderer', () => {
  it('renders document as embed JSON', async () => {
    const doc = Document({
      title: 'Report',
      children: [Heading({ children: 'Report' }), Text({ children: 'Summary text.' })],
    })
...
describe('builder — messaging formats', () => {
  it('toSlack produces valid JSON', async () => {
    const result = await createDocument().heading('Alert').text('Something happened.').toSlack()
  ...
  it('toTelegram produces HTML string', async () => {
    const result = await createDocument().heading('Notice').text('Please read.').toTelegram()
    expect(result).toContain('<b>Notice</b>')
  })

  it('toSvg produces SVG string', async () => {
    const result = await createDocument().heading('Chart Title').toSvg()
...
describe('builder — add and section', () => {
  ...
  it('add() accepts an array of nodes', () => {
    const doc = createDocument().add([Text({ children: 'A' }), Text({ children: 'B' })])
    const node = doc.build()
    expect(node.type).toBe('document')
  })

  it('section() wraps children in a Section node', async () => {
    const doc = createDocument().section([Text({ children: 'inside section' })])
...
describe('document metadata pass-through (PR #197)', () => {
  ...
  it(
    'PDF renderer writes metadata to the /Info dictionary (binary verified)',
    { timeout: 60_000 },
    async () => {
  ...
  it(
    'DOCX renderer writes metadata to docProps/core.xml (binary verified)',
    { timeout: 30_000 },
    async () => {
  ...
  it(
    'DOCX renderer omits empty metadata elements when fields are missing',
    { timeout: 30_000 },
    async () => {
