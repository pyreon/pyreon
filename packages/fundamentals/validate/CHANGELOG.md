...
- A pure-JIT schema installs `parse` as an own property after its first parse. (da78513)
...
- Make the pure-JIT parse seam small enough to inline. `parse()` and the Standard (0a8824d)
  ...
  in the PR.
- JIT parse: an inline array whose element subtree is pure inline (no fallback (a7d1d44)
