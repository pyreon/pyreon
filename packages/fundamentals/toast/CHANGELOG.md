...
- The auto-dismiss countdown is held by three independent sources, and a hidden tab no longer burns it (ac7699c)
...
- fix(toast): a description added later via `toast.update()` now renders (the `<Show>` gate was frozen) (83b1853)
...
- fix(toast): a toast created WHILE the container is hover/focus-paused no longer auto-dismisses under the cursor (e87a031)
