...
describe('renderVirtualModule', () => {
  it('re-exports the built-in components even when no user components are scanned', () => {
    ...
    // PR-K (H2) + PR-M (M6+M7+M8) + PR-F (H7 — Image) + Example — 14 built-ins.
    expect(out).toContain(
      'export const __components_meta__ = ["APICard","Callout","CodeBlock","CodeGroup","CompatMatrix","Details","Example","Image","Math","Mermaid","PackageBadge","Playground","PropTable","Tabs"]',
    )
  })
  ...
  it('emits per-component imports + a default __components map alongside built-ins', () => {
    ...
    // Built-ins re-exported via aliased imports __b0..__b13 in
    ...
    expect(out).toContain("import { Example as __b6 } from '@pyreon/zero-content'")
    expect(out).toContain("import { Image as __b7 } from '@pyreon/zero-content'")
    expect(out).toContain("import { Math as __b8 } from '@pyreon/zero-content'")
    expect(out).toContain("import { Mermaid as __b9 } from '@pyreon/zero-content'")
    expect(out).toContain("import { Tabs as __b13 } from '@pyreon/zero-content'")
    // User scan continues at __c14 / __c15 (idx starts after 14 built-ins).
    expect(out).toContain('import { Foo as __c14 } from "/abs/Foo.tsx"')
    expect(out).toContain('import __c15 from "/abs/Bar.tsx"')
    ...
    expect(out).toContain('export const Callout = __components.Callout')
    expect(out).toContain(
      'export const __components_meta__ = ["APICard","Callout","CodeBlock","CodeGroup","CompatMatrix","Details","Example","Image","Math","Mermaid","PackageBadge","Playground","PropTable","Tabs","Foo","Bar"]',
    )
  })
  ...
  it('user-scanned components override built-ins of the same name (escape hatch)', () => {
    ...
    // User Callout IS imported. With 13 built-ins remaining
    // (Callout removed from the 14-entry list), the user import
    // lands at idx 13 (`__c13`).
    expect(out).toContain('import __c13 from "/abs/Callout.tsx"')
    expect(out).toContain('export const Callout = __components.Callout')
