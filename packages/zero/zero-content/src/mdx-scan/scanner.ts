...
export function renderVirtualModule(scan: ScanResult): string {
  if (scan.components.length === 0) {
    return `// Auto-generated by @pyreon/zero-content. No src/mdx/ components found.\nexport {}\n`
  }
...
const __components = { ${names.join(', ')} } as const

export const __components_meta__ = ${JSON.stringify(
...
${scan.components
  .map((c) => `export const ${c.name} = __components.${c.name}`)
  .join('\n')}
`
}
