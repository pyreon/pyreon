...
describe('buildScaffold — native runtime delivery wiring (local-proof-found fixes)', () => {
  ...
  it('iOS lists the staged co-location group, and the build stages into it', () => {
    // The Android half of this has always worked because Gradle reads its
    // source roots at CONFIGURE time. project.yml is static YAML generated by
    // hand, so a co-located package installed after `xcodegen generate` can
    // only be reached through a path that is constant in the spec and
    // refreshed on disk — hence a staging group plus a build-time restage.
    // Without both halves, `useForm` builds on Android and fails on iOS with
    // `cannot find 'PyreonForm' in scope`.
    const yml = get('ios/project.yml')
