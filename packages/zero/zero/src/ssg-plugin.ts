...
/**
 * `_redirects` and `_headers` are LINE-ORIENTED config formats: one rule per
 * line, and the host parses whatever it finds. So a line terminator inside a
 * value is not cosmetic, it is a new rule.
 *
 * `to` comes from the argument to a `redirect()` thrown by a route loader at
 * build time, and the idiomatic CMS shape is
 * `if (post.redirectTo) throw redirect(post.redirectTo, 301)`. A value of
 ...
 *   # 301
 *
 * — a complete, valid Netlify / Cloudflare Pages rule, with the orphaned
 * status neutralised by the trailing `#`: every path on the two hosts that
 * read the file now redirects to the attacker.
 *
 * Stripped rather than escaped: these formats have no escape syntax, and a URL
 * path containing a raw line terminator is already malformed.
 */
 ...
 * a deploy log.
 */
export function renderNetlifyRedirects(entries: RedirectEntry[]): string {
  if (entries.length === 0) return ''
  const lines = ['# Auto-generated by @pyreon/zero SSG. Do not edit.']
  for (const e of entries) {
    lines.push(`${oneLine(e.from)} ${oneLine(e.to)} ${e.status}`)
  }
  return `${lines.join('\n')}\n`
}
