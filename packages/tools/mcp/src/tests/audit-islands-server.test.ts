...
describe('MCP server — audit_islands tool', () => {
  it('renders the audit header + summary line', async () => {
  ...
  it('returns the green-light message when the audit is clean', async () => {
    const { client, close } = await newClient()
    try {
      const text = await callTool(client, 'audit_islands', {})
  ...
  it('emits machine-readable JSON when json=true', async () => {
    const { client, close } = await newClient()
    try {
