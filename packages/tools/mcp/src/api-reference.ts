...
// <For each={table.rows()} by={(r) => r.id}>{(r) => <Text>{r.name}</Text>}</For>`,
    notes: 'The dependency-free, MULTIPLATFORM-portable table-state core — the alternative to `useTable` (which binds `@tanstack/table-core` and is web-only-rich). Pure signal logic (no DOM, no TanStack), so the SAME source drives sort / filter / paginate / row-selection on web AND, via co-located native Swift/Kotlin ports (`PyreonTableState`, behaviour-identical + compile-and-run verified), on iOS/Android — you render `rows()` with native `<For>` (tables ARE native: SwiftUI List / Compose LazyColumn), no WebView. `data` is an ACCESSOR so a `signal()`/`computed()` source stays reactive; `rows()` re-derives filtered → sorted → paginated. `toggleSort` cycles none → asc → desc → none; the filter is case-insensitive across every column (override with `filterFn`); `pageSize: 0` disables pagination. A `createTableState`-only import tree-shakes TanStack out entirely. See also: useTable.',
