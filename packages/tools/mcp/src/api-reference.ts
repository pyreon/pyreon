...
- Putting web-visible content in \`<NativeIOS>\` / \`<NativeAndroid>\` — both render NOTHING on web (they are no-ops there); only \`<Web>\` content reaches the browser.`,
  ...
  'primitives/defineNativeModule / useNativeModule': {
    signature: 'defineNativeModule<T>(name: string, webImpl: T) => T · useNativeModule<T>(name: string) => T · hasNativeModule(name: string) => boolean',
    example: `type Bluetooth = { connect(id: string): Promise<boolean> }
...
- Arming two stacked traps with the default lifetime-\`active\` — the stack then follows MOUNT order, not open order, so the wrong trap can own focus; tie each trap to its open state (\`active: () => isOpen()\`)`,
  ...
  'hooks/useInertOthers': {
    signature: '(getEl: () => HTMLElement | null, options?: { active?: boolean | (() => boolean) } | boolean | (() => boolean)) => void',
...
// <For each={table.rows()} by={(r) => r.id}>{(r) => <Text>{r.name}</Text>}</For>`,
    notes: 'The dependency-free, MULTIPLATFORM-portable table-state core — the alternative to `useTable` (which binds `@tanstack/table-core` and is web-only-rich). Pure signal logic (no DOM, no TanStack), so the SAME source drives sort / filter / paginate / row-selection on web AND, via native Swift/Kotlin ports, on iOS/Android — you render `rows()` with native `<For>` (tables ARE native: SwiftUI List / Compose LazyColumn), no WebView. `data` is an ACCESSOR so a `signal()`/`computed()` source stays reactive; `rows()` re-derives filtered → sorted → paginated. `toggleSort` cycles none → asc → desc → none; the filter is case-insensitive across every column (override with `filterFn`); `pageSize: 0` disables pagination. A `createTableState`-only import tree-shakes TanStack out entirely. See also: useTable.',
    mistakes: `- Passing \`data\` as a plain array instead of an accessor \`() => data()\` — the table then never re-derives when the source signal changes.
...
// → only components tagged \`form\``,
    notes: 'Serve the VERIFIED component catalog `atlas scan` writes (`atlas-catalog.json`) — every component with its real props, allowed values and scenario counts, read from your source rather than guessed. Filter with `tag`. Each line states how many scenarios are verified, failing and UNVERIFIED, because most Atlas checks are still stubs and a catalogued component is not automatically a checked one. With no catalog present the tool returns instructions to run `atlas scan` instead of an invented answer. See also: get_atlas_component, get_api.',
    mistakes: `- Treating a catalogued component as a verified one — the counts on each line are the point. \`unverified\` means nothing examined that scenario; it is not a pass.
...
- Putting layout props in a styler \`.theme()\` callback — \`direction\` / \`alignX\` / \`alignY\` / \`gap\` / \`block\` are Element ATTRS, not CSS; theme is for colors / spacing / borders. Re-declaring \`display\`/\`flex-direction\`/\`align-items\` in a theme fights the wrapper CSS Element already emits (and never reaches the button/fieldset/legend flex-fix inner layer at all)
- Setting \`direction\`/\`alignX\`/\`alignY\` on a SIMPLE element (no before/after slots) and wondering why nothing moves — the simple path reads the \`content*\` trio (\`contentDirection\` / \`contentAlignX\` / \`contentAlignY\`); the bare trio governs the SLOT axis on compound elements
- Treating \`alignX\`/\`alignY\` as main/cross axis — they are AXIS-FIXED (X is always horizontal, Y vertical) regardless of direction; \`contentAlignX: "spaceBetween"\` on an \`inline\` row is justify-content, on \`rows\` it is align-items
- Expecting an app-root Element to fill the viewport — Element renders \`inline-flex\` by default and shrink-wraps; pass \`block\` for a full-width flex root
