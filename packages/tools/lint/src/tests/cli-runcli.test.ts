...
// `runCli` is async now — the default lint path uses a worker pool — so the
// spy has to stay installed until the promise settles, not just until the
// call returns.
...
describe('runCli — exit-code contract', () => {
  it('--help / --version / --list return 0 without exiting', async () => {
    expect(await quiet(() => runCli(['--help']))).toBe(0)
    expect(await quiet(() => runCli(['--version']))).toBe(0)
    expect(await quiet(() => runCli(['--list']))).toBe(0)
  })

  it('a clean file returns 0', async () => {
    const root = fixture('clean.ts', 'export const answer = 42\n')
    expect(await quiet(() => runCli([root, '--format', 'json']))).toBe(0)
  })

  it('a file with an error-level violation returns 1', async () => {
  ...
  it('--quiet on a clean file still returns 0', async () => {
    const root = fixture('clean2.ts', 'export const x = 1\n')
    expect(await quiet(() => runCli([root, '--quiet']))).toBe(0)
  })
})
