...
export default defineManifest({
  ...
  tagline:
    'Pyreon-specific linter — 98 rules across 19 categories, config files, watch mode, AST cache, CLI + LSP',
  description:
    'Pyreon-specific lint rules powered by `oxc-parser`. Covers reactivity (15), JSX (11), lifecycle (6), performance (5), SSR (5), architecture (11), store (3), form (4), styling (5), hooks (3), accessibility (3), router (5), SSG (3), frontend (12), query (2), rx (1), i18n (1), storage (1), http (2) — 98 rules total. Programmatic API (`lint`, `lintFile`), CLI (`pyreon-lint`), watch mode (fs.watch + 100ms debounce + AstCache), LSP server, and `.pyreonlintrc.json` config with per-rule options via ESLint-style tuple form. Two independent reasons a rule can be off in a shipped preset. **Opt-in best-practice rules** (`meta.optIn` — the `frontend`/`query`/`rx`/`i18n`/`storage` categories plus the `form`/`router` opt-in rules): off in `recommended`/`strict`/`app`/`lib`, enabled wholesale by the `best-practices` preset or per-rule config. **Monorepo-scoped rules** (`meta.scope: \'monorepo\'` — `no-circular-import`, `no-cross-layer-import`, `no-error-without-prefix`, `no-querySelector-cast-in-test`, `require-browser-smoke-test`, `vitest-config-uses-shared`): these encode the Pyreon repository itself — its layer order, its private internal packages, its `[Pyreon]` error prefix — so EVERY consumer preset forces them off, `best-practices` included, and the Pyreon repo re-enables them by id in its own `.pyreonlintrc.json`. Library-scoped opt-in rules auto-gate on the project’s `package.json` dependencies (a project that doesn’t use `@pyreon/query` never sees query rules). Notable rules: `pyreon/no-process-dev-gate` (auto-fixable), `pyreon/query-options-as-function` (auto-fixable — wraps the options object literal in `() => (...)`; also a proactive MCP `validate` detector), `pyreon/require-img-alt` / `pyreon/img-requires-dimensions` / `pyreon/no-discarded-optimize-fields` (a11y + CLS — the last flags a raw `<img src={x.src}>` that discards a `?optimize` descriptor), `pyreon/heading-order` (a11y — flags a skipped heading level), `pyreon/color-contrast` (a11y — literal-hex contrast pairs), `pyreon/i18n-prefer-trans-for-rich-jsx`, `pyreon/prefer-typed-search-params`.',
  category: 'server',
  multiplatform: {
    tier: 'web-only',
    rationale:
