...
export default defineManifest({
  ...
  tagline:
    'Pyreon-specific linter — 83 rules across 18 categories, config files, watch mode, AST cache, CLI + LSP',
  description:
    'Pyreon-specific lint rules powered by `oxc-parser`. Covers reactivity (14), JSX (11), lifecycle (6), performance (6), SSR (3), architecture (7), store (3), form (4), styling (4), hooks (3), accessibility (3), router (5), SSG (3), frontend (7), query (1), rx (1), i18n (1), storage (1) — 83 rules total. Programmatic API (`lint`, `lintFile`), CLI (`pyreon-lint`), watch mode (fs.watch + 100ms debounce + AstCache), LSP server, and `.pyreonlintrc.json` config with per-rule options via ESLint-style tuple form. The `frontend`/`query`/`rx`/`i18n`/`storage` categories + the `form`/`router` opt-in rules are **opt-in best-practice rules** (`meta.optIn`): off in the `recommended`/`strict`/`app`/`lib` presets, enabled wholesale by the `best-practices` preset or per-rule config. Library-scoped opt-in rules auto-gate on the project’s `package.json` dependencies (a project that doesn’t use `@pyreon/query` never sees query rules). Notable rules: `pyreon/no-process-dev-gate` (auto-fixable), `pyreon/query-options-as-function` (auto-fixable — wraps the options object literal in `() => (...)`; also a proactive MCP `validate` detector), `pyreon/require-img-alt` / `pyreon/img-requires-dimensions` (a11y + CLS), `pyreon/i18n-prefer-trans-for-rich-jsx`, `pyreon/prefer-typed-search-params`.',
  category: 'server',
  features: [
    '83 rules across 18 categories',
...
//   pyreon-lint --list                                     # list all 83 rules
  ...
  api: [
    {
      ...
      summary:
        '83 rules across 18 categories. Auto-loads `.pyreonlintrc.json`. Presets: `recommended`, `strict`, `app`, `lib`. Per-rule options via tuple form in config (`["error", { exemptPaths: [...] }]`) or `ruleOptionsOverrides`. Wrong-typed options surface on `result.configDiagnostics`. Uses `oxc-parser` with AST caching.',
