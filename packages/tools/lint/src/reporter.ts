...
export function formatText(result: LintResult): string {
  ...
  if (result.counts.error > 0) {
    parts.push(
      `${SEVERITY_COLORS.error}${result.counts.error} error${result.counts.error !== 1 ? "s" : ""}${RESET}`,
    )
  }
  if (result.counts.warn > 0) {
    parts.push(
      `${SEVERITY_COLORS.warn}${result.counts.warn} warning${result.counts.warn !== 1 ? "s" : ""}${RESET}`,
    )
  }
  ...
  if (parts.length === 0) {
    lines.push(
      `${BOLD}\u2714 No problems found${RESET} ${DIM}(${result.fileCount} file${result.fileCount !== 1 ? "s" : ""} in ${result.durationMs}ms)${RESET}`,
    )
  } else {
