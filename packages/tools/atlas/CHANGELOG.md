...
- Every component renders on the workbench, and the scan says so when one does not. (9cb0e51)
...
- Type the workbench and observatory theme through rocketstyle's `withTheme<Tokens>()` instead of casts. (a92fd69)
...
- Workbench UX and DX fixes from the 2026-09 audit. (a0c7732)
...
- `atlas` refuses what it used to get wrong in silence, and `atlas build` ships a production site a fraction of the size. (0ea195a)
...
- Correctness fixes across discovery, verification and the dev server: (3e0241f)
...
- Fix an ambiguous component name resolving silently to one of its siblings. (edddc41)
...
- Keep optional render props absent in generated catalogs instead of replacing them with action-log callbacks, preventing components such as Combobox from rendering an empty preview. Also keep Docs scenario navigation from being miscompiled into a chained call that throws instead of selecting the scenario. (ac8d11a)
