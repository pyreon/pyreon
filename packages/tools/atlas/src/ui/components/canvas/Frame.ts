...
export const Frame = el
  ...
  .attrs({ tag: 'div' })
  .theme((t: T) => ({ display: 'flex', flexDirection: 'column', alignItems: 'stretch', borderRadius: t.radius.stage, overflow: 'hidden', boxShadow: '0 20px 50px -24px rgba(15,18,30,.35)', border: t.hairline, background: t.surface, transition: 'width .16s ease' }))
  .sizes(() => ({
    vFull: {},
