...
atlas: 2 failing scenario(s):
  ✗ button--empty
      ...
      seeAlso: ['atlas verify', 'atlas verify-browser', 'createAtlas'],
    },
    {
      name: 'atlas check',
      kind: 'function',
...
Input: 1 problem(s):
  · \`label\` is required and was not supplied  # omitted args → missing-required findings only`,
      ...
      seeAlso: ['atlas scan', 'atlas verify'],
    },
    {
      name: 'atlas verify',
      kind: 'function',
...
1 failing · 14 verified · 0 unverified`,
      ...
      seeAlso: ['atlas scan', 'atlas verify-browser', 'VerifyFinding'],
    },
    {
      name: 'VerifyFinding',
      kind: 'type',
  ...
  fix: 'Give "label" a non-empty value, or an aria-label if the text is decorative.',
      ...
      seeAlso: ['atlas verify', 'atlas scan'],
    },
    {
      name: 'atlas dev',
      kind: 'function',
...
const graph = await createAtlas(options).build()`,
      ...
      seeAlso: ['createAtlas', 'AtlasConfig.projects (monorepo — one site, several packages)'],
    },
    {
      name: 'atlas init',
      kind: 'function',
