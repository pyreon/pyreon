...
- `pyreon doctor`'s distribution gate now names an unbuilt checkout correctly when (1dcfb39)
...
- `pyreon doctor`'s distribution gate now distinguishes "this checkout was never (a1ccd9b)
...
- Three confirmed pre-release defects in the tools layer, each a case of a guard (e5567e3)
...
- `doctor --only doc-claims` went blind on JSON claim files. (5af46e1)
...
- `pyreon plain --json` now emits JSON on the empty branch. (3383952)
...
- `pyreon doctor` gains a `dependency-fabric` gate, reporting the workspace's own dependency health from `@pyreon/loom`. (2908ca9)
...
- `pyreon add` gains recipes for the three tool packages — `@pyreon/loom`, (a961be8)
...
- `@pyreon/loom` is now published: the monorepo dependency observatory. (19ee507)
  ...
  front door.
- `pyreon atlas [args]` — the Atlas component workbench from the CLI front door. (e6ff11f)
