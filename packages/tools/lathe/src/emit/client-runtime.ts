...
export function runtimePreamble(): string[] {
  return [
    ...
    ' */',
    'export interface Endpoint<T, I = EndpointInput, E = undefined> {',
    '  (...args: CallArgs<I & EndpointCallOptions>): Promise<T>',
    '  /** The declared error-body schemas — see `EndpointError`. */',
    '  readonly errors: E',
    ...
    '',
    '/**',
    ' * The `LatheHttpError` an endpoint declared with `errors: E` rejects with —',
    ' * `@pyreon/http`\'s `HttpErrorOf`, over this client\'s error class. `matched`',
    ' * is the `errors` key the body validated against and narrows `body` to it.',
    ' */',
    'export type HttpErrorOf<E> = LatheHttpError &',
    '  (',
    '    | {',
    '        [K in keyof E & (string | number)]: {',
    '          readonly matched: `${K}`',
    '          readonly status: StatusOfKey<K>',
    '          readonly body: Infer<E[K]>',
    '        }',
    '      }[keyof E & (string | number)]',
    '    | { readonly matched: undefined }',
    '  )',
    '',
    '/** Anything else a call rejects with: a network failure, a cancellation, a schema mismatch. */',
    ...
    '  : undefined',
    '',
    '/** Everything a call to `EP` can reject with — the error type of its hooks. */',
...
export function runtimeValidate(): string[] {
  return [
    ...
    '',
    '/**',
    ' * Decorate a rejection with its declared error schema — `@pyreon/http`\'s',
    ' * behaviour: exact status, then its range (`4XX`), then `default`. A body',
    ' * that does not match stays the same HTTP failure, `matched` undefined;',
    ' * replacing it with a schema error would hide what the server said.',
    ' */',
    'async function matchError(',
    '  err: LatheHttpError,',
    '  errors: Readonly<Record<string, unknown>> | undefined,',
    '  mode: ValidateMode,',
    '): Promise<LatheHttpError> {',
    '  if (!errors) return err',
    '  const exact = String(err.status)',
    '  const range = `${exact.charAt(0)}XX`',
    '  const key = [exact, range, range.toLowerCase(), "default"].find((k) => errors[k] !== undefined)',
    '  if (key === undefined) return err',
    '  const schema = errors[key]',
    '  if (mode === "off" || !isStandardSchema(schema)) return new LatheHttpError(err.status, err.url, err.body, key)',
    '  const result = await schema["~standard"].validate(err.body)',
    '  if (result.issues && result.issues.length > 0) {',
    '    if (mode === "warn") {',
    '      console.warn(`[lathe] the ${err.status} body from ${err.url} did not match its declared \\`${key}\\` error schema — \\`matched\\` is undefined.`)',
    '    }',
    '    return err',
    '  }',
    '  return new LatheHttpError(err.status, err.url, result.value, key)',
    '}',
  ]
}
...
export function runtimeError(): string[] {
  return [
    ...
    '    /**',
    '     * The `errors` key `body` validated against (`"404"`, `"4XX"`,',
    '     * `"default"`), so it has that schema\'s type; `undefined` otherwise.',
    '     */',
...
export function runtimeEndpoint(
  ...
  return [
    ...
    '    config?: {',
    '      response?: V',
    '      /** Error-body schemas by status (`404`), range (`"4XX"`) or `"default"`. */',
    '      errors?: E',
    '      responseType?: K',
