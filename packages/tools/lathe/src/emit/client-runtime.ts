...
export function runtimeTransport(): string[] {
  return [
    ...
    '  json?: unknown',
    '  /** A non-JSON body, already encoded (`URLSearchParams`, `FormData`, …). */',
    '  body?: unknown',
    '}',
    '',
    '/**',
...
export function runtimeEndpoint(client: ClientName, baseUrl: string): string[] {
  return [
    ...
    '      }',
    '      // Declared headers, then per-call ones (per-call wins); nullish values',
    '      // are omitted rather than sent as the text "undefined".',
    ...
    '          headers,',
    '          json: args?.json,',
    '          ...(args?.json === undefined && payload !== undefined ? { body: payload } : {}),',
    '        })',
    '        if (answer !== null) return validateResponse(config?.response, answer.json)',
    '      }',
    '      const body = await send(method, url, headers, payload, args?.signal)',
