...
export class SourceFile {
  ...
  doc(...parts: (string | undefined)[]): this {
    // An EMPTY string is a paragraph break; `undefined` (an absent summary, a
    // field with no description) is dropped. Breaks collapse, and none lead or
    // trail, so optional parts never leave a gap or a dangling ` *`.
    const kept = parts.filter((p): p is string => typeof p === 'string')
    if (!kept.some((p) => p.trim().length > 0)) return this
    ...
    const lines = kept.map(safeBlockComment).flatMap((p) => p.split('\n'))
    const flat: string[] = []
    for (const l of lines) {
      const blank = l.trim().length === 0
      if (blank && (flat.length === 0 || flat[flat.length - 1] === '')) continue
      flat.push(blank ? '' : l)
    }
    while (flat[flat.length - 1] === '') flat.pop()
    if (flat.length === 1) {
...
export function banner(specTitle: string, specVersion: string): string {
  return [
    '/* eslint-disable */',
    `// @generated by @pyreon/lathe from ${safeLineComment(specTitle)} ${safeLineComment(specVersion)}.`,
