 ...
 * the `el`/`txt` bases, tokens from `./theme` bound via `withTheme<LoomTokens>()`.
 *
...
export const SearchGlyph = txt
  .attrs({ tag: 'span' })
  .theme((t) => ({ color: t.faint, fontSize: '16px', flex: 'none', lineHeight: '1' }))
...
export const CyclesDot = el
  .attrs({ tag: 'span' })
  .variants((t) => ({ danger: { background: t.danger }, ok: { background: t.ok } }))
...
export const SearchMore = txt
  .attrs({ tag: 'div' })
  .theme((t) => ({ fontFamily: MONO, fontSize: '10.5px', color: t.faint, padding: '8px 12px' }))
