...
data class FrameSpec(var left: FrameLength, var top: FrameLength, var right: FrameLength, var bottom: FrameLength, var width: FrameLength, var height: FrameLength)
...
data class PictorialBar(var bar: PyreonChartRect, var horizontal: Boolean, var symbol: String, var repeat: Boolean, var fill: String)
...
fun computeLayout(cfg: LayoutConfig, measure: (String, Double) -> Double): PlotLayout {
    ...
    val bottom = (if (cfg.showXAxis) cfg.fontSize + labelGap + slantH + tickLen else 0.0) + xTitleH
...
fun stackLevels(seriesValues: List<List<Double>>): StackLevels {
...
fun layoutStackedBars(seriesValues: List<List<Double>>, plot: PyreonChartRect, yDomain: Domain, gapRatio: Double): List<StackSegment> = layoutStackLevels(stackLevels(seriesValues), seriesValues, plot, yDomain, gapRatio)

fun layoutStackedBarsH(seriesValues: List<List<Double>>, plot: PyreonChartRect, vDomain: Domain, gapRatio: Double): List<StackSegment> = layoutStackLevelsH(stackLevels(seriesValues), seriesValues, plot, vDomain, gapRatio)
...
fun stackedExtent(seriesValues: List<List<Double>>): Domain = stackLevelsExtent(stackLevels(seriesValues))
...
fun symbolCommand(cell: PyreonChartRect, b: PictorialBar): PyreonDrawCmd {
    if (b.symbol == "circle") {
      val r = ((if (cell.w < cell.h) cell.w else cell.h)).toDouble() / (2.0).toDouble()
      return PyreonDrawCmd(kind = "circle", fill = b.fill, center = PyreonChartPt(x = cell.x + (cell.w).toDouble() / (2.0).toDouble(), y = cell.y + (cell.h).toDouble() / (2.0).toDouble()), radius = r)
    }
...
fun pictorialCommands(b: PictorialBar): List<PyreonDrawCmd> {
    ...
    if (!b.repeat) {
      out.add(symbolCommand(cellAt(0.0, barLength), b))
      return out
    }
    if (unit <= 0.0) {
      return out
    ...
    for (k in 0 until 400) {
      ...
      out.add(symbolCommand(cellAt(from, unit), b))
      from = from + unit
    }
    return out
  }
...
fun seriesDomain(s: Series, spec: ChartSpec, yDomain: Domain, y2Domain: Domain): Domain = if (seriesOnRightAxis(s, spec)) y2Domain else yDomain
...
fun rawExtentOver(series: List<Series>): Domain {
    ...
    for (s in series) {
      ...
      if (s.kind == "waterfall") {
        ...
        span = extendSpan(span, listOf(we.min, we.max))
        continue
      }
      span = extendSpan(span, s.values)
      span = extendSpan(span, (s.values2 ?: listOf()))
      span = extendSpan(span, (s.errLow ?: listOf()))
      span = extendSpan(span, (s.errHigh ?: listOf()))
    }
    val e = if (span.seen) Domain(min = span.lo, max = span.hi) else Domain(min = 0.0, max = 1.0)
    return if (hasBars) Domain(min = if (e.min > 0.0) 0.0 else e.min, max = if (e.max < 0.0) 0.0 else e.max) else e
  }

fun extendSpan(span: ExtentSpan, values: List<Double>): ExtentSpan {
    var seen = span.seen
    var lo = span.lo
    var hi = span.hi
...
fun renderChartIn(raw: ChartSpec, measure: (String, Double) -> Double, l: PlotLayout): List<PyreonDrawCmd> {
      ...
      if (s.kind == "bars") {
        ...
        if (s.kind == "waterfall") {
          ...
          for (si in 0 until steps.length) {
            val st = steps[si]
            val fill = stateFill(s, st.datumIndex, if (st.value < 0.0) (s.negativeColor ?: withAlpha(s.color, 0.55)) else s.color)
          ...
          if (s.kind == "line") {
            ...
            if (s.kind == "band") {
              ...
              for (run in splitRuns(paired, place)) {
                ...
                val bandFill = withAlpha(s.color, (s.areaOpacity ?: AREA_MARK_OPACITY))
                if (poly.length > 2) {
              ...
              if (s.kind == "area") {
                for (run in splitRuns(s.values, place)) {
                  ...
                  if (pts.length > 1) {
                    ...
                    out.add(polygonCmd(poly, withAlpha(s.color, (s.areaOpacity ?: AREA_MARK_OPACITY)), sGrad, s.pattern))
                  }
                }
              } else {
    ...
    for (ti in 0 until l.yTicks.length) {
      ...
      out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = yAxisX - 6.0, y = tick.pos), size = t.fontSize, align = "end", baseline = "middle"))
    }
    ...
    for (ti in 0 until l.xTicks.length) {
      ...
      if (l.xLabelRotate != 0.0) {
        out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = tick.pos, y = xLabelY), size = t.fontSize, align = "end", baseline = "middle", rotate = l.xLabelRotate))
      } else {
        out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = tick.pos, y = xLabelY), size = t.fontSize, align = "middle", baseline = "top"))
      }
    }
...
fun pictorialBar(s: Series, bar: PyreonChartRect, horizontal: Boolean, fill: String): PictorialBar = PictorialBar(bar = bar, horizontal = horizontal, symbol = (s.symbol ?: "rect"), repeat = s.symbolRepeat == true, fill = fill)
