...
data class FrameLength(var mode: String, var amount: Double)

data class FrameSpec(var left: FrameLength, var top: FrameLength, var right: FrameLength, var bottom: FrameLength, var width: FrameLength, var height: FrameLength, var round: Boolean, var centerX: FrameLength, var centerY: FrameLength, var radius: FrameLength)
...
data class PieOptions(var innerRadius: Double, var showLabels: Boolean, var labelColor: String, var fontSize: Double, var arcs: ArcConfig? = null, var labels: PieLabelOptions? = null, var view: PyreonChartRect? = null, var empty: String? = null, var measure: ((String, Double) -> Double)? = null)
...
data class DialLen(var v: Double, var pct: Boolean)

data class DialStop(var at: Double, var color: String)
...
data class ToolboxOptions(var fontSize: Double, var color: String, var active: String? = null, var actives: List<String>? = null, var gap: Double? = null)

data class ToolboxLayout(var cmds: List<PyreonDrawCmd>, var boxes: List<PyreonChartRect>, var height: Double)
...
private val DEFAULT_ARCS: ArcConfig = ArcConfig(start = START, sweep = TAU, clockwise = true, minAngle = 0.0, padAngle = 0.0, rose = "", zeros = false)
...
private val FILLER = 30.0

private val EDGE_GAP = 15.0

private val MOVE_HANDLE = 7.0

private val GAP: Double = (0.0).toDouble() / (0.0).toDouble()

fun frameLength(l: FrameLength, extent: Double, fallback: Double): Double {
    if (l.mode == "px") {
      return l.amount
    }
    if (l.mode == "pct") {
      return ((l.amount).toDouble() / (100.0).toDouble()) * extent
    }
    return fallback
  }

fun frameEdge(l: FrameLength, extent: Double, size: Double): Double {
    if (l.mode == "center") {
      return ((extent - size)).toDouble() / (2.0).toDouble()
    }
    if (l.mode == "start") {
      return 0.0
    }
...
fun frameView(spec: FrameSpec, width: Double, height: Double): PyreonChartRect {
    val hasW = spec.width.mode != ""
    val hasH = spec.height.mode != ""
    val w0 = frameLength(spec.width, width, width)
    val h0 = frameLength(spec.height, height, height)
    val hasLeft = spec.left.mode != ""
    val hasTop = spec.top.mode != ""
    val hasRight = spec.right.mode != ""
    val hasBottom = spec.bottom.mode != ""
    val right = frameLength(spec.right, width, 0.0)
    val bottom = frameLength(spec.bottom, height, 0.0)
    val x = if (hasLeft) frameEdge(spec.left, width, if (hasW) w0 else 0.0) else if (hasW && hasRight) width - right - w0 else 0.0
    val y = if (hasTop) frameEdge(spec.top, height, if (hasH) h0 else 0.0) else if (hasH && hasBottom) height - bottom - h0 else 0.0
    val w = if (hasW) w0 else Math.max(0.0, width - x - right)
    val h = if (hasH) h0 else Math.max(0.0, height - y - bottom)
    return PyreonChartRect(x = x, y = y, w = Math.max(0.0, w), h = Math.max(0.0, h))
  }

fun frameRect(spec: FrameSpec, width: Double, height: Double): PyreonChartRect {
    val view = frameView(spec, width, height)
    if (!spec.round) {
      return view
    }
    val cx = view.x + frameLength(spec.centerX, view.w, (view.w).toDouble() / (2.0).toDouble())
    val cy = view.y + frameLength(spec.centerY, view.h, (view.h).toDouble() / (2.0).toDouble())
    val half = (Math.min(view.w, view.h)).toDouble() / (2.0).toDouble()
    val r = frameLength(spec.radius, half, half)
    return PyreonChartRect(x = cx - r, y = cy - r, w = r * 2.0, h = r * 2.0)
  }

fun frameRectAt(spec: FrameSpec, width: Double, height: Double, dx: Double, dy: Double): PyreonChartRect {
    val r = frameRect(spec, width, height)
    return PyreonChartRect(x = r.x - dx, y = r.y - dy, w = r.w, h = r.h)
  }

fun frameViewAt(spec: FrameSpec, width: Double, height: Double, dx: Double, dy: Double): PyreonChartRect {
    val r = frameView(spec, width, height)
    return PyreonChartRect(x = r.x - dx, y = r.y - dy, w = r.w, h = r.h)
  }
...
fun layoutArcs(slices: List<Slice>): List<ArcGeometry> = layoutArcsWith(slices, DEFAULT_ARCS)
...
fun layoutArcsWith(slices: List<Slice>, cfg: ArcConfig): List<ArcGeometry> {
    ...
    val unit = (TAU).toDouble() / ((if (total > 0.0) total else (count).toDouble())).toDouble()
    val minPad = cfg.minAngle + cfg.padAngle
    val halfPad = (cfg.padAngle).toDouble() / (2.0).toDouble()
...
fun renderPie(slices: List<Slice>, box: PyreonChartRect, opts: PieOptions): List<PyreonDrawCmd> {
    ...
    for (a in arcs) {
      ...
      out.add(PyreonDrawCmd(kind = "polygon", fill = a.slice.color, points = arcPolygon(center, inner + a.reach * (radius - inner), inner, a.start, a.end)))
    }
    val lab = opts.labels
    if (lab != null) {
      ...
      for (l in placed) {
        ...
        if (l.text != "" && l.rotate != 0.0) {
          out.add(PyreonDrawCmd(kind = "text", fill = fill, text = l.text, at = l.at, size = lab.fontSize, align = l.align, baseline = "middle", rotate = l.rotate))
        } else {
          if (l.text != "") {
            out.add(PyreonDrawCmd(kind = "text", fill = fill, text = l.text, at = l.at, size = lab.fontSize, align = l.align, baseline = "middle"))
          }
        }
      }
      return out
...
fun layoutPieLabels(arcs: List<ArcGeometry>, center: PyreonChartPt, radius: Double, inner: Double, view: PyreonChartRect, o: PieLabelOptions, measure: (String, Double) -> Double): List<PieLabel> {
    ...
    val aligns: MutableList<String> = mutableListOf()
    val nxs: MutableList<Double> = mutableListOf()
    val nys: MutableList<Double> = mutableListOf()
    val outside: MutableList<Boolean> = mutableListOf()
    val l0x: MutableList<Double> = mutableListOf()
    val l0y: MutableList<Double> = mutableListOf()
    val l1x: MutableList<Double> = mutableListOf()
    val l1y: MutableList<Double> = mutableListOf()
    val l2x: MutableList<Double> = mutableListOf()
    val l2y: MutableList<Double> = mutableListOf()
    val rots: MutableList<Double> = mutableListOf()
    ...
    for (a in arcs) {
      ...
      val nx = Math.cos((a.mid).toDouble())
      val ny = Math.sin((a.mid).toDouble())
      val rs = inner + a.reach * (radius - inner)
      idx.add(a.index)
      texts.add(text)
      nxs.add(nx)
      nys.add(ny)
      outside.add(isOut)
      rots.add(labelRotation(o, a.mid, nx, ny))
      if (o.position == "center") {
        ...
        if (!isOut) {
          val x1 = (((rs + inner)).toDouble() / (2.0).toDouble()) * nx + center.x
          val y1 = (((rs + inner)).toDouble() / (2.0).toDouble()) * ny + center.y
          tx.add(x1 + nx * 3.0)
          ty.add(y1 + ny * 3.0)
          aligns.add("middle")
          l0x.add(x1)
          ...
          val x1 = rs * nx + center.x
          val y1 = rs * ny + center.y
          val x2 = x1 + nx * (o.leg1 + radius - rs)
          val y2 = y1 + ny * (o.leg1 + radius - rs)
          val x3 = x2 + (if (nx < 0.0) -1.0 else 1.0) * o.leg2
          val edge = o.alignTo == "edge"
          tx.add(if (edge) (if (nx < 0.0) view.x + edgeDist else view.x + view.w - edgeDist) else x3 + (if (nx < 0.0) -o.distance else o.distance))
          ty.add(y2)
          aligns.add(if (edge) (if (nx > 0.0) "end" else "start") else if (nx > 0.0) "start" else "end")
          l0x.add(x1)
          l0y.add(y1)
          l1x.add(x2)
          l1y.add(y2)
...
fun labelRotation(o: PieLabelOptions, mid: Double, nx: Double, ny: Double): Double {
    if (o.rotate == "fixed") {
      return ((o.rotateDegrees * kotlin.math.PI)).toDouble() / (180.0).toDouble()
    }
    if (o.position == "center") {
      return 0.0
    }
    if (o.rotate == "radial") {
      return if (nx < 0.0) -mid + kotlin.math.PI else -mid
    }
    val out = o.position != "inside" && o.position != "inner"
    if (o.rotate == "tangential" || (o.rotate == "tangential-noflip" && !out)) {
      var rad = Math.atan2((nx).toDouble(), (ny).toDouble())
      if (rad < 0.0) {
        rad = kotlin.math.PI * 2.0 + rad
      }
      if (ny > 0.0 && o.rotate != "tangential-noflip") {
        rad = kotlin.math.PI + rad
      }
      return rad - kotlin.math.PI
    }
    return 0.0
  }

fun truncateLabel(text: String, width: Double, size: Double, measure: (String, Double) -> Double): String {
    val container = Math.max(0.0, width - 1.0)
    if (container <= 0.0) {
      return ""
    }
    var ellipsis = "..."
    ...
    val content = container - ellipsisW
    var line = text
...
fun shiftOnY(list: List<Int>, ty: List<Double>, rh: Double, minBound: Double, maxBound: Double): Shifted {
    ...
    val p4 = takeBoundsGap(p3, rh, gapBottom(p3), gapTop(p3), -1.0)
...
fun squeezeGaps(pos0: List<Double>, rh: Double, d: Double, maxPct: Double): List<Double> {
    val len = pos0.length
...
fun takeBoundsGap(pos0: List<Double>, rh: Double, gapThis: Double, gapOther: Double, moveDir: Double): List<Double> {
    if (gapThis >= 0.0) {
      return pos0
    }
...
fun squeezeWhenBailout(pos0: List<Double>, d: Double): List<Double> {
    val len = pos0.length
...
fun limitLine(x0: Double, y0: Double, x1: Double, y1: Double, x2: Double, y2: Double, nx: Double, ny: Double, minTurnDeg: Double, maxSurfaceDeg: Double): PyreonChartPt {
    var ex = x1
    var ey = y1
    if (minTurnDeg <= 180.0 && minTurnDeg > 0.0) {
      val turn = ((minTurnDeg).toDouble() / (180.0).toDouble()) * kotlin.math.PI
      val ax = x0 - ex
      val ay = y0 - ey
      val bx = x2 - ex
      val by = y2 - ey
      val la = Math.sqrt((ax * ax + ay * ay).toDouble())
      val lb = Math.sqrt((bx * bx + by * by).toDouble())
      if (la >= 0.001 && lb >= 0.001) {
        val ux = (bx).toDouble() / (lb).toDouble()
        val uy = (by).toDouble() / (lb).toDouble()
        val turnCos = ((ax).toDouble() / (la).toDouble()) * ux + ((ay).toDouble() / (la).toDouble()) * uy
        if (Math.cos((turn).toDouble()) < turnCos) {
          val proj = project(ex, ey, x2, y2, x0, y0)
          val px = proj.x + ux * ((proj.d).toDouble() / (Math.tan((kotlin.math.PI - turn).toDouble())).toDouble())
          val py = proj.y + uy * ((proj.d).toDouble() / (Math.tan((kotlin.math.PI - turn).toDouble())).toDouble())
          val t = if (x2 != ex) ((px - ex)).toDouble() / ((x2 - ex)).toDouble() else ((py - ey)).toDouble() / ((y2 - ey)).toDouble()
          if (t == t) {
            if (t < 0.0) {
    ...
    if (maxSurfaceDeg <= 180.0 && maxSurfaceDeg > 0.0) {
      val surf = ((maxSurfaceDeg).toDouble() / (180.0).toDouble()) * kotlin.math.PI
      val ax = ex - x0
      val ay = ey - y0
      val bx = x2 - ex
      val by = y2 - ey
      ...
      if (la >= 0.001 && lb >= 0.001) {
        val ux = (bx).toDouble() / (lb).toDouble()
        val uy = (by).toDouble() / (lb).toDouble()
        val surfCos = ((ax).toDouble() / (la).toDouble()) * nx + ((ay).toDouble() / (la).toDouble()) * ny
        if (surfCos < Math.cos((surf).toDouble())) {
          val proj = project(ex, ey, x2, y2, x0, y0)
          val angle2 = Math.acos((ux * nx + uy * ny).toDouble())
          val newAngle = (kotlin.math.PI).toDouble() / (2.0).toDouble() + angle2 - surf
          if (newAngle >= (kotlin.math.PI).toDouble() / (2.0).toDouble()) {
            ex = x2
            ey = y2
          } else {
            val px = proj.x + ux * ((proj.d).toDouble() / (Math.tan(((kotlin.math.PI).toDouble() / (2.0).toDouble() - newAngle).toDouble())).toDouble())
            val py = proj.y + uy * ((proj.d).toDouble() / (Math.tan(((kotlin.math.PI).toDouble() / (2.0).toDouble() - newAngle).toDouble())).toDouble())
            val t = if (x2 != ex) ((px - ex)).toDouble() / ((x2 - ex)).toDouble() else ((py - ey)).toDouble() / ((y2 - ey)).toDouble()
            if (t == t) {
              if (t < 0.0) {
...
fun project(x1: Double, y1: Double, x2: Double, y2: Double, x: Double, y: Double): Projection {
    val dx1r = x2 - x1
    val dy1r = y2 - y1
    val len = Math.sqrt((dx1r * dx1r + dy1r * dy1r).toDouble())
    val ux = (dx1r).toDouble() / (len).toDouble()
    val uy = (dy1r).toDouble() / (len).toDouble()
    val t = (x - x1) * ux + (y - y1) * uy
    val ox = x1 + t * ux
    val oy = y1 + t * uy
...
fun computeLayout(cfg: LayoutConfig, measure: (String, Double) -> Double): PlotLayout {
    ...
    val yGap = if (cfg.yLabelInside == true) 0.0 else ((cfg.yLabelMargin ?: labelGap))
    val yBand = (if (cfg.showYAxis) yLabelW + yGap + tickLen + ((cfg.yOffset ?: 0.0)) else 0.0) + yTitleH
    ...
    val contain = cfg.insetContain == true
    val insL = (cfg.insetLeft ?: 0.0)
    val insR = (cfg.insetRight ?: 0.0)
    val gLeft = if (cfg.insetLeft != null) (if (contain) Math.max(insL, left) else insL) else left + ((cfg.reserveLeft ?: 0.0))
    val gRight = if (cfg.insetRight != null) (if (contain) Math.max(insR, right) else insR) else right
    ...
    if (cfg.showXAxis && cfg.horizontal != true) {
      ...
      if (mode == "echarts") {
        ...
        if (angle != 0.0) {
          rotate = angle
          slantH = widestX * Math.abs(Math.sin((rad).toDouble())) + cfg.fontSize * Math.abs(Math.cos((rad).toDouble())) - cfg.fontSize
          if (slantH < 0.0) {
            slantH = 0.0
          }
        }
    ...
    val insT = (cfg.insetTop ?: 0.0)
    val insB = (cfg.insetBottom ?: 0.0)
    val top = if (cfg.insetTop != null) (if (contain) Math.max(insT, autoTop) else insT) else autoTop
    val bottom = if (cfg.insetBottom != null) (if (contain) Math.max(insB, autoBottom) else insB) else autoBottom
    val gutters = Gutters(left = gLeft, right = gRight, top = top, bottom = bottom)
    val plot = PyreonChartRect(x = gLeft, y = top, w = Math.max(0.0, cfg.width - gLeft - gRight), h = Math.max(0.0, cfg.height - top - bottom))
...
fun echartsCategoryEvery(labels: List<String>, axisW: Double, edge: Boolean, fontSize: Double, rad: Double, measure: (String, Double) -> Double): Int {
    ...
    val unitW = Math.abs(span * Math.cos((rad).toDouble()))
...
fun stackLevelsExtent(levels: StackLevels): Domain {
    var lo = 0.0
    var hi = 0.0
...
fun layoutStackedBars(seriesValues: List<List<Double>>, plot: PyreonChartRect, yDomain: Domain, gapRatio: Double): List<StackSegment> = layoutStackLevels(stackLevels(seriesValues, listOf(), listOf(), listOf()), seriesValues, plot, yDomain, gapRatio)

fun layoutStackedBarsH(seriesValues: List<List<Double>>, plot: PyreonChartRect, vDomain: Domain, gapRatio: Double): List<StackSegment> = layoutStackLevelsH(stackLevels(seriesValues, listOf(), listOf(), listOf()), seriesValues, plot, vDomain, gapRatio)
...
fun stackedExtent(seriesValues: List<List<Double>>): Domain = stackLevelsExtent(stackLevels(seriesValues, listOf(), listOf(), listOf()))
...
fun fraction(v: Double, lo: Double, hi: Double): Double {
    ...
    return Math.max(0.0, Math.min(1.0, ((v - lo)).toDouble() / ((hi - lo)).toDouble()))
  }

fun band(center: PyreonChartPt, outer: Double, inner: Double, a: Double, b: Double, fill: String, round: Boolean): PyreonDrawCmd {
    val lo = Math.min(a, b)
    val hi = Math.max(a, b)
    if (round && hi - lo < kotlin.math.PI * 2.0) {
...
fun sausagePolygon(center: PyreonChartPt, outer: Double, inner: Double, lo: Double, hi: Double): List<PyreonChartPt> {
    val r0 = Math.max(inner, 0.0)
    val r = Math.max(outer, 0.0)
...
fun renderDial(spec: DialSpec, center: PyreonChartPt, r: Double, palette: List<String>? = null): List<PyreonDrawCmd> {
    ...
    val span = dir * spec.sweep
    val start = spec.start
    if (spec.lineShow) {
      var prev = start
      val bands: MutableList<PyreonDrawCmd> = mutableListOf()
      for (s in spec.stops) {
        val at = start + span * Math.min(Math.max(s.at, 0.0), 1.0)
    ...
    val splitLen = dialLen(spec.splitLength, r)
    val tickLen = dialLen(spec.tickLength, r)
    ...
    for (i in 0..spec.splitNumber) {
      ...
      if (spec.splitShow) {
        val d = if (spec.splitDistance != 0.0) spec.splitDistance + spec.lineWidth else spec.lineWidth
        val stroke = if (spec.splitColor == "auto") dialColor(spec.stops, (i).toDouble() / (spec.splitNumber).toDouble()) else spec.splitColor
        layers.add(Layered(z = 0.0, seq = seq, cmd = PyreonDrawCmd(kind = "line", from = PyreonChartPt(x = ux * (r - d) + center.x, y = uy * (r - d) + center.y), to = PyreonChartPt(x = ux * (r - splitLen - d) + center.x, y = uy * (r - splitLen - d) + center.y), stroke = stroke, width = spec.splitWidth)))
        seq = seq + 1.0
      }
      if (spec.labelShow && i < spec.labels.length) {
        val d = spec.labelDistance + spec.splitDistance
        val at = PyreonChartPt(x = ux * (r - splitLen - d) + center.x, y = uy * (r - splitLen - d) + center.y)
        val fill = if (spec.labelColor == "") dialColor(spec.stops, (i).toDouble() / (spec.splitNumber).toDouble()) else spec.labelColor
        var rot = 0.0
        if (spec.labelRotate == "radial") {
          rot = -angle + kotlin.math.PI * 2.0
          if (rot > (kotlin.math.PI).toDouble() / (2.0).toDouble()) {
            rot = rot + kotlin.math.PI
          }
        } else {
          if (spec.labelRotate == "tangential") {
            rot = -angle - (kotlin.math.PI).toDouble() / (2.0).toDouble()
          } else {
            if (spec.labelRotate == "fixed") {
              rot = ((spec.labelDegrees * kotlin.math.PI)).toDouble() / (180.0).toDouble()
            }
          }
        }
        if (rot == 0.0) {
      ...
      if (spec.tickShow && i != spec.splitNumber) {
        ...
        for (j in 0..spec.tickSplit) {
          ...
          layers.add(Layered(z = 0.0, seq = seq, cmd = PyreonDrawCmd(kind = "line", from = PyreonChartPt(x = tx * (r - d) + center.x, y = ty * (r - d) + center.y), to = PyreonChartPt(x = tx * (r - tickLen - d) + center.x, y = ty * (r - tickLen - d) + center.y), stroke = stroke, width = spec.tickWidth)))
          seq = seq + 1.0
          angle = angle + subStep
        }
        angle = angle - subStep
    ...
    for (di in 0 until spec.data.length) {
      ...
      if (spec.titleShow && d.name != "") {
        ...
        layers.add(Layered(z = textZ, seq = seq, cmd = PyreonDrawCmd(kind = "text", fill = spec.titleColor, text = d.name, at = at, size = spec.titleSize, align = "middle", baseline = "middle")))
        seq = seq + 1.0
      }
      if (spec.detailShow && d.detail != "") {
        val at = PyreonChartPt(x = center.x + dialLen(d.detailX, r), y = center.y + dialLen(d.detailY, r))
        if (spec.detailBg != "" || spec.detailBorderWidth > 0.0) {
          val pad = spec.detailPadding
          val bw = spec.detailWidth + pad[1] + pad[3]
          val bh = (if (spec.detailHeight >= 0.0) spec.detailHeight else spec.detailSize) + pad[0] + pad[2]
          val box = PyreonChartRect(x = at.x - (bw).toDouble() / (2.0).toDouble(), y = at.y - (bh).toDouble() / (2.0).toDouble(), w = bw, h = bh)
          if (spec.detailBg != "") {
            layers.add(Layered(z = textZ, seq = seq, cmd = PyreonDrawCmd(kind = "rect", rect = box, fill = spec.detailBg)))
            seq = seq + 1.0
          }
          if (spec.detailBorderWidth > 0.0) {
            val edge = listOf(PyreonChartPt(x = box.x, y = box.y), PyreonChartPt(x = box.x + box.w, y = box.y), PyreonChartPt(x = box.x + box.w, y = box.y + box.h), PyreonChartPt(x = box.x, y = box.y + box.h), PyreonChartPt(x = box.x, y = box.y))
            layers.add(Layered(z = textZ, seq = seq, cmd = PyreonDrawCmd(kind = "polyline", stroke = spec.detailBorder, width = spec.detailBorderWidth, points = edge)))
            seq = seq + 1.0
          }
        }
        val fill = if (spec.detailColor != "") spec.detailColor else if (spec.progressShow) colorOf(d, di) else auto
        if (spec.detailBold) {
          layers.add(Layered(z = textZ, seq = seq, cmd = PyreonDrawCmd(kind = "text", fill = fill, text = d.detail, at = at, size = spec.detailSize, align = "middle", baseline = "middle", weight = "bold")))
        } else {
          layers.add(Layered(z = textZ, seq = seq, cmd = PyreonDrawCmd(kind = "text", fill = fill, text = d.detail, at = at, size = spec.detailSize, align = "middle", baseline = "middle")))
        }
        seq = seq + 1.0
      }
    }
    if (spec.anchorShow) {
      val at = PyreonChartPt(x = center.x + dialLen(spec.anchorX, r), y = center.y + dialLen(spec.anchorY, r))
      val half = (spec.anchorSize).toDouble() / (2.0).toDouble()
      val z = if (spec.anchorAbove) 1.0 else 0.0
      if (spec.anchorIcon != "circle") {
        val cell = PyreonChartRect(x = at.x - half, y = at.y - half, w = spec.anchorSize, h = spec.anchorSize)
        layers.add(Layered(z = z, seq = seq, cmd = PyreonDrawCmd(kind = "polygon", fill = spec.anchorColor, points = iconPoints(spec.anchorIcon, cell))))
        if (spec.anchorBorderWidth > 0.0) {
          ...
          ring.add(ring[0])
          layers.add(Layered(z = z, seq = seq, cmd = PyreonDrawCmd(kind = "polyline", stroke = spec.anchorBorder, width = spec.anchorBorderWidth, points = ring)))
        }
      } else {
        if (spec.anchorBorderWidth > 0.0) {
          layers.add(Layered(z = z, seq = seq, cmd = PyreonDrawCmd(kind = "circle", fill = spec.anchorBorder, center = at, radius = half + (spec.anchorBorderWidth).toDouble() / (2.0).toDouble())))
          seq = seq + 1.0
          layers.add(Layered(z = z, seq = seq, cmd = PyreonDrawCmd(kind = "circle", fill = spec.anchorColor, center = at, radius = Math.max(0.0, half - (spec.anchorBorderWidth).toDouble() / (2.0).toDouble()))))
        } else {
          layers.add(Layered(z = z, seq = seq, cmd = PyreonDrawCmd(kind = "circle", fill = spec.anchorColor, center = at, radius = half)))
        }
      }
      seq = seq + 1.0
    }
    ...
    for (i in 0 until count) {
      ...
      if (spec.pointerShow) {
        val a = start + span * f
        val fill = if (d.pointerColor == "auto") auto else if (d.pointerColor != "") d.pointerColor else colorOf(d, i)
        val len = dialLen(spec.pointerLength, r)
        val w = dialLen(spec.pointerWidth, r)
        val ox = dialLen(spec.pointerX, r)
        val oy = dialLen(spec.pointerY, r)
        val points = if (spec.pointerIcon == "") pointerPoints(center, a, len, w, ox, oy) else turnAbout(center, a, iconPoints(spec.pointerIcon, PyreonChartRect(x = ox - (w).toDouble() / (2.0).toDouble(), y = oy - len, w = w, h = len)))
        layers.add(Layered(z = 0.0, seq = seq, cmd = PyreonDrawCmd(kind = "polygon", fill = fill, points = points)))
        seq = seq + 1.0
      }
      if (spec.progressShow) {
        val raw = if (spec.max == spec.min) 0.0 else ((d.value - spec.min)).toDouble() / ((spec.max - spec.min)).toDouble()
        val t = if (spec.progressClip) f else raw
        val width = if (spec.progressOverlap) spec.progressWidth else (spec.lineWidth).toDouble() / ((count * 1.0)).toDouble()
        val outer = if (spec.progressOverlap) r else r - ring * width
        val fill = if (d.progressColor == "auto") auto else if (d.progressColor != "") d.progressColor else colorOf(d, i)
        val z = if (spec.progressOverlap) 100.0 - f * 100.0 else 0.0
        layers.add(Layered(z = z, seq = seq, cmd = band(center, outer, outer - width, start, start + span * t, fill, spec.progressRound)))
        seq = seq + 1.0
      }
      ring = ring + 1.0
    }
    layers.sortWith(Comparator { p, q -> ((if (p.z != q.z) p.z - q.z else p.seq - q.seq)).compareTo(0.0) })
    val out: MutableList<PyreonDrawCmd> = mutableListOf()
    for (l in layers) {
      out.add(l.cmd)
    }
...
fun pointerPoints(center: PyreonChartPt, angle: Double, len: Double, w: Double, ox: Double, oy: Double): List<PyreonChartPt> {
    val k = if (w >= (len).toDouble() / (3.0).toDouble()) 1.0 else 2.0
    val local = listOf(PyreonChartPt(x = ox, y = oy + w * k), PyreonChartPt(x = ox - w, y = oy), PyreonChartPt(x = ox, y = oy - len), PyreonChartPt(x = ox + w, y = oy))
    return turnAbout(center, angle, local)
  }

fun turnAbout(center: PyreonChartPt, angle: Double, local: List<PyreonChartPt>): List<PyreonChartPt> {
    val turn = angle + (kotlin.math.PI).toDouble() / (2.0).toDouble()
    val c = Math.cos((turn).toDouble())
    val s = Math.sin((turn).toDouble())
    val out: MutableList<PyreonChartPt> = mutableListOf()
    for (p in local) {
      out.add(PyreonChartPt(x = center.x + p.x * c - p.y * s, y = center.y + p.x * s + p.y * c))
    }
    return out
  }

fun iconPoints(icon: String, cell: PyreonChartRect): List<PyreonChartPt> {
    if (icon == "arrow") {
      val x = cell.x + (cell.w).toDouble() / (2.0).toDouble()
      val y = cell.y + (cell.h).toDouble() / (2.0).toDouble()
      val dx = ((cell.w).toDouble() / (3.0).toDouble()) * 2.0
      return listOf(PyreonChartPt(x = x, y = y), PyreonChartPt(x = x + dx, y = y + cell.h), PyreonChartPt(x = x, y = y + ((cell.h).toDouble() / (4.0).toDouble()) * 3.0), PyreonChartPt(x = x - dx, y = y + cell.h))
    }
    return symbolPoints(cell, if (icon == "emptyCircle") "circle" else icon)
  }

fun renderDialIn(spec: DialSpec, box: PyreonChartRect, palette: List<String>): List<PyreonDrawCmd> {
...
fun minorPositions(ticks: List<Tick>, pieces: Double): List<Double> {
    val out: MutableList<Double> = mutableListOf()
    val n = Math.floor(pieces + 0.5)
    if (n < 2.0) {
      return out
    }
...
fun adjustLabel(s: Series, cmdsOf: (PyreonChartPt, String, String) -> List<PyreonDrawCmd>, at: PyreonChartPt, align: String, baseline: String): List<PyreonDrawCmd> {
    ...
    val dy = Math.cos((rad).toDouble()) * oy - Math.sin((rad).toDouble()) * ox
    val ha = (s.labelAlign ?: "")
    val va = (s.labelVerticalAlign ?: "")
    val cmds = cmdsOf(PyreonChartPt(x = at.x + dx, y = at.y + dy), if (ha == "left") "start" else if (ha == "center") "middle" else if (ha == "right") "end" else align, if (va == "top" || va == "middle" || va == "bottom") va else baseline)
    if (deg == 0.0) {
      return cmds
    }
    val ax = at.x + dx
    ...
    for (i in 0 until cmds.length) {
      ...
      if (cmd.kind != "text") {
        ...
        val p0 = (cmd.at ?: PyreonChartPt(x = ax, y = ay))
        val rx = p0.x - ax
        val ry = p0.y - ay
        out.add(cmd.copy(at = PyreonChartPt(x = ax + cc * rx - sn * ry, y = ay + sn * rx + cc * ry), rotate = 0.0 - deg))
      }
    }
    return out
  }
...
fun seriesLabelCmds(s: Series, index: Int, fallback: String, at: PyreonChartPt, align: String, baseline: String, t: ChartTheme, measure: (String, Double) -> Double): List<PyreonDrawCmd> {
    ...
    return adjustLabel(s, { a, al, bl -> labelCommands(text, (s.labelRich ?: listOf()), a, al, bl, if (color == "") t.label else color, if (size > 0.0) size else t.fontSize, measure) }, at, align, baseline)
  }
...
fun barLabelCmds(s: Series, index: Int, fallback: String, r: PyreonChartRect, shapeFill: String, t: ChartTheme, measure: (String, Double) -> Double): List<PyreonDrawCmd> {
    ...
    val size = (s.labelSize ?: 0.0)
    val text = labelTextAt(s, index, fallback)
    return adjustLabel(s, { a, al, bl -> labelCommands(text, (s.labelRich ?: listOf()), a, al, bl, if (own == "") auto.textFill else own, if (size > 0.0) size else t.fontSize, measure, halo, width) }, place.at, place.align, place.baseline)
  }
...
fun edgeCategoryPoints(spec: ChartSpec): Boolean {
...
fun minBarLength(r: PyreonChartRect, minLen: Double, up: Boolean, horizontal: Boolean, plot: PyreonChartRect): PyreonChartRect {
    ...
    if (!horizontal) {
      ...
      if (up) {
        val y = Math.max(plot.y, r.y + r.h - minLen)
        return PyreonChartRect(x = r.x, y = y, w = r.w, h = r.y + r.h - y)
      }
      return PyreonChartRect(x = r.x, y = r.y, w = r.w, h = Math.min(plot.y + plot.h, r.y + minLen) - r.y)
    }
    if (r.w >= minLen) {
      return r
    }
    if (up) {
      return PyreonChartRect(x = r.x, y = r.y, w = Math.min(plot.x + plot.w, r.x + minLen) - r.x, h = r.h)
    }
    val x = Math.max(plot.x, r.x + r.w - minLen)
    return PyreonChartRect(x = x, y = r.y, w = r.x + r.w - x, h = r.h)
  }

fun barBackgroundRect(r: PyreonChartRect, plot: PyreonChartRect, horizontal: Boolean): PyreonChartRect = if (horizontal) PyreonChartRect(x = plot.x, y = r.y, w = plot.w, h = r.h) else PyreonChartRect(x = r.x, y = plot.y, w = r.w, h = plot.h)
...
fun levelsOf(series: List<Series>): StackLevels {
    val values: MutableList<List<Double>> = mutableListOf()
    val groups: MutableList<String> = mutableListOf()
    val strategies: MutableList<String> = mutableListOf()
    val descs: MutableList<Boolean> = mutableListOf()
    for (s in series) {
      values.add(s.values)
      groups.add((s.barStack ?: ""))
      strategies.add((s.stackStrategy ?: ""))
      descs.add((s.stackDesc ?: false))
    }
...
fun renderChartIn(raw: ChartSpec, measure: (String, Double) -> Double, l: PlotLayout): List<PyreonDrawCmd> {
    ...
    if (spec.showYAxis && spec.yTicks == true && spec.horizontal != true) {
      ...
      for (tick in l.yTicks) {
        out.add(PyreonDrawCmd(kind = "line", from = PyreonChartPt(x = yLineX, y = tick.pos), to = PyreonChartPt(x = yLineX + yDir * yLen, y = tick.pos), stroke = (spec.yTickColor ?: yLineColor), width = 1.0))
      }
    }
    ...
    val xLineY = if (spec.xAxisOnZero == true && spec.horizontal != true && yDomain.min < 0.0 && yDomain.max > 0.0) scaleLinear(yDomain, plot.y + plot.h, plot.y, 0.0) else xAxisY
    val xLineColor = (spec.xAxisLineColor ?: t.axis)
    if (spec.showXAxis && spec.xAxisLine != false) {
      out.add(PyreonDrawCmd(kind = "line", from = PyreonChartPt(x = plot.x, y = xLineY), to = PyreonChartPt(x = plot.x + plot.w, y = xLineY), stroke = xLineColor, width = (spec.xAxisLineWidth ?: 1.0)))
    }
    if (spec.showXAxis && spec.xTicks == true && spec.horizontal != true) {
      ...
      for (tx in xTickPositions(spec, l, plot)) {
        out.add(PyreonDrawCmd(kind = "line", from = PyreonChartPt(x = tx, y = xLineY), to = PyreonChartPt(x = tx, y = xLineY + xDir * xLen), stroke = (spec.xTickColor ?: xLineColor), width = 1.0))
      }
    }
    ...
    for (a in notes) {
      ...
      if (a.xFrom != null && a.xTo != null) {
        val span = categorySpanPixels(spec, l.xDomainUsed, plot, xFrom, xTo)
        val x1 = span.x
        val x2 = span.y
    ...
    for (a in notes) {
      ...
      val ax = (a.x ?: 0.0)
      if (a.x != null) {
        val xPos = categoryXPixel(spec, l.xDomainUsed, plot, ax)
      ...
      if (a.x1 != null && a.y1 != null && a.x2 != null && a.y2 != null) {
        val p1 = PyreonChartPt(x = categoryXPixel(spec, l.xDomainUsed, plot, sx1), y = scaleLinear(yDomain, plot.y + plot.h, plot.y, sy1))
        val p2 = PyreonChartPt(x = categoryXPixel(spec, l.xDomainUsed, plot, sx2), y = scaleLinear(yDomain, plot.y + plot.h, plot.y, sy2))
    ...
    for (oIdx in 0 until spec.series.length) {
      ...
      if (spec.horizontal == true) {
        ...
        val rects = barsLaidH(spec, sIdx, plot, yDomain)
        val bgH = (s.barBackground ?: "")
        if (bgH != "") {
          for (r in rects) {
            if (r.w >= 0.0) {
              out.add(rectCmd(barBackgroundRect(r, plot, true), bgH, null, null, null))
            }
          }
        }
      ...
      if (s.kind == "bars") {
        ...
        val bgV = (s.barBackground ?: "")
        if (bgV != "") {
          for (r in rects) {
            if (r.w >= 0.0) {
              out.add(rectCmd(barBackgroundRect(r, plot, false), bgV, null, null, null))
            }
          }
        }
        for (ri in 0 until rects.length) {
            ...
            if (s.symbol != null && progress >= 1.0) {
              ...
              for (i in 0 until dots.length) {
                ...
                if (s.symbolHollow == true) {
                  ...
                  out.add(symbolCommand(cell, lineSymbol, s.color))
                }
              }
            }
          } else {
    ...
    for (ti in 0 until l.xTicks.length) {
      ...
      if (l.xLabelRotate != 0.0) {
        out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = tick.pos, y = xLabelY), size = t.fontSize, align = if (l.xLabelRotate < 0.0) "end" else "start", baseline = "middle", rotate = if (xTop) -l.xLabelRotate else l.xLabelRotate))
      } else {
        out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = tick.pos, y = xLabelY), size = t.fontSize, align = "middle", baseline = if (xBelow) "top" else "bottom"))
      }
    }
...
fun hollowSymbol(cell: PyreonChartRect, symbol: String, stroke: String, surface: String): List<PyreonDrawCmd> {
    val inside = if (surface == "") "#ffffff" else surface
    if (symbol == "circle") {
      val r = ((if (cell.w < cell.h) cell.w else cell.h)).toDouble() / (2.0).toDouble()
      val c = PyreonChartPt(x = cell.x + (cell.w).toDouble() / (2.0).toDouble(), y = cell.y + (cell.h).toDouble() / (2.0).toDouble())
      return listOf(PyreonDrawCmd(kind = "circle", fill = stroke, center = c, radius = r + 1.0), PyreonDrawCmd(kind = "circle", fill = inside, center = c, radius = Math.max(0.0, r - 1.0)))
    }
    val body = symbolCommand(cell, symbol, inside)
    val edge = symbolPoints(cell, symbol)
    val ring: MutableList<PyreonChartPt> = mutableListOf()
    for (p in edge) {
      ring.add(p)
    }
...
fun retype(s: Series, kind: String, stack: String): Series {
    ...
    return s.copy(kind = k)
  }

fun applyMagicType(spec: ChartSpec, kind: String, stack: String): ChartSpec {
    if (kind == "" && stack == "") {
      return spec
    }
    val series: MutableList<Series> = mutableListOf()
    ...
    for (s in series) {
      out.add(if (bars > 1.0 && s.kind == "bars") s.copy(kind = "grouped") else s)
    }
    return spec.copy(series = out)
  }
...
fun toolboxGlyph(tool: String): String {
    if (tool == "saveAsImage") {
      return "⤓"
    }
    if (tool == "restore") {
...
fun renderToolbox(tools: List<String>, box: PyreonChartRect, opts: ToolboxOptions): ToolboxLayout {
    ...
    var i = 0.0
    for (tool in tools) {
      val x = box.x + box.w - size - (n - 1.0 - i) * (size + gap)
      var active = opts.active == tool
      for (a in (opts.actives ?: listOf())) {
        if (a == tool) {
          active = true
        }
      }
      boxes.add(PyreonChartRect(x = x, y = box.y, w = size, h = size))
      if (active) {
        cmds.add(PyreonDrawCmd(kind = "rect", rect = PyreonChartRect(x = x, y = box.y, w = size, h = size), fill = "rgba(99,102,241,0.18)"))
      }
      cmds.add(PyreonDrawCmd(kind = "text", fill = opts.color, text = toolboxGlyph(tool), at = PyreonChartPt(x = x + (size).toDouble() / (2.0).toDouble(), y = box.y + (size).toDouble() / (2.0).toDouble()), size = opts.fontSize + 2.0, align = "middle", baseline = "middle"))
      i = i + 1.0
    }
...
fun sliderRect(box: SliderBox, plot: PyreonChartRect, width: Double, height: Double): PyreonChartRect {
    val moveHandle = if (box.brush) MOVE_HANDLE else 0.0
    val unset = FrameLength(mode = "", amount = 0.0)
    val spec = FrameSpec(left = box.left, top = if (box.top.mode != "") box.top else if (box.bottom.mode != "") unset else FrameLength(mode = "px", amount = height - FILLER - EDGE_GAP - moveHandle), right = if (box.right.mode != "") box.right else if (box.left.mode != "") unset else FrameLength(mode = "px", amount = width - plot.x - plot.w), bottom = box.bottom, width = if (box.width.mode != "") box.width else FrameLength(mode = "px", amount = plot.w), height = if (box.height.mode != "") box.height else FrameLength(mode = "px", amount = FILLER), round = false, centerX = unset, centerY = unset, radius = unset)
    val r = frameView(spec, width, height)
    return PyreonChartRect(x = r.x + (if (box.brush) 2.8 else 2.5), y = r.y + (if (box.brush) 6.5 else 0.0), w = r.w, h = r.h)
  }
    ...
    for (i in 0 until pts.length) {
      ...
      if (i > 0) {
        ...
        if (q.x < x1 && p.x > x1 && x1 > x0) {
          out.add(PyreonChartPt(x = x1, y = q.y + (((p.y - q.y) * (x1 - q.x))).toDouble() / ((p.x - q.x)).toDouble()))
        }
      }
      if (p.x >= x0 && p.x <= x1) {
        out.add(p)
...
fun renderSliderZoom(values: List<Double>, win: ZoomWindow, strip: PyreonChartRect, brush: Boolean): List<PyreonDrawCmd> {
    val cmds: MutableList<PyreonDrawCmd> = mutableListOf()
    val x0 = strip.x + strip.w * win.start
    val x1 = strip.x + strip.w * win.end
    val bottom = strip.y + strip.h
    var lo = 0.0
    var hi = 0.0
    var seen = false
    for (i in 0 until values.length) {
    ...
    if (seen && values.length > 1) {
      ...
      val before = cutRun(pts, strip.x, x0)
      val inside = cutRun(pts, x0, x1)
      val after = cutRun(pts, x1, strip.x + strip.w)
    ...
    cmds.add(PyreonDrawCmd(kind = "rect", rect = PyreonChartRect(x = x0, y = strip.y, w = x1 - x0, h = strip.h), fill = "rgba(135,175,255,0.2)"))
    val bx = strip.x + 0.5
    val by = strip.y + 0.5
    val bw = strip.w - 1.0
    val bh = strip.h - 1.0
    cmds.add(PyreonDrawCmd(kind = "polyline", stroke = "#e0e4f2", width = 1.0, points = listOf(PyreonChartPt(x = bx, y = by), PyreonChartPt(x = bx + bw, y = by), PyreonChartPt(x = bx + bw, y = by + bh), PyreonChartPt(x = bx, y = by + bh), PyreonChartPt(x = bx, y = by))))
    if (brush) {
      cmds.add(PyreonDrawCmd(kind = "rect", rect = PyreonChartRect(x = x0, y = strip.y - 6.5, w = x1 - x0, h = 7.0), fill = "rgba(130,146,204,0.5)", corners = listOf(2.0, 2.0, 0.0, 0.0)))
    }
    ...
    for (k in 0 until ends.length) {
      ...
      val r = 0.1 * s
      cmds.add(PyreonDrawCmd(kind = "rect", rect = grip, fill = "#ffffff", corners = listOf(r, r, r, r)))
      cmds.add(PyreonDrawCmd(kind = "polyline", stroke = "#c0c9e6", width = 1.0, points = listOf(PyreonChartPt(x = grip.x, y = grip.y), PyreonChartPt(x = grip.x + grip.w, y = grip.y), PyreonChartPt(x = grip.x + grip.w, y = grip.y + grip.h), PyreonChartPt(x = grip.x, y = grip.y + grip.h), PyreonChartPt(x = grip.x, y = grip.y))))
    }
    return cmds
  }
...
fun clampTo(v: Double, lo: Double, hi: Double): Double = if (v < lo) lo else if (v > hi) hi else v

fun brushAreaFromDrag(type: String, plot: PyreonChartRect, ax: Double, ay: Double, bx: Double, by: Double): BrushArea {
    val x0 = clampTo(ax, plot.x, plot.x + plot.w)
    val x1 = clampTo(bx, plot.x, plot.x + plot.w)
    val y0 = clampTo(ay, plot.y, plot.y + plot.h)
    val y1 = clampTo(by, plot.y, plot.y + plot.h)
    if (type == "polygon") {
      return BrushArea(type = type, points = listOf(PyreonChartPt(x = x0, y = y0)))
    }
    if (type == "lineX") {
      return BrushArea(type = type, points = listOf(PyreonChartPt(x = x0, y = plot.y), PyreonChartPt(x = x1, y = plot.y + plot.h)))
    }
    if (type == "lineY") {
      return BrushArea(type = type, points = listOf(PyreonChartPt(x = plot.x, y = y0), PyreonChartPt(x = plot.x + plot.w, y = y1)))
    }
    return BrushArea(type = "rect", points = listOf(PyreonChartPt(x = x0, y = y0), PyreonChartPt(x = x1, y = y1)))
  }

fun brushPolygonAdd(area: BrushArea, plot: PyreonChartRect, x: Double, y: Double): BrushArea {
    val px = clampTo(x, plot.x, plot.x + plot.w)
    val py = clampTo(y, plot.y, plot.y + plot.h)
    val pts: MutableList<PyreonChartPt> = mutableListOf()
    ...
    pts.add(PyreonChartPt(x = px, y = py))
    return BrushArea(type = area.type, points = pts)
  }

fun brushAreaUsable(area: BrushArea): Boolean {
    var n = 0.0
    ...
    val a = area.points[0]
    val b = area.points[1]
    val w = if (a.x < b.x) b.x - a.x else a.x - b.x
    val h = if (a.y < b.y) b.y - a.y else a.y - b.y
    if (area.type == "lineX") {
      return w >= 3.0
    }
    if (area.type == "lineY") {
...
fun pointInBrushArea(area: BrushArea, x: Double, y: Double): Boolean {
    ...
    val hx = if (a.x < b.x) b.x else a.x
    val ly = if (a.y < b.y) a.y else b.y
    val hy = if (a.y < b.y) b.y else a.y
...
fun brushDatumPoints(raw: ChartSpec, l: PlotLayout, k: Int): List<PyreonChartPt> {
    ...
    if (s.kind == "bars" || s.kind == "waterfall") {
      for (r in barsForIn(raw, k, plot)) {
        out.add(PyreonChartPt(x = r.x + (r.w).toDouble() / (2.0).toDouble(), y = if (r.w < 0.0) -1000000.0 else r.y + (r.h).toDouble() / (2.0).toDouble()))
      }
      return out
    }
    if (s.kind == "stacked" || s.kind == "grouped") {
    ...
    val dom = seriesDomain(s, spec, yDomain, resolveY2Domain(spec))
    if (spec.horizontal == true) {
...
fun brushSelection(spec: ChartSpec, l: PlotLayout, areas: List<BrushArea>): List<BrushSeriesSelection> {
    ...
    for (k in 0 until spec.series.length) {
      ...
      out.add(BrushSeriesSelection(seriesIndex = kf, dataIndex = idx))
      kf = kf + 1.0
    }
    return out
  }
...
fun brushOnlySeries(sel: List<BrushSeriesSelection>, only: List<Double>): List<BrushSeriesSelection> {
    ...
    for (x in sel) {
      ...
      out.add(if (kept) x else BrushSeriesSelection(seriesIndex = x.seriesIndex, dataIndex = listOf()))
    }
    return out
  }
...
fun renderBrushAreas(areas: List<BrushArea>, fill: String, stroke: String): List<PyreonDrawCmd> {
    val out: MutableList<PyreonDrawCmd> = mutableListOf()
    for (a in areas) {
      val ring: MutableList<PyreonChartPt> = mutableListOf()
      if (a.type == "polygon") {
        ...
        if (a.points.length >= 2) {
          ...
          ring.add(PyreonChartPt(x = q.x, y = p.y))
          ring.add(PyreonChartPt(x = q.x, y = q.y))
      ...
      edge.add(ring[0])
      out.add(PyreonDrawCmd(kind = "polyline", stroke = stroke, width = 1.0, points = edge))
    }
    return out
  }
...
fun applyChartAction(s: ChartActionState, a: ChartActionInput): ChartActionState {
    val t = a.type
    ...
    if (t == "legendInverseSelect") {
      ...
      return s.copy(hidden = next)
    }
    if (t == "dataZoom") {
      val w = clampWindow(ZoomWindow(start = a.start, end = a.end))
      val full = ZoomWindow(start = 0.0, end = 1.0)
      return s.copy(zoom = if (isFullWindow(w)) full else w)
    }
    if (t == "takeGlobalCursor") {
      return s.copy(brushType = a.brushType)
    }
    if (t == "brush") {
