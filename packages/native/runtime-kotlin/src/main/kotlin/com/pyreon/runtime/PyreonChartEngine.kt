        ...
        if (s.kind == "waterfall") {
          ...
          for (si in 0 until steps.length) {
            ...
            if (s.showValues == true && progress >= 1.0) {
              ...
              out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = fmt(v), at = PyreonChartPt(x = st.rect.x + (st.rect.w).toDouble() / (2.0).toDouble(), y = if (v < 0.0) st.rect.y + st.rect.h + 4.0 else st.rect.y - 4.0), size = t.fontSize, align = "middle", baseline = if (v < 0.0) "top" else "bottom"))
            }
          }
        } else {
          if (s.kind == "line") {
