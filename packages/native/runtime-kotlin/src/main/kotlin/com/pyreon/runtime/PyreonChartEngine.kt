...
fun rectCmd(rect: PyreonChartRect, fill: String, corners: List<Double>?): PyreonDrawCmd {
    if (corners == null) {
...
fun renderChart(spec: ChartSpec, measure: (String, Double) -> Double): List<PyreonDrawCmd> {
    ...
    if (stackedSeries.length > 0) {
      for (seg in layoutStackedBars(stackedSeries.map({ s -> s.values }), plot, yDomain, 0.25)) {
        ...
        out.add(rectCmd(rS, stackedSeries[seg.seriesIndex].color, stackedSeries[seg.seriesIndex].corners))
    ...
    if (groupedSeries.length > 0) {
      for (seg in layoutGroupedBars(groupedSeries.map({ s -> s.values }), plot, yDomain, 0.25)) {
        ...
        out.add(rectCmd(rG, groupedSeries[seg.seriesIndex].color, groupedSeries[seg.seriesIndex].corners))
    ...
    for (s in spec.series) {
      ...
      if (spec.horizontal == true) {
        ...
        for (r in rects) {
          ...
          if (s.symbol == null) {
            out.add(rectCmd(grown, s.color, s.corners))
          } else {
...
fun hitCandle(count: Int, plot: PyreonChartRect, px: Double, py: Double): Int {
    ...
    val band = (plot.w).toDouble() / (countF).toDouble()
    val posF = Math.floor(((px - plot.x)).toDouble() / (band).toDouble())
    var i = 0
    var iF = 0.0
