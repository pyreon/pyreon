...
fun renderDial(spec: DialSpec, center: PyreonChartPt, r: Double): List<PyreonDrawCmd> {
    ...
    for (d in spec.data) {
      ...
      if (spec.detailShow && d.detail != "") {
        ...
        val fill = if (spec.detailColor != "") spec.detailColor else if (spec.progressShow) d.color else auto
        if (spec.detailBold) {
    ...
    for (i in 0 until count) {
      ...
      if (spec.pointerShow) {
        val a = start + span * f
        val fill = if (d.pointerColor == "auto") auto else if (d.pointerColor != "") d.pointerColor else d.color
      ...
      if (spec.progressShow) {
        ...
        val fill = if (d.progressColor == "auto") auto else if (d.progressColor != "") d.progressColor else d.color
...
fun renderChartIn(raw: ChartSpec, measure: (String, Double) -> Double, l: PlotLayout): List<PyreonDrawCmd> {
    ...
    for (ti in 0 until l.xTicks.length) {
      ...
      if (l.xLabelRotate != 0.0) {
        out.add(PyreonDrawCmd(kind = "text", fill = t.label, text = tick.label, at = PyreonChartPt(x = tick.pos, y = if (xTop) xAxisY - 6.0 else xAxisY + 6.0), size = t.fontSize, align = if (l.xLabelRotate < 0.0) "end" else "start", baseline = "middle", rotate = if (xTop) -l.xLabelRotate else l.xLabelRotate))
      } else {
