...
const PLOT_UNLOWERED_REASON: Readonly<Record<string, string>> = {
  ...
  maxPoints:
    'row thinning is not yet built in the emitter, and the host still needs to map a tap\'s index back through the kept rows — but the ARITHMETIC crosses now (`decimate-values.ts` is in ENGINE_FILES), so decimate before the chart: `const ys: number[] = rows.map(r => r.y)` then `const keep = lttbIndices([], ys, 1000)`, and plot `keep.map(i => rows[i])`. Both emitters coerce a direct `lttbIndices`/`minMaxBuckets` call\'s series argument to Double at the call site, so an Int-valued `number[]` (no fractional initializer) needs no widening — a manual `* 1.0` is no longer required and is dead weight if you write one. `lttbIndices` returns the kept INDICES, so a tap still names the row you decimated from',
