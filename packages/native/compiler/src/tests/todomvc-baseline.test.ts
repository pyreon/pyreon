...
describe('TodoMVC gap-tracking baseline', () => {
  ...
  it('G1 — Field two-way binding emits Compose `onValueChange` on Kotlin', () => {
    // CLOSED. Phase E migrated the source from <TextField value onInput>
    // (legacy SwiftUI-flavored) to <Field value onChangeText={(t) =>
    // draft.set(t)}> (canonical Pyreon vocabulary). The canonical
    // <Field> emit threads the user-provided arrow callback verbatim
    // (with arrow-param preservation from PR #920) — so the shape is
    // `onValueChange = { t -> draft = t }`, not the legacy
    // `<TextField>` auto-derived `{ draft = it }`.
    const out = transform(source, { target: 'kotlin' })
