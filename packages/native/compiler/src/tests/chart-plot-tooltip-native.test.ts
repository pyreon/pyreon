...
describe('<PlotChart tooltip> — a tap tooltip on both targets', () => {
  it('Swift: the tap binds the local hit once, reads the crossing lines over the sliced series (the y formatter applied), and the select still speaks the same index', () => {
    ...
    expect(r.code).toContain('let pyreonLocal = plotHitBars(pyreonSpec, pyreonChartMeasure, Double(pyreonTap.location.x), Double(pyreonTap.location.y)); pyreonTip = pyreonLocal < 0 ? [] : tooltipLines(tooltipAt(pyreonLocal, pyreonCats, pyreonSeries.map { TooltipSeries(label: $0.label, values: $0.values, color: $0.color, values2: $0.values2, rValues: $0.rValues, extras: $0.extras) }), compact); pyreonTipAt = PyreonChartPt(x: Double(pyreonTap.location.x), y: Double(pyreonTap.location.y)); let i = (pyreonLocal < 0 ? -1 : pyreonLocal)')
  ...
  it('Kotlin: the same, with remembered state and the density-scaled tap', () => {
    ...
    expect(r.code).toContain('val pyreonLocal = plotHitBars(pyreonSpec, ::pyreonChartMeasure, (pyreonTap.x / pyreonDensity).toDouble(), (pyreonTap.y / pyreonDensity).toDouble()); pyreonTip = if (pyreonLocal < 0) listOf() else tooltipLines(tooltipAt(pyreonLocal, pyreonCats, pyreonSeries.map { TooltipSeries(label = it.label, values = it.values, color = it.color, values2 = it.values2, rValues = it.rValues, extras = it.extras) }), ::compact); pyreonTipAt = PyreonChartPt(')
  ...
  it('a named tooltipFormatter lowers (its string split on newlines); the grammar\'s <Tip> draws too and names crosshair as web-only', () => {
    ...
    // The prop is named AND diagnosed: a bare name is a status, not a reason.
    expect(g.warnings).toHaveLength(1)
    expect(g.warnings[0]).toContain('<PlotChart>: `crosshair` (')
    expect(g.warnings[0]).toContain('hover')
    expect(g.warnings[0]).toContain('is not lowered on native; the chart renders without.')
