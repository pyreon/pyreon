...
describe('<PlotChart tooltip> — a tap tooltip on both targets', () => {
  ...
  it('Kotlin: the same, with remembered state and the density-scaled tap', () => {
    ...
    expect(r.code).toContain('val pyreonLocal = plotHitBars(pyreonSpec, ::pyreonChartMeasure, (pyreonTap.x / pyreonDensity).toDouble(), (pyreonTap.y / pyreonDensity).toDouble()); pyreonTip = if (pyreonLocal < 0) listOf() else tooltipLines(tooltipAt(pyreonLocal, pyreonCats, pyreonSeries.map { TooltipSeries(label = it.label, values = it.values, color = it.color, values2 = it.values2, rValues = it.rValues) }), ::compact); pyreonTipAt = PyreonChartPt(')
  ...
  it('a named tooltipFormatter lowers (its string split on newlines); the grammar\'s <Tip> draws too and names crosshair as web-only', () => {
    ...
    expect(f.code).toContain('pyreonTip = pyreonLocal < 0 ? [] : describe(tooltipAt(pyreonLocal, pyreonCats, pyreonSeries.map { TooltipSeries(label: $0.label, values: $0.values, color: $0.color, values2: $0.values2, rValues: $0.rValues) })).components(separatedBy: "\\n")')
