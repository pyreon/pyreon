...
<DocText>Below the divider.</DocText>`,
  ...
  <DocPage>
    ...
    <DocText>Preview me at real A4 dimensions.</DocText>
      ...
      mistakes: [
        'Treating it as export-only chrome — it is a BROWSER preview wrapper (paper backdrop + page sizing); the actual export runs through `createDocumentExport` / `extractDocumentTree`.',
      ...
      seeAlso: ['DocDocument', 'createDocumentExport'],
    },
    {
      name: 'documentTheme',
      kind: 'constant',
...
const brandTheme = {
  ...
  colors: { ...documentTheme.colors, primary: '#0ea5e9' },
      ...
      mistakes: [
        'Mutating `documentTheme` in place — it is a shared module-level object; spread-clone it (`{ ...documentTheme, ... }`) to override, or you change it for every consumer.',
      ],
      seeAlso: ['DocDocument'],
    },
  ],
