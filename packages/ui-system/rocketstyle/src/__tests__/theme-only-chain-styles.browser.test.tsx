 ...
 *
 * `.theme()` supplies values; something has to render them. Before this,
 * nothing did unless the author also chained `.styles()` — so a theme-only
 * chain rendered COMPLETELY UNSTYLED in a browser, while
 * `@pyreon/native-compiler` reads the same `.theme()` statically and emits real
 * view modifiers. One declaration, fully styled on iOS/Android and bare on the
 * web. That is what kept rocketstyle from running the same source everywhere.
 *
 * Real Chromium, deliberately: styler writes rules through `insertRule`, whose
 * text is not in `textContent`, so happy-dom cannot see whether any CSS exists.
 */
describe('a theme-only rocketstyle chain', () => {
  const mountCard = (Card: unknown): HTMLElement => {
    const host = document.createElement('div')
    document.body.appendChild(host)
    mount(() => h(Card as never, {}, h('span', {}, 'hi')), host)
    return host.querySelector('[data-rocketstyle]') as HTMLElement
  }

  it('renders its theme values as CSS', () => {
    const Card = rocketstyle()({ name: 'ThemeOnlyCard', component: Element }).theme(() => ({
  ...
  it('does NOT double-apply when the author chained their own .styles()', () => {
    const Card = rocketstyle()({ name: 'ExplicitStylesCard', component: Element })
  ...
  it('leaves pseudo-state groups out of the base declarations', () => {
    ...
    const Card = rocketstyle()({ name: 'PseudoCard', component: Element }).theme(() => ({
      backgroundColor: '#6b7280',
