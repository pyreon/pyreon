...
- [#2541](https://github.com/pyreon/pyreon/pull/2541) [`9415d31`](https://github.com/pyreon/pyreon/commit/9415d31a864be2cb66da4775baec8f9b059203de) Thanks [@vitbokisch](https://github.com/vitbokisch)! - Bump `oxc-parser` to `^0.142.0` (from `^0.140.0`).
...
- [#2657](https://github.com/pyreon/pyreon/pull/2657) [`f498ee6`](https://github.com/pyreon/pyreon/commit/f498ee6604f0d4be0756caef5f07b30e9c1c6de9) Thanks [@vitbokisch](https://github.com/vitbokisch)! - SSR fast path: a DOM wrapper holding a COMPONENT child now compiles to `_ssr(...)`
...
- [#2515](https://github.com/pyreon/pyreon/pull/2515) [`a4a1766`](https://github.com/pyreon/pyreon/commit/a4a1766e341a4f8b3557c4d55885b183aab2d62b) Thanks [@vitbokisch](https://github.com/vitbokisch)! - Stop bailing the SSR compile-to-string fast path on nested self-closing / void elements.
...
- [#2694](https://github.com/pyreon/pyreon/pull/2694) [`9729e91`](https://github.com/pyreon/pyreon/commit/9729e91111b7d5c1414d7df5d7ed0080a904eee8) Thanks [@vitbokisch](https://github.com/vitbokisch)! - SSR now emits `class=""` for an EMPTY-STRING class resolution (nullish stays omitted), across all emission paths: `renderPropValue`, `_ssrAttr`, and both compiler backends' static bakes. The client has always materialized `class=""` for the same value (`[class]` attribute selectors distinguish presence), so omitting it server-side was a real SSR/CSR parity divergence — CSS matching `[class]` behaved differently before and after hydration — and it forced hydration adoption to pay an attribute write per row purely to materialize the attribute. The SSR render-fuzz byte-identity gate verified all four sites agree.

- [#2644](https://github.com/pyreon/pyreon/pull/2644) [`85ad5bf`](https://github.com/pyreon/pyreon/commit/85ad5bf91a6e822afbc109721b51b1cbb1422274) Thanks [@vitbokisch](https://github.com/vitbokisch)! - perf(ssr): root self-closing elements are eligible for the compile-to-string fast path

  A component whose own body is a self-closing element — `<Icon> = () => <img …/>`, `<Divider> = () => <hr/>`, `<Input> = () => <input …/>`, `<Spacer> = () => <br/>`, or a bare `<div class="box" />` — fell to the slow `h()` SSR path. [#2515](https://github.com/pyreon/pyreon/issues/2515) widened the NESTED case (a `<img/>` inside a parent no longer drops its whole enclosing component) but deliberately left the ROOT gate in place; this closes it, plus the `.map` and `<For>` item-body gates, so `items.map(i => <img src={i.src}/>)` (an image gallery) no longer bails the whole list.
  ...
  Semantics were already implemented — `ssrSerializeElement` has emitted both forms correctly since [#2515](https://github.com/pyreon/pyreon/issues/2515) (void → `<img … />` with the load-bearing space, non-void → `<div …></div>`). Only the gates were left closed. Landed in BOTH backends; the native binary is what ~80% of users compile with, so a JS-only fix would have changed nothing for them.
...
- [#2384](https://github.com/pyreon/pyreon/pull/2384) [`bf97dfa`](https://github.com/pyreon/pyreon/commit/bf97dfa0f6f0f7f341625a4a4d14bf100e869163) Thanks [@vitbokisch](https://github.com/vitbokisch)! - perf(ssr): fuse per-item row bodies into a direct concat — Pyreon is now the fastest SSR renderer at every list size
...
- [#2074](https://github.com/pyreon/pyreon/pull/2074) [`32e1c66`](https://github.com/pyreon/pyreon/commit/32e1c660b4d1da33c592ef5165774981843f8180) Thanks [@vitbokisch](https://github.com/vitbokisch)! - Two root-cause fixes at the template child/attr classification seam (both backends, byte-identical):
...
- compiler: bare-signal attribute values now bind directly (`_bindDirect`), matching the accessor form (ab44b96)
