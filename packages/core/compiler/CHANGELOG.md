...
- New fixable detector, `charts-legacy-import`. `pyreon check --fix` and the MCP `migrate_pyreon` tool now migrate code written against `@pyreon/charts` 0.51: (d5a7c06)
...
- Detector findings can be suppressed inline, with the comment `@pyreon/lint` already uses (96426be)
...
- Bake LITERAL expression children into the template HTML. `{"t"}`, `{54}`, (a01e106)
...
- Two detector/pre-pass fixes found while covering the compiler's branches. (b6cda55)
...
- Two template-emit fixes found by the new compiled-path hydration parity fuzz, (8429598)
...
- fix(compiler): the compiled SSR path emitted a `javascript:` URL that every other path blocked (a370824)
...
- fix(compiler): text fusion rendered HTML entities as literal characters (55699c3)
...
- Text fusion: a static-text run around an interpolation — `<p>Hello {name}!</p>`, (f4e9268)
...
- fix(core,compiler): the URL guard is bypassable by an obfuscated scheme on every path (c26fcef)
