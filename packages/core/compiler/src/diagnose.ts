...
<div class="controls"><Show when={showLock}><button>Lock</button></Show></div>`,
  ...
  },
  {
    ...
    diagnose: () => ({
      ...
      fix: 'Upgrade `@pyreon/runtime-dom` — new `<For>` entries mounted at the tail now carry a `pos` that places them AFTER every survivor, so the LIS moves each new row to its logical slot. No app code change needed. If you cannot upgrade, force a full re-render for that update (change the container `key`, or clear then re-set the array) so the reconciler rebuilds from scratch instead of reordering.',
