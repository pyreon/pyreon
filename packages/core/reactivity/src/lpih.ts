...
async function _writeToPath(path: string): Promise<number> {
  ...
  // Single try/catch covering BOTH writeFile AND rename. The previous shape only
  // guarded the rename — if `fs.writeFile` itself threw (disk full, EIO, EACCES,
  // transient FS error), the partial tmp file leaked on disk with a unique PID+seq
  // name.
  try {
    ...
    // Covers BOTH paths: writeFile-failed (tmp may not exist → unlink ENOENT,
    // swallowed) AND rename-failed (tmp exists).
    try {
      await fs.unlink(tmp)
    } catch {
...
function _startPollingAt(path: string, intervalMs: number): () => void {
  ...
  const tick = async (): Promise<void> => {
    ...
    if (active) {
      ...
      // Node-only; the setTimeout return type in browsers is a number with no .unref.
