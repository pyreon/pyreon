...
// ─── Default sanitizer SEAM ─────────────────────────────────────────────────
// The fallback allowlist sanitizer (~100 tag strings + the sanitize walker)
// lives in ./sanitizer and REGISTERS itself here — apps that never use the
// sanitized `innerHTML` prop (or `sanitizeHtml()`) tree-shake all of it.
// @pyreon/vite-plugin auto-injects the registration import when a module's
// source uses `innerHTML`; non-Vite consumers add
// `import '@pyreon/runtime-dom/sanitizer'` once (the throw below names the
// fix). `dangerouslySetInnerHTML` stays raw (React semantics) and never
// needs the sanitizer.
let _defaultSanitizer: SanitizeFn | null = null
/** @internal Registered by ./sanitizer at module load. */
export function _setDefaultSanitizer(fn: SanitizeFn): void {
  _defaultSanitizer = fn
}

export function sanitizeHtml(html: string): string {
  if (_customSanitizer) return _customSanitizer(html)
  if (_defaultSanitizer) return _defaultSanitizer(html)
  throw new Error(
    "[Pyreon] The sanitized `innerHTML` prop (or sanitizeHtml()) needs a sanitizer. " +
      "Add `import '@pyreon/runtime-dom/sanitizer'` once (auto-injected by @pyreon/vite-plugin), " +
      'or register your own via setSanitizer((html) => ...). ' +
      '`dangerouslySetInnerHTML` bypasses sanitization and needs neither.',
  )
}
...
// only for `on*` — a strict subset, so don't treat them as disjoint. Useful
// ratios: applyEvent/applyProp = handler density; the difference = attr density.
...
// Track the CSS property names an element's last-applied style object set, so a
// reactive style going from `{ color, fontSize }` to `{ color }` removes the
