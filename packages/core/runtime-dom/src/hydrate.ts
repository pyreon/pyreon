      ...
      if (end) {
        ...
        _setPendingForAdoption({
          ...
          adoptRows: (ops) => {
            ...
            const { items, n, getKey, renderItem, tailMarker: opsTail, setEntry } = ops
            ...
            for (let i = 0; i < n; i++) {
              const row = parsedRows[i]!
              const after: Node = i + 1 < n ? parsedRows[i + 1]!.marker : opsTail
              ...
              if (tplAdopted) {
                ...
                cleanup =
                  ...
                  hydrateChild(rowVNode as VNodeChild, row.first, parent, after, `${path}.for`)[0]
