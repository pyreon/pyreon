...
export const Image = (props: ImageProps): VNode => {
  ...
  // The canonical src dispatch (asset-pipeline arc): a BARE name (no scheme, no slash)
  // is a bundled asset on native (Assets.xcassets / res/drawable via PMTC) — on web the
  // assets step materializes the shared assets/ dir under /assets/, so the same source
  // resolves here. Scheme'd URLs and path-style srcs pass through untouched.
  const src =
