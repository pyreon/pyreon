...
describe('hydration blob — same-path layout+page collision', () => {
  it('serialize emits DISTINCT keys for same-path records, hydrate maps them back', () => {
    ...
    const server = createRouter({ routes, mode: 'history', url: '/dash' }) as RouterInstance
    ...
    const client = createRouter({ routes, mode: 'history', url: '/dash' }) as RouterInstance
  ...
  it('non-colliding chains keep the EXACT legacy bare-path format', () => {
    ...
    const router = createRouter({ routes: [{ path: '/', component: C, children: [page] } as RouteRecord], mode: 'history', url: '/about' }) as RouterInstance
