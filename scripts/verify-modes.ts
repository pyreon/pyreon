...
interface Cell {
  ...
  /**
   * Central per-path mode overrides (Tier-4). Forwarded to
   * `zero({ routeRules })` in the generated verify config — asserts the
   * rules flow through build-time route filtering end-to-end (e.g. a rule
   * declaring a route 'spa' makes the SSG build emit the CSR shell there).
   */
...
const MATRIX: Cell[] = [
  ...
  // ─── Tier-4: routeRules — central glob overrides flow through the build ────
  // A rule declares /about 'spa' inside a FULL-SSG build: the SSG plugin must
  // emit the CSR SHELL at /about (file exists, prerendered content absent)
  // while every other route prerenders normally, incl. getStaticPaths posts.
  // Bisect: drop the __routeRules inlining from renderSsgEntrySource →
  // /about prerenders with real content → the DoesNotContain assert fails.
  {
    example: 'cpa-pw-blog',
    mode: 'ssg',
    routeRules: { '/about': { renderMode: 'spa' } },
    smoke: (dist) => {
      assertFileExists(join(dist, 'index.html'))
      // Rule-declared 'spa' route → CSR shell: the file EXISTS (it's a real
      // URL static hosts must serve) but carries no prerendered about
      // content (placeholders blanked).
      const about = join(dist, 'about', 'index.html')
      assertFileExists(about)
      assertFileDoesNotContain(about, '<h1>About</h1>')
      // Rules must not disturb the enumerated dynamic routes.
      assertFileExists(join(dist, 'blog', 'welcome', 'index.html'))
    },
  },

  // ─── Tier-4: mode 'auto' (EXPERIMENTAL) — inference end-to-end ─────────────
  // ssr-showcase under mode:'auto': loaders exist → the app pipeline is
  // inferred 'ssr' (server bundle emitted); no-loader routes are inferred
  // 'ssg' and prerendered; loader routes are inferred 'ssr' and NOT
  // prerendered; explicit declarations (hybrid-static 'ssg', hybrid-spa
  // 'spa') are preserved untouched. Bisect: force inferRouteMode to 'ssg'
  // → posts/index.html gets prerendered → the Absent assert fails.
  {
    example: 'ssr-showcase',
    mode: 'auto',
    smoke: (dist) => {
      // inference chose the server pipeline
      assertFileExists(join(dist, 'server', 'entry-server.js'))
      // no-loader route inferred 'ssg' → prerendered with content
      assertFileContains(join(dist, 'about', 'index.html'), 'about-page')
      // loader route inferred 'ssr' → NOT prerendered
      assertFileAbsent(join(dist, 'posts', 'index.html'))
      // explicit declarations preserved
      assertFileContains(join(dist, 'hybrid-static', 'index.html'), 'hybrid-static-stamp')
      assertFileAbsent(join(dist, 'hybrid-spa', 'index.html'))
    },
  },
...
function configSourceFor(cell: Cell): string {
  ...
  const routeRulesConfig = cell.routeRules ? `, routeRules: ${JSON.stringify(cell.routeRules)}` : ''
...
export default defineConfig({
  plugins: [pyreon(${cell.collapse ? '{ collapse: true }' : ''}), zero({ mode: ${JSON.stringify(cell.mode)}${baseConfig}${adapterConfig}${ssgConfig}${i18nConfig}${routeRulesConfig} })${seoPluginCall}],
