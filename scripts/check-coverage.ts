...
const BELOW_FLOOR_EXEMPTIONS: Record<string, FloorExemption> = {
  ...
  '@pyreon/runtime-dom': {
    ...
    currentBranches: 90,
    reason:
      'DOM renderer. Ratcheted 92/83 -> 96/90 by the 92%+ campaign (measured 96.80/90.51, functions 99.01, lines 97.68). Branches do NOT clear 92 yet and the residual is named rather than smoothed over. The lift targeted the hydration machinery, which is where this package\'s failures are silent: the row-plan bail contract on its VNODE side (a plan compiled from row 0 replayed over rows whose vnodes differ — the DOM side agrees with itself, so row 0\'s bindings land on a row that needed different ones), the compiled-template adopt verifier\'s marker scanning plus its rows-2..N plan replay, mismatch recovery (which turned up two shipped bugs: a tag mismatch recovering at the END of the child list, and an unparseable <For> rendering its list twice), async-component hydration, and the dev overlays that install global capture-phase listeners on the user\'s own app. What remains is ~230 arms dominated by TypeScript-forced defensive narrowings (`?? parent`, `if (!entry) continue`, `if (!ref.current) return`) that the public API cannot produce, plus the compiled-slot hydration path, which needs the `transformJSX`-compiling harness the map-slot suites use rather than hand-built DOM. Raise in lockstep; never lower.',
  },
  '@pyreon/vue-compat': {
