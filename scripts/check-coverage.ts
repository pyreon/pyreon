...
const BELOW_FLOOR_EXEMPTIONS: Record<string, FloorExemption> = {
  ...
  '@pyreon/vite-plugin': {
    ...
    currentBranches: 90,
    reason:
      'Vite plugin. Ratcheted 94/87 -> 95/90 by the 92%+ campaign (measured 95.34/90.33, functions 94.28, lines 95.83) — still under the 92 bar on branches, and under the gate\'s 95, with the residual named. Four suites landed over surface that had none: the transform hook\'s early-exit LADDER (each rung returns early, so one firing when it should not silently skips every transform below it — a missing island name makes the marker and the registry disagree and the island never hydrates, with nothing in the build output); the SSR compile-to-string capability gate, which the anti-pattern catalog records shipping broken once already (flipped default-on it injected `@pyreon/runtime-server` into app source and deterministically 500\'d an app that depends on it only transitively, because a strict install never exposes a transitive dep); the balanced-args parser (the hand-rolled-parser-over-escape-contexts class); and the auto-import merge (the `_rp` injected-import collision class). The remaining ~69 arms are dominated by integration-tier paths — the `configureServer` dev middleware needs a real dev server, and the rocketstyle-collapse resolver boots a NESTED Vite build — proven by the zero-hmr and ssr-showcase e2e rather than by the node runner.',
  },
  '@pyreon/solid-compat': {
    currentStatements: 96,
    currentBranches: 91,
