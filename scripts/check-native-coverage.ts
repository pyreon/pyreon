...
`,
  ...
  },
  {
    ...
    mechanism: 'pmtc-lowers',
    rationale:
      'createFlow/useFlow and the interactive Flow host lower to PyreonFlowState + SwiftUI/Compose on both targets: complete public state operations, all seven layouts, gestures, built-in/static custom renderers, handles, resizing, multiple toolbars, custom edges/connection lines, background, controls, minimap, panels, accessibility, and portable inline styles. Device-proven on iOS Simulator and Android Emulator by the native-counter and native-tasks apps (rendered chrome read from pixels and frames, every gesture, the keyboard commands, colorMode including system, a 400-node culled scale scenario); the state surface is replayed against the web engine as oracle. Inline <svg> shapes and plain div/p/span structure inside a renderer lower natively (device-proven by a native-tasks custom node); CSS-dependent renderers, SVG text/gradients/transforms and runtime renderer maps remain explicit platform boundaries that warn by name.',
...
`,
  },
  {
    ...
    mechanism: 'webview-host',
    rationale:
      'Optional compatibility bridge for applications that deliberately need the browser SVG/DOM/CSS renderer unchanged inside WKWebView or Android WebView; the regular @pyreon/flow editor has a direct SwiftUI/Compose lowering path. Bridge proven in real Chromium, and device-proven on iOS Simulator and Android Emulator by examples/native-tasks: graph and command push in, selection and viewport events back out, once-only commands, and a reactive html swap that re-injects the bridge.',
    snippet: `import { signal } from '@pyreon/reactivity'
import { Stack, Text, WebView } from '@pyreon/primitives'
export function C() {
  const nums = signal([1, 2, 3])
